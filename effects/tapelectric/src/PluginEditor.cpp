...
{
    setSize(580, 620); // Increased height for additional controls
    ...
    setupSlider(humDriftRateSlider, humDriftRateLabel, "Drift Rate", "humDriftRate", humDriftRateAtt);
    setupSlider(humDriftDepthSlider, humDriftDepthLabel, "Drift Depth", "humDriftDepth", humDriftDepthAtt);
    setupSlider(saturationSlider, saturationLabel, "Saturation", "saturation", saturationAtt);
    ...
    setupSlider(inputVolumeSlider, inputVolumeLabel, "Input Mix", "inputVol", inputVolAtt);
    setupSlider(inputDriftSlider, inputDriftLabel, "Input Drift", "inputDrift", inputDriftAtt);
    setupSlider(inputDriftRateSlider, inputDriftRateLabel, "Drift Rate", "inputDriftRate", inputDriftRateAtt);
    setupSlider(inputDriftDepthSlider, inputDriftDepthLabel, "Drift Depth", "inputDriftDepth", inputDriftDepthAtt);
    setupSlider(inputWobbleSlider, inputWobbleLabel, "Wobble", "inputWobble", inputWobbleAtt);
    setupSlider(inputWobbleRateSlider, inputWobbleRateLabel, "Wob Rate", "inputWobbleRate", inputWobbleRateAtt);
    setupSlider(inputWobbleDepthSlider, inputWobbleDepthLabel, "Wob Depth", "inputWobbleDepth", inputWobbleDepthAtt);
    setupSlider(inputDelayBaseSlider, inputDelayBaseLabel, "Delay Base", "inputDelayBase", inputDelayBaseAtt);
    setupSlider(inputDelayModSlider, inputDelayModLabel, "Delay Mod", "inputDelayMod", inputDelayModAtt);
...
void TapElectricAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    g.drawText("RESONANCES", 15, 245, 150, 20, juce::Justification::left);
    g.drawText("NOISE & INPUT", 15, 435, 150, 20, juce::Justification::left);
    ...
    const juce::Colour& color, const juce::String& label)
{
    ...
    float qVal = eq.qSlider.getValue(); // <--- NEW: Get Q value from the slider
    ...
    // Draw frequency, gain, and Q text
    ...
    juce::String qText = "Q " + juce::String(qVal, 1); // <--- NEW: Format Q string

    // <--- NEW: Updated drawText call
    g.drawText(freqText + "| " + gainText + "| " + qText,
...
void TapElectricAudioProcessorEditor::resized()
{
    ...
    // ROW 1B: Extended Hum Drift controls (smaller knobs)
    int r1b = 180;
    const int smallKnobSize = 45;
    auto layoutSmallControl = [&](int col, int rowY, juce::Slider& s, juce::Label& l) {
        int x = leftMargin + (col * colWidth);
        l.setBounds(x, rowY, colWidth, labelHeight);
        s.setBounds(x + (colWidth - smallKnobSize) / 2, rowY + labelHeight, smallKnobSize, smallKnobSize);
        s.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 60, 14);
    };
    layoutSmallControl(3, r1b, humDriftRateSlider, humDriftRateLabel);
    layoutSmallControl(4, r1b, humDriftDepthSlider, humDriftDepthLabel);
    layoutSmallControl(5, r1b, saturationSlider, saturationLabel);
    ...
    // ROW 4: Extended Input Drift/Wobble controls (small knobs)
    int r4 = 540;
    layoutSmallControl(0, r4, inputDriftRateSlider, inputDriftRateLabel);
    layoutSmallControl(1, r4, inputDriftDepthSlider, inputDriftDepthLabel);
    layoutSmallControl(2, r4, inputWobbleRateSlider, inputWobbleRateLabel);
    layoutSmallControl(3, r4, inputWobbleDepthSlider, inputWobbleDepthLabel);
    layoutSmallControl(4, r4, inputDelayBaseSlider, inputDelayBaseLabel);
    layoutSmallControl(5, r4, inputDelayModSlider, inputDelayModLabel);
}
