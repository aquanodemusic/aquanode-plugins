...
juce::AudioProcessorValueTreeState::ParameterLayout PrecisionUtilityAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>(
        "delay",
        "Delay",
        juce::NormalisableRange<float>(0.0f, 10000.0f, 0.025f),
        0.0f,
        "ms"));
...
void PrecisionUtilityAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float volLeft = apvts.getRawParameterValue("volLeft")->load();
    float volRight = apvts.getRawParameterValue("volRight")->load();
    ...
    // Apply per-channel volume first (before any other processing)
    if (totalNumInputChannels >= 1)
        buffer.applyGain(0, 0, bufferSize, volLeft);
    if (totalNumInputChannels >= 2)
        buffer.applyGain(1, 0, bufferSize, volRight);
