...
private:
    ...
    struct Resonate
    {
        ...
        // Membrane modulator (fixed-amount wobble; membraneSamples is a
        // hardcoded constant now, no chorus/LFO knobs behind it)
        uint32_t noiseSeed[2] = { 12345u, 67890u };
        float    membraneFilter[2] = { 0.0f, 0.0f };
        double   membraneSamples = 0.4;

        // DC-offset accumulator (always-on bipolar leaky integrator)
    ...
    double globalSmooth = 0.0;    // knob removed; always 0, filtering behaviour unchanged
