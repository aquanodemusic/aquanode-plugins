/*
  ==============================================================================
    ...
    Layout (logical coordinates -- the whole thing is scaled by a transform):
      Col 1  - Filter (On/Off, Type, Frequency), MIDI In
      Col 2  - Res Mode (Global/Individual), Shape Mode (Full/Odd), Color,
               Decay, Const, Save/Load Preset
      Col 3-9- Resonators I-VII (Decay + Const + Color + Pan expand on Per Res)
                Only Resonator I is on by default; II-VII start off.
      Col 8  - Width, Gain, Dry/Wet, Wet Only (right-most column)
    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    // ── Column 2: Res Mode / Shape Mode / Color / Decay / Const / Presets ──
    content.addAndMakeVisible(modeSelector);
    content.addAndMakeVisible(modeLabel);
    modeSelector.addItem("Full", 1);
    modeSelector.addItem("Odd", 2);
...
// Filter, Width, Gain, Dry/Wet) stays global and stays live.
...
//==============================================================================
// Laid out in LOGICAL coordinates -- unchanged pixel maths, only scaled.
void ResonateAudioProcessorEditor::layoutContent()
{
    ...
    // ── Column 1: Filter -> Presets -> MIDI -> Branding ───────────────────────
    ...
    // Extra vertical spacing above Preset Save/Load
    col1.removeFromTop(spacing * 2);

    // 4. Preset Save / Load
    presetLabel.setBounds(col1.removeFromTop(labelH));
    {
        auto presetRow = col1.removeFromTop(24);
    ...
    // 6. Branding Logo (Centered horizontally in Column 1)
    auto brandingArea = col1.removeFromTop(60);
    branding.setBounds(brandingArea.withSizeKeepingCentre(80, 60));

    bounds.removeFromLeft(spacing);

    // ── Column 2: Res Mode / Shape Mode / Color / Decay / Const ───────────────
