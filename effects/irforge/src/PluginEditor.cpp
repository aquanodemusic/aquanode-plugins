...
{
    // getSourceGeneration() bumps on every load — including a windowed
    // reload that leaves the sample count identical to before, which is why
    // this used to compare sample counts and silently miss the update.
    const int gen = processor.getSourceGeneration();
...
void SourceDisplay::refreshOverview()
{
    const juce::ScopedLock sl(processor.sourceLock);
    ovMin = processor.getOverviewMin();
    ovMax = processor.getOverviewMax();
    totalSeconds = processor.getOverviewTotalSeconds();
}

float SourceDisplay::maxSpanFrac() const
{
    if (!processor.isFileBacked()) return 1.0f;
    const double total = processor.getSourceFileTotalSeconds();
    if (total <= 0.0) return 1.0f;
    return (float)juce::jlimit(0.0, 1.0, processor.getMaxLoadWindowSeconds() / total);
}
...
void SourceDisplay::paint(juce::Graphics& g)
{
    ...
    const bool hasSrc = processor.hasSource() && !ovMax.empty();
    ...
    // waveform, dim outside the crop region and lit inside it. This is the
    // whole-timeline overview, not the windowed audio actually resident in
    // memory, so it never shifts when the resident window moves.
    const int cols = (int)ovMax.size();
    for (int c = 0; c < cols; ++c)
    {
        ...
        const float y0 = mid - ovMax[(size_t)c] * halfH;
        const float y1 = mid - ovMin[(size_t)c] * halfH;
...
{
    const float n = xToNorm((float)e.x);
    const float maxSpan = maxSpanFrac();
    ...
    // Handles can never be pulled further apart than maxSpanFrac allows — for
    // a file-backed source longer than the load window, that's the window
    // size, since that's the most that can ever be resident for building at
    // once. The actual disk read to follow the crop happens once the drag
    // finishes (mouseUp), not on every drag frame.
    if (draggingStart)
        set(pid::cropStart, juce::jlimit(juce::jmax(0.0f, ce - maxSpan), ce - 0.002f, n));
    else if (draggingEnd)
        set(pid::cropEnd, juce::jlimit(cs + 0.002f, juce::jmin(1.0f, cs + maxSpan), n));
    repaint();
}

void SourceDisplay::mouseUp(const juce::MouseEvent&)
{
    ...
    const float cs = processor.apvts.getRawParameterValue(pid::cropStart)->load();
    const float ce = processor.apvts.getRawParameterValue(pid::cropEnd)->load();
    processor.ensureWindowCovers(cs * total, ce * total);
}

void SourceDisplay::mouseDoubleClick(const juce::MouseEvent&)
{
    const float span = maxSpanFrac();
    for (auto pair : { std::make_pair(pid::cropStart, 0.0f), std::make_pair(pid::cropEnd, span) })
        if (auto* p = processor.apvts.getParameter(pair.first))
            p->setValueNotifyingHost(pair.second);

    const double total = processor.getSourceFileTotalSeconds();
    processor.ensureWindowCovers(0.0, span * total);
    repaint();
}
...
void IRForgeAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    // a cool blue glow behind the shape section, in the middle
    auto glowCentreMid = juce::Point<float>(getWidth() * 0.5f, getHeight() * 0.78f);
    juce::ColourGradient glowMid(Palette::cool().withAlpha(0.10f), glowCentreMid.x, glowCentreMid.y,
        Palette::cool().withAlpha(0.0f),
        glowCentreMid.x + getWidth() * 0.30f, glowCentreMid.y, true);
    g.setGradientFill(glowMid);
    g.fillEllipse(juce::Rectangle<float>(getWidth() * 0.6f, getWidth() * 0.6f)
        .withCentre(glowCentreMid));

    // a green glow behind the output section, on the right
    auto glowCentreRight = juce::Point<float>(getWidth() * 0.87f, getHeight() * 0.78f);
    juce::ColourGradient glowRight(Palette::outputGreen().withAlpha(0.10f), glowCentreRight.x, glowCentreRight.y,
        Palette::outputGreen().withAlpha(0.0f),
        glowCentreRight.x + getWidth() * 0.30f, glowCentreRight.y, true);
    g.setGradientFill(glowRight);
    g.fillEllipse(juce::Rectangle<float>(getWidth() * 0.6f, getWidth() * 0.6f)
        .withCentre(glowCentreRight));
