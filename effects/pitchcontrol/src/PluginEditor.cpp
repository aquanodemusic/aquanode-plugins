...
void LabelledKnob::reattach(const juce::String& newParamID,
                             juce::AudioProcessorValueTreeState& apvts)
{
    // Destroy old attachment first, then create the new one
    m_attach.reset();
    m_attach = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
...
void PitchControlAudioProcessorEditor::updateModeUI(bool fftMode)
{
    if (fftMode)
    {
        // Depth slot → Ring Shift (electrify)
        m_depthKnob.reattach(PitchControlAudioProcessor::electrifyParamID(),
                              audioProcessor.apvts);
        m_depthKnob.setLabelText("Ring Shift");

        // Boost Q slot → Pull
        m_boostQKnob.reattach(PitchControlAudioProcessor::pullParamID(),
                               audioProcessor.apvts);
        m_boostQKnob.setLabelText("Pull");
        m_boostQKnob.setVisible(true);

        // Shift Strength slot → its own parameter (visible)
        m_shiftStrengthKnob.reattach(PitchControlAudioProcessor::shiftStrengthParamID(),
                                      audioProcessor.apvts);
        m_shiftStrengthKnob.setLabelText("Shift Strength");
    ...
    {
        // Depth slot → Depth (dB)
        m_depthKnob.reattach(PitchControlAudioProcessor::depthParamID(),
                              audioProcessor.apvts);
        m_depthKnob.setLabelText("Depth (dB)");

        // Boost Q slot → Boost Q
        m_boostQKnob.reattach(PitchControlAudioProcessor::boostQParamID(),
                               audioProcessor.apvts);
        m_boostQKnob.setLabelText("Boost Q");
        m_boostQKnob.setVisible(true);

        // Shift Strength slot → Output Gain
        m_shiftStrengthKnob.reattach(PitchControlAudioProcessor::outputGainParamID(),
                                      audioProcessor.apvts);
        m_shiftStrengthKnob.setLabelText("Output Gain");
        m_shiftStrengthKnob.setVisible(true);
...
void PitchControlAudioProcessorEditor::resized()
{
    ...
    // Controls row — 5 knobs + right-hand stack, all fitting the existing width
    const int ctrlY = 394;
    const int ctrlH = 135;
    const int knobW = 90;   // slightly narrower than before (was 95)
    const int gap   = 8;    // slightly tighter gap (was 10)
    const int stackW = 140;

    // 5 * 90 + 4 * 8 = 450 + 32 = 482 knobs width
    // 482 + 8 + 140 = 630 total — fits in W=632 with 1px to spare each side
    const int totalKnobsW = knobW * 5 + gap * 4;
    const int totalW = totalKnobsW + gap + stackW;
    const int startX = margin + (W - totalW) / 2;

    m_depthKnob         .setBounds(startX,                     ctrlY, knobW, ctrlH);
    m_qKnob             .setBounds(startX + 1*(knobW + gap),   ctrlY, knobW, ctrlH);
    m_boostKnob         .setBounds(startX + 2*(knobW + gap),   ctrlY, knobW, ctrlH);
    m_boostQKnob        .setBounds(startX + 3*(knobW + gap),   ctrlY, knobW, ctrlH);
    m_shiftStrengthKnob .setBounds(startX + 4*(knobW + gap),   ctrlY, knobW, ctrlH);
