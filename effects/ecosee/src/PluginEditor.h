...
private:
    ...
    // Backs the "Visible Points" slider (VS_POINT_COUNT): every
    // datapoint-edge-datapoint panel iterates history indices
    // [0, kHistorySize) as before, but skips anything before this index, so
    // only the newest N frames are drawn/connected while the full history
    // buffer keeps recording underneath regardless of N.
    int visibleHistoryStart() const;

    // Buffers up one unbroken run of visible, audible points (a "run" ends
    // wherever isAudible() fails, matching the old havePrev-reset gap
    // behaviour) and renders it either as the classic dot-by-dot polyline
    // or, if VS_SNAKE_MODE is on, as a single smooth faded trail -- shared
    // by every datapoint-edge-datapoint panel so both draw styles and the
    // Visible Points slider behave identically everywhere. Clears the three
    // vectors after drawing so the caller can keep reusing them for the
    // next run.
    void flushTrailRun (juce::Graphics& g, std::vector<juce::Point<float>>& pts,
                         std::vector<juce::Colour>& colours, std::vector<float>& radii,
                         bool snakeMode, bool useGlowLine, float lineThickness);

    // The "smooth snake" renderer: a Catmull-Rom spline through `pts`,
    // subdivided so the curve glides continuously between real datapoints
    // instead of visiting them as sharp vertices, coloured by interpolating
    // between each pair's colours and faded from a dim tail (oldest visible
    // point) to a fully bright, glowing head (newest point).
    void drawSnakeTrail (juce::Graphics& g, const std::vector<juce::Point<float>>& pts,
                          const std::vector<juce::Colour>& colours, const std::vector<float>& radii,
                          bool useGlowLine, float lineThickness, float tailBrightness, float headBrightness);
