...
void EcoSeeAudioProcessorEditor::drawSpreadEntropySpace (juce::Graphics& g, juce::Rectangle<float> area)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (! isAudible (f))
        {
            havePrev = false; // break the trail across the silent gap
            continue;
        }
        ...
        if (havePrev)
        {
            glowLine (g, prev, sp, colour, 1.2f);
            fineWhiteLine (g, prev, sp);
        }

        glowDot (g, sp, 1.9f + f.amplitude01 * 3.0f, colour);
...
void EcoSeeAudioProcessorEditor::drawToneMap (juce::Graphics& g, juce::Rectangle<float> area)
{
    ...
    juce::Point<float> toneMapPrev;
    bool haveToneMapPrev = false;
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (! isAudible (f))
        {
            haveToneMapPrev = false; // break the trail across the silent gap
            continue;
        }
        ...
        if (haveToneMapPrev)
            fineWhiteLine (g, toneMapPrev, pos);

        glowDot (g, pos, 1.5f + yN * 2.4f, colour);
        maybeLabelPoint (g, pos, Vec3f { f.centroidHz, f.amplitude01, (float) i }, i);

        toneMapPrev = pos;
        haveToneMapPrev = true;
    }
}
...
void EcoSeeAudioProcessorEditor::drawFmAmCube (juce::Graphics& g, juce::Rectangle<float> area)
{
    ...
    for (int i = 1; i < n; ++i)
    {
        ...
        glowDot (g, sp, 1.6f + f.amplitude01 * 2.4f, colour);
