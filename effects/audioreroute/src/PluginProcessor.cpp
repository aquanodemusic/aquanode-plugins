...
std::atomic<float>        AudioRerouterPluginAudioProcessor::channelTransmitterThreshold[8];
...
AudioRerouterPluginAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterBool>("afc", "Auto Feedback Ctrl", false));
...
void AudioRerouterPluginAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    // Always report zero latency — PDC compensation is not appropriate for
    // a feedback-loop plugin: reporting non-zero latency causes the DAW to
    // delay other tracks, which desynchronises the feedback path.
    setLatencySamples(0);

    // AFC per-instance state
    afcWetGain = 1.0f;
    afcPrevPeak = 0.0f;
    // Attack: traverse full 0→1 range in ~0.5 s (fast enough to catch runaway feedback)
    afcAttackRate = 1.0f / (0.5f * static_cast<float>(sampleRate));
    // Release: traverse full range in ~20 s normally; 5× faster when signal is
    // already trending downward (AFC oversteered — bring wetness back up sooner).
    afcReleaseRate = 1.0f / (20.0f * static_cast<float>(sampleRate));
    ...
    juce::MidiBuffer& midiMessages)
{
    ...
    if (!mode)
    {
        ...
        // Publish the effective ceiling for this channel so AFC receivers know
        // the safe peak level.  1.0 when the limiter is disabled (full scale).
        channelTransmitterThreshold[channel].store(limiterOn ? threshold : 1.0f);
    ...
    else
    {
        ...
        // ── Wet level / Automatic Feedback Control ────────────────────────
        // When AFC is off: apply the user's wet gain parameter as before.
        // When AFC is on: the user's wet knob is frozen; AFC computes its own
        // multiplier (afcWetGain) that keeps the wet signal below the channel's
        // effective threshold ceiling.
        //
        // The "effective threshold" is the minimum of:
        //   • The transmitter's ceiling (limiterEnabled ? threshold : 1.0)
        //   • This receiver's own ceiling (same rule)
        // This ensures that AFC respects every limiter that is active anywhere
        // in the plugin chain on this channel.
        //
        // Peak is measured on the raw ring-buffer content (before any gain) so
        // that the applied-peak estimate (rawPeak × afcWetGain) is accurate.
        {
            const bool afcOn = apvts.getRawParameterValue("afc")->load() > 0.5f;

            if (afcOn)
            {
                // ── Measure raw wet peak ───────────────────────────────────────
                float rawWetPeak = 0.0f;
                for (int ch = 0; ch < totalOut; ++ch)
                {
                    const float* data = buffer.getReadPointer(ch);
                    for (int s = 0; s < numSamples; ++s)
                        rawWetPeak = std::max(rawWetPeak, std::abs(data[s]));
                }

                // ── Compute target ceiling ────────────────────────────────────
                const float txThresh = channelTransmitterThreshold[channel].load();
                const float ownThresh = limiterOn ? threshold : 1.0f;
                const float target = std::min(txThresh, ownThresh);

                // ── AFC gain update ───────────────────────────────────────────
                constexpr float kAfcFloor = 1.0e-4f; // ≈ -80 dBFS
                const float appliedPeak = rawWetPeak * afcWetGain;

                if (appliedPeak > target)
                {
                    // Over ceiling → reduce gain (fast attack)
                    afcWetGain -= afcAttackRate * static_cast<float>(numSamples);
                    afcWetGain = std::max(0.0f, afcWetGain);
                }
                else if (afcWetGain < 2.0f && rawWetPeak > kAfcFloor)
                {
                    // Under ceiling WITH actual signal → recover wet gain (slow release).
                    // No signal at all: leave afcWetGain where it is — silence × any gain
                    // is still silence, and climbing to 2.0 in silence is misleading.
                    //
                    // Release is faster when the signal is trending downward (oversteering):
                    // the loop is dying out faster than needed, so recover quicker.
                    const float trend = rawWetPeak / (afcPrevPeak + 1.0e-9f);
                    const float factor = (trend < 0.9f) ? 5.0f : 1.0f;
                    afcWetGain += afcReleaseRate * factor * static_cast<float>(numSamples);
                    afcWetGain = std::min(2.0f, afcWetGain); // full wet-slider range
                }

                afcPrevPeak = rawWetPeak;
                afcCurrentGain.store(afcWetGain, std::memory_order_relaxed);

                buffer.applyGain(afcWetGain);
            }
            else
            {
                // Manual wet control — reset AFC state so it starts cleanly if
                // the user re-enables it later.
                afcWetGain = 1.0f;
                afcPrevPeak = 0.0f;
                afcCurrentGain.store(wetGain, std::memory_order_relaxed);
