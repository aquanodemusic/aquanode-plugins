...
public:
    ...
    void timerCallback() override; // polls afcCurrentGain → animates wet knob

private:
    // Connects or disconnects the wet APVTS attachment depending on whether
    // AFC is active.  When active, the attachment is destroyed so we can drive
    // the slider display manually; when inactive it is recreated so the user
    // can control the knob normally.
    void setAfcActive(bool active);

    AudioRerouterPluginAudioProcessor& audioProcessor;

    AudioRerouterLookAndFeel laf;

    // ── AFC toggle (top-right corner of header bar) ───────────────────────────
    juce::ToggleButton afcToggle;
    juce::Label        afcLabel;
