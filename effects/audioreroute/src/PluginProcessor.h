...
public:
    ...
    // Effective threshold (limiterEnabled ? threshold : 1.0f) published each block
    // by the transmitter so receiver-side AFC can target the correct ceiling.
    static std::atomic<float>        channelTransmitterThreshold[8];

    // Current AFC-computed wet multiplier — written by the audio thread, read
    // by the editor timer to animate the wet knob while AFC is active.
    std::atomic<float> afcCurrentGain { 1.0f };
...
private:
    ...
    // Per-instance Automatic Feedback Control state (receive mode only).
    // afcWetGain is the multiplier AFC applies instead of the user's wet parameter.
    // Attack is fast (0→1 of range in ~0.5 s); release is slow with a
    // "fast-recovery" mode when the signal is already trending downward.
    float afcWetGain    = 1.0f;
    float afcAttackRate  = 0.0f; // per sample, set in prepareToPlay
    float afcReleaseRate = 0.0f; // per sample, set in prepareToPlay
    float afcPrevPeak    = 0.0f; // smoothed peak from previous block for trend detection
