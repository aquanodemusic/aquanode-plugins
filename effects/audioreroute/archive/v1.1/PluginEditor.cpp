    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    // ── PDC toggle (header, top-right) ───────────────────────────────────────
    {
        const bool pdcOn = *audioProcessor.apvts.getRawParameterValue("pdc") > 0.5f;
        pdcToggle.setToggleState(pdcOn, juce::dontSendNotification);
        pdcToggle.setColour(juce::ToggleButton::textColourId, kFg);
        pdcToggle.addListener(this);
        addAndMakeVisible(pdcToggle);

        pdcLabel.setText("PDC", juce::dontSendNotification);
        pdcLabel.setJustificationType(juce::Justification::centredRight);
        pdcLabel.setColour(juce::Label::textColourId, kLabel);
        pdcLabel.setFont(juce::Font(juce::FontOptions{}.withHeight(11.0f)));
...
void AudioRerouterPluginAudioProcessorEditor::resized()
{
    ...
    // ── PDC toggle: top-right corner of the header bar ────────────────────────
    // "PDC" text label + small checkbox, right-aligned within the header strip.
    // Header strip occupies y=0..34.  Centre vertically at y=8, h=20.
    {
        const int pdcCheckW = 22;   // just the tick box, no button text
        const int pdcLabelW = 32;
        const int pdcRight  = W - marginX;
        pdcToggle.setBounds(pdcRight - pdcCheckW, 7, pdcCheckW, 20);
        pdcLabel.setBounds(pdcRight - pdcCheckW - pdcLabelW - 14, 7, pdcLabelW, 20);
    }
...
void AudioRerouterPluginAudioProcessorEditor::buttonClicked(juce::Button* button)
{
    ...
    if (button == &pdcToggle)
    {
        const float newVal = pdcToggle.getToggleState() ? 1.0f : 0.0f;
        audioProcessor.apvts.getParameter("pdc")->setValueNotifyingHost(newVal);
        repaint(); // refresh the indicator dot in the header
        return;
    }
