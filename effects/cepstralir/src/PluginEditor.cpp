    ...
    : AudioProcessorEditor(&p), processorRef(p)
{
    ...
    setupSlider(hardCapSlider, hardCapLabel, "CAP (ms)", 10.0, 5000.0, 500.0, " ms");

    fftSizeSlider.setNumDecimalPlacesToDisplay(0);
    fftSizeSlider.onValueChange = [this] { triggerReprocess(); };
    
    hardCapSlider.setNumDecimalPlacesToDisplay(0);
    ...
    // Hard cap toggle
    hardCapToggle.setColour(juce::ToggleButton::textColourId, juce::Colour(0xff003355));
    hardCapToggle.setColour(juce::ToggleButton::tickColourId, juce::Colour(0xff00aa99));
    hardCapToggle.setColour(juce::ToggleButton::tickDisabledColourId, juce::Colour(0xffb0b0b0));
    hardCapToggle.setToggleState(false, juce::dontSendNotification);
    hardCapToggle.onStateChange = [this] 
    { 
        hardCapSlider.setEnabled(hardCapToggle.getToggleState());
        triggerReprocess(); 
    };
    addAndMakeVisible(hardCapToggle);
    
    // Disable hard cap slider initially
    hardCapSlider.setEnabled(false);
...
void CepstralIREditor::resized()
{
    ...
    sliderBounds(hardCapSlider, hardCapLabel);

    // Toggles - make them taller for better visibility
    auto toggleArea = controlArea.removeFromLeft(180).withTrimmedTop(20);
    windowToggle.setBounds(toggleArea.removeFromTop(26));
    linearPhaseToggle.setBounds(toggleArea.removeFromTop(26));
    hardCapToggle.setBounds(toggleArea.removeFromTop(26));
...
void CepstralIREditor::loadFile(const juce::File& f)
{
    ...
    *processorRef.useHardCapParam = hardCapToggle.getToggleState();
    *processorRef.hardCapLengthParam = (float)hardCapSlider.getValue();
