    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    makeKnob(feedbackSlider, "feedback");   // uniform colour, no special amber
...
void AquatonAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    if (knobs[5].slider->isVisible())
        g.drawHorizontalLine(knobs[5].slider->getY() - 3, 28.f, (float)(getWidth() - 28));
    if (knobs[10].slider->isVisible())
        g.drawHorizontalLine(knobs[10].slider->getY() - 3, 28.f, (float)(getWidth() - 28));
}
...
void AquatonAudioProcessorEditor::resized()
{
    ...
    // Three knob rows, with a small gap between each
    const int rowY1 = 82;
    const int rowY2 = rowY1 + knobH + 5;   // 187
    const int rowY3 = rowY2 + knobH + 5;   // 292
    const int btmY  = rowY3 + knobH + 8;   // 400
    ...
    //--- Save / Load buttons (header, top-right) ---
    saveBtn.setBounds(getWidth() - 235, 22, 105, 30);
    loadBtn.setBounds(getWidth() - 123, 22, 105, 30);
