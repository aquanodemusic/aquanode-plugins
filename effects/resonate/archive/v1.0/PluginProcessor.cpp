/*
  ==============================================================================
    ...
    - Mode A (simple comb), Mode B (all-pass diffusion), Mode C (square wave)
...
void ResonatorAudioProcessor::StateVariableFilter::setFrequency(float normalizedFreq, float resonance)
{
    freq = juce::jlimit(0.0f, 0.99f, normalizedFreq);
    q = juce::jlimit(0.1f, 10.0f, resonance);
}

float ResonatorAudioProcessor::StateVariableFilter::process(float input, int channel, FilterType type)
{
...
void ResonatorAudioProcessor::StateVariableFilter::reset()
{
    low[0] = low[1] = 0.0f;
    band[0] = band[1] = 0.0f;
}
...
void ResonatorAudioProcessor::Resonator::prepare(double sampleRate)
{
...
void ResonatorAudioProcessor::Resonator::updateParameters(double sampleRate, double globalDecay, 
                                                          ...
                                                          bool constMode)
{
    ...
    // For Mode B, set all-pass coefficient
    if (mode == ModeB)
    {
        allpassCoeff = 0.6f;
    }
...
float ResonatorAudioProcessor::Resonator::readDelayLinear(int channel, double delaySamples)
{
...
float ResonatorAudioProcessor::Resonator::processA(float input, int channel)
{
    if (!enabled || channel < 0 || channel > 1)
        return 0.0f;

    // Mode A: Classic comb filter with damping
    float delayedSample = readDelayLinear(channel, delayInSamples);
...
float ResonatorAudioProcessor::Resonator::processB(float input, int channel)
{
    if (!enabled || channel < 0 || channel > 1)
        return 0.0f;

    // Mode B: All-pass diffused comb filter
    // More complex, diffused resonance
    float delayedSample = readDelayLinear(channel, delayInSamples);
    
    // All-pass filter in feedback path
    float apInput = delayedSample;
    float apOutput = -allpassCoeff * apInput + allpassState[channel];
    allpassState[channel] = apInput + allpassCoeff * apOutput;
    
    // One-pole damping
    lpfState[channel] += lpfCoeff * (apOutput - lpfState[channel]);
    float dampedFeedback = lpfState[channel];
    
    // Comb with all-pass diffusion
    float feedbackSignal = static_cast<float>(feedback) * dampedFeedback;
    float output = input + feedbackSignal;
...
void ResonatorAudioProcessor::Resonator::reset()
{
    for (int ch = 0; ch < 2; ++ch)
    {
...
void ResonatorAudioProcessor::updateResonatorParameters()
{
