...
void WaveformDisplay::paint(juce::Graphics& g)
{
    ...
    // ---- Live circular-buffer window (FX mode) ---------------------------
    const double sampleRate = processor.getSampleRate();
    const float  timeWindow = processor.getValueTreeState().getRawParameterValue("timeWindow")->load();
    const juce::int64 writePos = processor.getCircWritePos();
    const juce::int64 windowSamples = juce::jmax((juce::int64)1,
        (juce::int64)(timeWindow * (sampleRate > 0.0 ? sampleRate : 44100.0)));
    ...
    if (availableLen < 64)
    {
        ...
        g.setColour(textCol);
        g.setFont(14.0f);
        g.drawText("Waiting for live audio input... feed audio into the plugin to begin granulating.",
            getLocalBounds(), juce::Justification::centred);
        return;
    }

    const juce::int64 windowStartAbs = writePos - availableLen;
    auto  bounds = getLocalBounds().reduced(4);
    const int displayWidth = bounds.getWidth();
    if (displayWidth <= 0)
        return;

    // Helper: mono-summed sample at an absolute circular-buffer index
    auto readMono = [this](juce::int64 abs) -> float
        {
            return 0.5f * (processor.getCircSample(0, abs) + processor.getCircSample(1, abs));
        };

    juce::Path waveformPath;

    // Top half — oldest audio on the left, newest (just-recorded) on the right
    for (int x = 0; x < displayWidth; ++x)
    {
        juce::int64 s0 = windowStartAbs + ((juce::int64)x * availableLen) / displayWidth;
        juce::int64 s1 = windowStartAbs + ((juce::int64)(x + 1) * availableLen) / displayWidth;
        s1 = juce::jmax(s1, s0 + 1);
        const juce::int64 step = juce::jmax((juce::int64)1, (s1 - s0) / 4);

        float maxVal = 0.0f;
        for (juce::int64 s = s0; s < s1; s += step)
            maxVal = juce::jmax(maxVal, readMono(s));
    ...
    for (int x = displayWidth - 1; x >= 0; --x)
    {
        ...
        float minVal = 0.0f;
        for (juce::int64 s = s0; s < s1; s += step)
            minVal = juce::jmin(minVal, readMono(s));
    ...
    // Window region overlay — position/spread now relative to the live window
    ...
    {
        const double normalizedPos = (grain.samplePosition - (double)windowStartAbs) / (double)availableLen;
...
// Click/drag on the live waveform scrubs the "Start Pos" (grainPosition)
// parameter directly, since there is no static loaded sample to click into.
void WaveformDisplay::mouseDown(const juce::MouseEvent& event)
{
    const float pos = juce::jlimit(0.0f, 1.0f, (float)event.x / (float)getWidth());
    if (auto* param = processor.getValueTreeState().getParameter("grainPosition"))
    ...
    : AudioProcessorEditor(&p), audioProcessor(p), waveformDisplay(p)
{
    ...
    // ---- Load button -------------------------------------------------------
    // Sample loading isn't used in live-input FX mode; the control is kept
    // (harmless) but hidden from the GUI.
    ...
    setupSlider(timeWindowSlider, timeWindowLabel, "Time Win", "timeWindow", timeWindowAttachment);
    ...
    // Note ADSR — not meaningful in live-input FX mode (there's no MIDI note
    // or mouse-press "note" to shape); parameters are retained but hidden.
    ...
    for (auto* s : { &noteAttackSlider, &noteDecaySlider, &noteSustainSlider, &noteReleaseSlider })
        s->setVisible(false);
    for (auto* l : { &noteAttackLabel, &noteDecayLabel, &noteSustainLabel, &noteReleaseLabel })
        l->setVisible(false);
    ...
    setupSlider(pitchSlider, pitchLabel, "Pitch", "pitch", pitchAttachment);
...
void GranulateAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    g.drawText("Granulate by aquanode", 15, 10, 195, 25,
        juce::Justification::centredLeft);
    // G hint — between plugin name and Sidebar button
    g.setFont(13.0f);
    g.drawText("G = Per Grain   |   Live granular FX", 215, 10, 260, 25,
        juce::Justification::centredLeft);
...
void GranulateAudioProcessorEditor::resized()
{
    ...
    // Note ADSR knobs are hidden in FX mode — the freed-up slot is reused for
    // the new live Time Window control; the remaining three stay empty.
    timeWindowSlider.setBounds(x, y + labelGap, knobSize, knobSize + 20); x += spacingX;
