...
public:
    ...
    // Routes a file through standard-format detection; shows the raw-import
    // dialog for any file that AudioFormatManager cannot open.
    void handleFileImport(const juce::File& file);

private:
    // Shows an AlertWindow asking the user for raw PCM settings, then calls
    // audioProcessor.loadSampleRaw() on confirmation.
    void showRawImportDialog(const juce::File& file);
    ...
    std::unique_ptr<juce::AudioProcessorValueTreeState::SliderAttachment> timeWindowAttachment;
