...
public:
    ...
    // Import any file as raw (headerless) PCM audio — mirrors Audacity's
    // "Import Raw Data" feature.  bitDepth must be 16 or 24 (little-endian
    // signed integers assumed); numChannels 1 or 2; sampleRate in Hz.
    void loadSampleRaw(const juce::File& file, int bitDepth,
    ...
    // The plugin continuously records its live audio input into a rolling
    // circular buffer. Grains are drawn from the most recent `timeWindow`
    // seconds of that buffer instead of from a loaded sample.
    juce::int64 getCircWritePos() const { return circWritePos; }
    float       getCircSample(int channel, juce::int64 absIndex) const;
...
private:
    ...
    // Raw-import bookkeeping — persisted in getStateInformation so the plugin
    // can reproduce the same loadSampleRaw() call when the DAW session is reopened.
    bool   lastLoadedIsRaw = false;
    int    lastRawBitDepth = 16;
    int    lastRawNumChannels = 1;
    double lastRawSampleRate = 44100.0;

    std::mt19937 rng;
    std::uniform_real_distribution<float> dist01{ 0.0f, 1.0f };

    // ---- Live circular buffer state (FX mode) ------------------------------
    juce::AudioBuffer<float> circularBuffer;      // fixed-capacity ring, 2 channels
    int                      circCapacitySamples = 0;
    juce::int64              circWritePos = 0;    // monotonically increasing sample count written

    std::array<GrainVoice, MAX_GRAINS_PER_NOTE> liveGrains{};
    float liveGrainTimer = 0.0f;

    void pushLiveAudio(const juce::AudioBuffer<float>& input, int numSamples);
    void triggerLiveGrain(GrainVoice& grain, float basePosition01,
        juce::int64 windowStartAbs, juce::int64 availableLen, bool reverse);
