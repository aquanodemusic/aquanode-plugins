                                                  ...
                                                  juce::MidiBuffer& midiMessages)
{
    ...
    // engine walks the buffer below so tapped notes are seen this block.
...
void AquanodeModularAudioProcessor::initializePatch()
{
    ...
    // Effect build: a blank slate is the host's own signal passed straight
    // through, so the patch starts where an effect patch wants to start.
    const int src = addModule ("io.audioIn",  { 60, 60 });
   #else
    // Instrument build: a blank slate makes a sound as soon as you play.
    const int src = addModule ("osc.basic",   { 60, 60 });
   #endif

    const int out = addModule ("io.audioOut", { 460, 60 });

   #if AQUANODE_FX_BUILD
    // Nothing to compensate in an effect build: the host's audio arrives
    // already summed and there are no voices to count, so a Polyphony Sum here
    // would sit in the global lane passing everything through at unity. Wire
    // the input straight to the output instead.
    addCable (src, "audioOut", out, "audioIn");
   #else
    // Polyphony Sum sits between the source and the output as standard. Voices
    // are summed with no per-voice scaling, so without it a six-note chord
    // arrives at roughly six times the level of a single note and clips - which
    // is worst on Android, where the device output stage distorts on top of it.
    // Amount defaults to 1/sqrt(N), the correct law for uncorrelated voices.
    // Delete the module if you want the raw, hot sum back.
    const int psum = addModule ("util.polysum", { 260, 60 });
    addCable (src,  "audioOut", psum, "audioIn");
    addCable (psum, "audioOut", out,  "audioIn");
   #endif
    ...
    const std::function<std::shared_ptr<juce::AudioBuffer<float>> (const juce::String&, double&)>& sampleLoader)
{
    ...
    if (auto* modulesEl = xml.getChildByName ("Modules"))
    {
        for (auto* m : modulesEl->getChildWithTagNameIterator ("Module"))
        {
            ...
            inst->collapsed = m->getBoolAttribute ("collapsed", false);
...
//==============================================================================
bool AquanodeModularAudioProcessor::exportPatchToStream (juce::OutputStream& out)
{
...
bool AquanodeModularAudioProcessor::exportPatchToZip (const juce::File& zipFile)
{
    ...
    return exportPatchToStream (out);
}

bool AquanodeModularAudioProcessor::importFromZipObject (juce::ZipFile& zip)
{
...
bool AquanodeModularAudioProcessor::importPatchFromStream (juce::InputStream& in)
{
    // ZipFile needs random access, so buffer the picked document into memory.
    juce::MemoryBlock mb;
    in.readIntoMemoryBlock (mb);

    juce::ZipFile zip (new juce::MemoryInputStream (mb, true), true);   // owns + frees the stream
    return importFromZipObject (zip);
}
