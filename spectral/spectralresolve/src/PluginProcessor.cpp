...
SpectralResolverProcessor::createParams()
{
    ...
    p.push_back(std::make_unique<juce::AudioParameterBool>(
        "HF_WEIGHT_ON", "HF Weight On", true));
    p.push_back(std::make_unique<juce::AudioParameterFloat>(
        "HF_WEIGHT_CUTOFF", "HF Weight Cutoff",
        juce::NormalisableRange<float>(50.f, 5000.f, 1.f, 0.35f), 500.f));
...
void SpectralResolverProcessor::rebuildFFT(int order, int hopDenom, int windowChoice)
{
    ...
    for (int n = 0; n < N; ++n)
    {
        ...
        default:
        {
            const float x = 2.f * float(n) / float(N - 1) - 1.f;
