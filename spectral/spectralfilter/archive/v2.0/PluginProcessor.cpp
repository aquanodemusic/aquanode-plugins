    ...
    juce::MidiBuffer& midiMessages)
{
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
    {
        ...
        if (inputFifoIndex[0] >= fftSize)
        {
            ...
            // 6. Pan (mid-side tilt, pan=0 is identity pass-through)
            {
                ...
                if (totalIn >= 2 && pStr != 0.f)
                {
                    for (int bin = 0; bin < numBins; ++bin)
                    {
                        ...
                        // Tilt the side channel toward the requested pan direction.
                        // pan > 0 shifts energy right, pan < 0 shifts energy left.
                        float tiltRe = sideRe * pan;
                        float tiltIm = sideIm * pan;

                        fftBuffer[0][bin * 2] = midRe - tiltRe;
                        fftBuffer[0][bin * 2 + 1] = midIm - tiltIm;
                        fftBuffer[1][bin * 2] = midRe + tiltRe;
                        fftBuffer[1][bin * 2 + 1] = midIm + tiltIm;
                    }
                }
            }
