...
public:
    ...
    void setMaskColor(juce::Colour c);
    ...
    juce::Colour getMaskColor()        const { return maskColor; }
    void resetColors();
    ...
    // Normalised 0–1 frequency range applied to all processing (0 = bin 0, 1 = bin numBins-1)
    std::atomic<float>* pFreqRangeStart{ nullptr };
    std::atomic<float>* pFreqRangeEnd  { nullptr };
    ...
    // Raw pointers for lock-free audio-thread reads of speed params (set after construction)
    std::atomic<float>* pSpeedFilter{ nullptr };
    std::atomic<float>* pSpeedPhase { nullptr };
    std::atomic<float>* pSpeedFreq  { nullptr };
    std::atomic<float>* pSpeedPan   { nullptr };
    std::atomic<float>* pSpeedBin   { nullptr };
...
private:
    ...
    void stageFreqShiftAndPan(int numActiveChannels, int rangeLo, int rangeHi);
    ...
    juce::Colour maskColor{ 0x7700bb00 };
