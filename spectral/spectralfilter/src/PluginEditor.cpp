    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    makeColorRow(maskColorLabel, "Mask:", maskColorInput, audioProcessor.getMaskColor().toDisplayString(false), [this] { updateMaskColor(); });
    ...
    resetColorsButton.onClick = [this]()
        {
            ...
            maskColorInput.setText(audioProcessor.getMaskColor().toDisplayString(false), juce::dontSendNotification);
            repaint();
        };
    ...
    auto makeShiftStrip = [&](ShiftStrip& s, const juce::String& labelText,
        juce::Colour onColour, juce::Colour textOnColour, juce::Colour knobAccent,
        ...
        std::function<void(int)> valueChangeFn)
        {
            ...
            // Speed knob — symmetric -1000..+1000 bins/sec, centre = 0 = stopped
            s.speedLF = std::make_unique<StrengthKnobLookAndFeel>(knobAccent);
            s.speedKnob.setLookAndFeel(s.speedLF.get());
            s.speedKnob.setSliderStyle(juce::Slider::Rotary);
            s.speedKnob.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 52, 14);
            s.speedKnob.setRange(-1000.0, 1000.0, 0.1);
            s.speedKnob.setDoubleClickReturnValue(true, 10.0);
            s.speedKnob.setTooltip("Auto-shift speed (bins/sec). Centre = stopped. Negative = reverse.");
            s.speedKnob.textFromValueFunction = [](double v) -> juce::String {
                return juce::String(v, 1);
                };
            s.speedKnob.valueFromTextFunction = [](const juce::String& t) -> double {
                return t.getDoubleValue();
                };
            addAndMakeVisible(s.speedKnob);
        };

    // ---- Strip 1: Filter Shift ----
    makeShiftStrip(filterShift, "Filter Shift:",
        juce::Colour(0xff225566), juce::Colour(0xff55eedd), juce::Colour(0xff55eedd),
        [this]()
        {
    ...
    makeShiftStrip(phaseShift, "Phase Shift:",
        juce::Colour(0xff552266), juce::Colour(0xffdd55bb), juce::Colour(0xffdd55bb),
        [this]()
        {
    ...
    makeShiftStrip(freqShift, "Freq Shift:",
        juce::Colour(0xff115566), juce::Colour(0xff44ccff), juce::Colour(0xff44ccff),
        [this]()
        {
    ...
    makeShiftStrip(panShift, "Pan Shift:",
        juce::Colour(0xff554400), juce::Colour(0xffffcc33), juce::Colour(0xffffcc33),
        [this]()
        {
    ...
    makeShiftStrip(globalFreqShift, "Global Shift:",
        juce::Colour(0xff225566), juce::Colour(0xff55eedd), juce::Colour(0xff88bbff),
        [this]()
        {
    ...
    // ---- Wire speed knobs to APVTS ----
    filterSpeedAttachment = std::make_unique<SliderAttachment>(
        audioProcessor.apvts, "speedFilter", filterShift.speedKnob);
    phaseSpeedAttachment = std::make_unique<SliderAttachment>(
        audioProcessor.apvts, "speedPhase", phaseShift.speedKnob);
    freqSpeedAttachment = std::make_unique<SliderAttachment>(
        audioProcessor.apvts, "speedFreq", freqShift.speedKnob);
    panSpeedAttachment = std::make_unique<SliderAttachment>(
        audioProcessor.apvts, "speedPan", panShift.speedKnob);
    binSpeedAttachment = std::make_unique<SliderAttachment>(
        audioProcessor.apvts, "speedBin", globalFreqShift.speedKnob);

    // ---- Freq Range knobs ----
    // Skew factor 0.35 gives the knob's physical midpoint at ~6% of Nyquist (~1.3kHz at 44.1k),
    // providing fine resolution in the low-frequency bins where it matters most.
    auto makeRangeKnob = [&](juce::Slider& knob, std::unique_ptr<juce::LookAndFeel_V4>& lf,
        juce::Label& lbl, const juce::String& labelText, const juce::String& tooltip)
        {
            lf = std::make_unique<StrengthKnobLookAndFeel>(juce::Colour(0xffaaaaff));
            knob.setLookAndFeel(lf.get());
            knob.setSliderStyle(juce::Slider::Rotary);
            knob.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 52, 14);
            knob.setRange(0.0, 1.0, 0.0001);
            knob.setSkewFactor(0.35);   // logarithmic feel — midpoint ~6% of Nyquist
            knob.setDoubleClickReturnValue(true, (labelText == "Start" ? 0.0 : 1.0));
            knob.setTooltip(tooltip);
            knob.textFromValueFunction = [this](double v) -> juce::String {
                float hz = static_cast<float>(v) * static_cast<float>(audioProcessor.currentSampleRate * 0.5);
                if (hz < 1000.f) return juce::String(juce::roundToInt(hz)) + "Hz";
                return juce::String(hz / 1000.f, 2) + "kHz";
                };
            knob.valueFromTextFunction = [this](const juce::String& s) -> double {
                float hz = s.upToFirstOccurrenceOf(" ", false, false).getFloatValue();
                if (s.containsIgnoreCase("k")) hz *= 1000.f;
                float nyq = static_cast<float>(audioProcessor.currentSampleRate * 0.5);
                return juce::jlimit(0.0, 1.0, static_cast<double>(hz / nyq));
                };
            styleLabel(lbl);
    ...
    makeRangeKnob(freqStartKnob, freqStartKnobLF, freqStartLabel, "Start",
        "Active range start (0% = bin 0). Processing only affects bins inside Start to End.");
    makeRangeKnob(freqEndKnob, freqEndKnobLF, freqEndLabel, "End",
        "Active range end (100% = Nyquist). Processing only affects bins inside Start to End.");

    freqStartAttachment = std::make_unique<SliderAttachment>(
        audioProcessor.apvts, "freqRangeStart", freqStartKnob);
    freqEndAttachment = std::make_unique<SliderAttachment>(
        audioProcessor.apvts, "freqRangeEnd", freqEndKnob);
...
SpectralFilterAudioProcessorEditor::~SpectralFilterAudioProcessorEditor()
{
    ...
    filterSpeedAttachment.reset();
    phaseSpeedAttachment.reset();
    freqSpeedAttachment.reset();
    panSpeedAttachment.reset();
    binSpeedAttachment.reset();

    freqStartAttachment.reset();
    freqEndAttachment.reset();
    ...
    filterShift.speedKnob.setLookAndFeel(nullptr);
    phaseShift.speedKnob.setLookAndFeel(nullptr);
    freqShift.speedKnob.setLookAndFeel(nullptr);
    panShift.speedKnob.setLookAndFeel(nullptr);
    globalFreqShift.speedKnob.setLookAndFeel(nullptr);

    freqStartKnob.setLookAndFeel(nullptr);
    freqEndKnob.setLookAndFeel(nullptr);
...
void SpectralFilterAudioProcessorEditor::timerCallback()
{
    ...
    // Zero out display values outside the active frequency range so the
    // visualisation matches what is actually being processed.
    {
        auto [lo, hi] = getActiveBinRange();
        for (int i = 0; i < audioProcessor.numBins; ++i)
        {
            if (i < lo || i > hi)
            {
                displayCurveDB[i] = 0.0f;
                displayPhaseRad[i] = 0.0f;
                displayFreqShift[i] = 0.0f;
                displayPan[i] = 0.0f;
                fftDisplayData[i] = 0.0f;
            }
        }
    }
...
std::pair<int, int> SpectralFilterAudioProcessorEditor::getActiveBinRange() const
{
    const float ns = static_cast<float>(freqStartKnob.getValue());
    const float ne = static_cast<float>(freqEndKnob.getValue());
    const float lo = std::min(ns, ne), hi = std::max(ns, ne);
    const int nb = audioProcessor.numBins - 1;
    return { juce::jlimit(0, nb, static_cast<int>(lo * nb)),
             juce::jlimit(0, nb, static_cast<int>(hi * nb)) };
}
...
void SpectralFilterAudioProcessorEditor::drawFreqRangeMask(juce::Graphics& g)
{
    auto [lo, hi] = getActiveBinRange();
    const float xLo = binToX(lo);
    const float xHi = binToX(hi);
    const float h = static_cast<float>(getHeight());
    const float w = static_cast<float>(getWidth());

    // Solid fill using the user-chosen mask colour
    const juce::Colour mask = audioProcessor.getMaskColor();
    g.setColour(mask);
    if (xLo > 0.f)
        g.fillRect(0.f, 0.f, xLo, h);
    if (xHi < w)
        g.fillRect(xHi, 0.f, w - xHi, h);

    // Boundary lines — slightly brighter than the mask colour
    g.setColour(mask.brighter(0.4f));
    g.drawVerticalLine(static_cast<int>(xLo), 0.f, h);
    g.drawVerticalLine(static_cast<int>(xHi), 0.f, h);

    // Frequency labels at the boundary lines
    const float ny = nyquist();
    const float fpb = ny / static_cast<float>(audioProcessor.numBins - 1);
    auto freqLabel = [](float freq) -> juce::String {
        if (freq < 1000.f) return juce::String(static_cast<int>(freq)) + "Hz";
        return juce::String(freq / 1000.f, 1) + "k";
        };
    g.setFont(10.f);
    // Label colour: use contrasting brightness relative to mask
    g.setColour(mask.getPerceivedBrightness() > 0.5f ? juce::Colours::black : juce::Colours::white);
    float loFreq = static_cast<float>(lo) * fpb;
    float hiFreq = static_cast<float>(hi) * fpb;
    g.drawText(freqLabel(loFreq), static_cast<int>(xLo) + 2, 4, 48, 12, juce::Justification::left);
    g.drawText(freqLabel(hiFreq), static_cast<int>(xHi) - 50, 4, 48, 12, juce::Justification::right);
}
...
void SpectralFilterAudioProcessorEditor::resized()
{
    ...
    // Row 18: Mask Color
    maskColorInput.setBounds(rightEdge - inputW, startY + rowH * 18, inputW, 22);
    maskColorLabel.setBounds(rightEdge - inputW - labelW, startY + rowH * 18, labelW, 22);

    // Row 19: Reset Colors
    resetColorsButton.setBounds(rightEdge - inputW, startY + rowH * 19, inputW, 22);
    ...
    const int knobW = 44;   // square speed knob (fits within strip height with a little overhang)
    const int lbW = 84;
    ...
    globalFreqShift.speedKnob.setBounds(w - knobW - gap, s5y - 4, knobW, sliderH + 8);
    globalFreqShift.autoBtn.setBounds(w - knobW - autoBtnW - gap * 2, s5y, autoBtnW, sliderH);
    globalFreqWrapButton.setBounds(w - knobW - autoBtnW - wrapBtnW - gap * 3, s5y, wrapBtnW, sliderH);
    globalFreqShift.slider.setBounds(lbW + gap, s5y,
        w - lbW - wrapBtnW - autoBtnW - knobW - gap * 5, sliderH);

    // Strip 4: Pan Shift
    const int s4y = s5y - rowSz;
    panShift.label.setBounds(gap, s4y, lbW, sliderH);
    panShift.speedKnob.setBounds(w - knobW - gap, s4y - 4, knobW, sliderH + 8);
    panShift.autoBtn.setBounds(w - knobW - autoBtnW - gap * 2, s4y, autoBtnW, sliderH);
    panShift.slider.setBounds(lbW + gap, s4y, w - lbW - autoBtnW - knobW - gap * 4, sliderH);

    // Strip 3: Freq Shift (per-bin)
    const int s3y = s4y - rowSz;
    freqShift.label.setBounds(gap, s3y, lbW, sliderH);
    freqShift.speedKnob.setBounds(w - knobW - gap, s3y - 4, knobW, sliderH + 8);
    freqShift.autoBtn.setBounds(w - knobW - autoBtnW - gap * 2, s3y, autoBtnW, sliderH);
    freqShift.slider.setBounds(lbW + gap, s3y, w - lbW - autoBtnW - knobW - gap * 4, sliderH);

    // Strip 2: Phase Shift
    const int s2y = s3y - rowSz;
    phaseShift.label.setBounds(gap, s2y, lbW, sliderH);
    phaseShift.speedKnob.setBounds(w - knobW - gap, s2y - 4, knobW, sliderH + 8);
    phaseShift.autoBtn.setBounds(w - knobW - autoBtnW - gap * 2, s2y, autoBtnW, sliderH);
    phaseShift.slider.setBounds(lbW + gap, s2y, w - lbW - autoBtnW - knobW - gap * 4, sliderH);

    // Strip 1 (top-most): Filter Shift
    const int s1y = s2y - rowSz;
    filterShift.label.setBounds(gap, s1y, lbW, sliderH);
    filterShift.speedKnob.setBounds(w - knobW - gap, s1y - 4, knobW, sliderH + 8);
    filterShift.autoBtn.setBounds(w - knobW - autoBtnW - gap * 2, s1y, autoBtnW, sliderH);
    filterShift.slider.setBounds(lbW + gap, s1y, w - lbW - autoBtnW - knobW - gap * 4, sliderH);

    // ---- Freq Range knobs — bottom-left, above filter shift strip ----
    const int rangeKnobSize = 52;
    const int rangeKnobY = s1y - rangeKnobSize - gap * 2;
    const int rangeLabelH = 14;
    freqStartKnob.setBounds(gap, rangeKnobY, rangeKnobSize, rangeKnobSize);
    freqStartLabel.setBounds(gap, rangeKnobY + rangeKnobSize, rangeKnobSize, rangeLabelH);
    freqEndKnob.setBounds(gap + rangeKnobSize + gap, rangeKnobY, rangeKnobSize, rangeKnobSize);
    freqEndLabel.setBounds(gap + rangeKnobSize + gap, rangeKnobY + rangeKnobSize, rangeKnobSize, rangeLabelH);
}
...
void SpectralFilterAudioProcessorEditor::updateMaskColor()
{
    try { audioProcessor.setMaskColor(parseHex(maskColorInput.getText())); repaint(); }
    catch (...) { maskColorInput.setText(audioProcessor.getMaskColor().toDisplayString(false)); }
}