...
static juce::Font spectralLockFont(float height, bool bold = false)
{
...
juce::Colour SpectralLockLookAndFeel::spectralLockColour(float t)
{
...
juce::Font SpectralLockLookAndFeel::getLabelFont(juce::Label&) { return spectralLockFont(15.0f); }
    ...
    juce::Slider& s)
{
    ...
    g.setColour(spectralLockColour(sliderPos));
    ...
    juce::Slider::SliderStyle style, juce::Slider& s)
{
    ...
    juce::ColourGradient grad(spectralLockColour(0.15f), minPos, cy, spectralLockColour(0.9f), maxPos, cy, false);
...
void SpectralLockKnob::paint(juce::Graphics& g)
{
    ...
    g.setFont(spectralLockFont(13.0f, true));
...
void MatrixComponent::paint(juce::Graphics& g)
{
    ...
    for (int c = 0; c < 12; ++c)
    {
        for (int r = 0; r < 12; ++r)
        {
            ...
            if (isActive)
            {
                g.setColour(LNF::spectralLockColour((float)r / 11.0f));
        ...
        g.setColour(LNF::dim);
        g.setFont(spectralLockFont(11.5f));
...
void RangeSelector::paint(juce::Graphics& g)
{
    ...
    g.setFont(spectralLockFont(14.5f, true));
...
void BandView::paint(juce::Graphics& g)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        g.setColour(LNF::spectralLockColour((float)i / (float)(n - 1)).withAlpha(0.25f + 0.75f * v));
        g.fillRect(bar);
    }

    g.setColour(LNF::dim.withAlpha(0.6f));
    g.setFont(spectralLockFont(11.0f, true));
...
void SpectralLockAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    for (int i = 0; i < 95; ++i)
    {
        g.setColour(LNF::spectralLockColour((float)i / 94.0f));
        g.fillRect(rule.getX() + (float)i * 2.0f, rule.getY(), 2.0f, 2.5f);
    }

    g.setColour(LNF::dim.withAlpha(0.7f));
    g.setFont(spectralLockFont(13.5f));
