    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    // Add items in the APVTS parameter order: index 0 = 32, index 1 = 128, etc.
    // Item IDs must be index + 1 (JUCE requirement: IDs start at 1).
    fftSizeCombo.addItem ("32",   1);
    fftSizeCombo.addItem ("128",  2);
    fftSizeCombo.addItem ("256",  3);
    fftSizeCombo.addItem ("512",  4);
    fftSizeCombo.addItem ("1024", 5);
    fftSizeCombo.addItem ("2048", 6);
    fftSizeCombo.addItem ("4096", 7);
    fftSizeCombo.addItem ("8192", 8);

    // Create APVTS attachment — automatically syncs with parameter
    fftSizeAttachment = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment> (
        audioProcessor.apvts, "fftSizeIndex", fftSizeCombo);
    ...
    randomizeButton.onClick = [this]
        {
            ...
            // No manual slider update needed — APVTS attachment handles it automatically
        };
    ...
    // Create APVTS attachment — automatically syncs with parameter and triggers repaint via listener
    maxLatencyAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment> (
        audioProcessor.apvts, "maxLatency", maxLatencySlider);
