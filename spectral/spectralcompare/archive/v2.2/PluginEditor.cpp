...
void SpectralCompareAudioProcessorEditor::mouseDown(const juce::MouseEvent& event)
{
    ...
    switch (drawTarget)
    {
        case 0: { const float dB = juce::jlimit(-kFilterRange, kFilterRange, filterYToDB(filterLastDragY));
                  audioProcessor.setMainFilterCurveRange(bin, bin, dB, dB); break; }
        case 1: { const float dB = juce::jlimit(-kFilterRange, kFilterRange, filterYToDB(filterLastDragY));
                  audioProcessor.setSidechainFilterCurveRange(bin, bin, dB, dB); break; }
        case 2: { const float dB = spectrumYToDB(filterLastDragY);
                  audioProcessor.setGateFilterCurveRange(bin, bin, dB, dB); break; }
        case 3: { const float dB = spectrumYToDB(filterLastDragY);
...
void SpectralCompareAudioProcessorEditor::mouseDrag(const juce::MouseEvent& event)
{
    ...
    switch (drawTarget)
    {
        case 0: { const float s = juce::jlimit(-kFilterRange, kFilterRange, filterYToDB(filterLastDragY));
        ...
        case 1: { const float s = juce::jlimit(-kFilterRange, kFilterRange, filterYToDB(filterLastDragY));
                  const float e = juce::jlimit(-kFilterRange, kFilterRange, filterYToDB(y));
                  audioProcessor.setSidechainFilterCurveRange(startBin, endBin, s, e); break; }
        case 2: { const float s = spectrumYToDB(filterLastDragY);
                  const float e = spectrumYToDB(y);
                  audioProcessor.setGateFilterCurveRange(startBin, endBin, s, e); break; }
        case 3: { const float s = spectrumYToDB(filterLastDragY);
                  const float e = spectrumYToDB(y);
