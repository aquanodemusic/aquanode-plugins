...
public:
    ...
    // Smoothing coefficients — set from editor slider (0=instant, 0.99=very slow)
    // Float stores/loads are naturally atomic on x86/ARM, so no lock needed.
    std::atomic<float> smoothMain     { 0.7f };
    std::atomic<float> smoothSidechain{ 0.7f };
    ...
    void   setDiffColor      (juce::Colour c);
    ...
    juce::Colour getDiffColor()             const { return diffColor; }
...
private:
    ...
    juce::Colour diffColor            { 0xff00ccff };
