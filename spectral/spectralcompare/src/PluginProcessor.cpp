...
SpectralCompareAudioProcessor::createParameterLayout()
{
    ...
    // Spectral reassignment visual mode
    params.push_back(std::make_unique<juce::AudioParameterBool>("reassign", "Reassign", false));
    ...
    apvts(*this, nullptr, "SpectralCompare", createParameterLayout())
{
    ...
    reassignParam = apvts.getRawParameterValue("reassign");
...
void SpectralCompareAudioProcessor::allocateBuffers()
{
    ...
    // Spectral-reassignment scratch buffers + BH window (size matches fftSize)
    raWorkH.assign(fftSize * 2, 0.0f);
    raWorkDH.assign(fftSize * 2, 0.0f);
...
// ============================================================================
// Spectral reassignment helpers (display-only visual path)
// ============================================================================

// Blackman-Harris 4-term window (Harris 1978, ~92 dB sidelobes) and its exact
// analytical time-derivative.  Both are used only for the reassignment display
// path; the audio path keeps its existing Hann window.
void SpectralCompareAudioProcessor::buildReassignWindows()
{
    const int N = fftSize;
    ...
    for (int n = 0; n < N; ++n)
    {
        const double ph = twoPi * n / double(N);
        raWinH [n] = float(a0
                         - a1 * std::cos(ph)
                         + a2 * std::cos(2.0 * ph)
                         - a3 * std::cos(3.0 * ph));
        raWinDH[n] = float((twoPi / double(N)) * (
                          a1 * std::sin(ph)
...
// Frequency-only spectral reassignment.
//
//   ω̂_k  =  ω_k  −  Im(cDH_k / cH_k) * fs / (2π)
//
// `data`  — fftSize raw time-domain samples (unwindowed)
// `normF` — 2.0f / fftSize, converts raw FFT magnitude to display scale
// `frame` — output; cleared and filled with (freqHz, magLin) pairs that pass
//            the adaptive RMS threshold and the ±2-bin shift guard.
void SpectralCompareAudioProcessor::runFreqReassignment(
    const float*              data,
    int                       N,
    const std::vector<float>& winH,
    const std::vector<float>& winDH,
    std::vector<float>&       workH,
    std::vector<float>&       workDH,
    juce::dsp::FFT&           fft,
    float                     fs,
    float                     normF,
    ReassignFrame&            frame)
{
    // Apply windows and zero the upper half (in-place complex scratch buffers)
    std::fill(workH .begin(), workH .end(), 0.0f);
    std::fill(workDH.begin(), workDH.end(), 0.0f);
    for (int n = 0; n < N; ++n)
    {
        workH [n] = data[n] * winH [n];
        workDH[n] = data[n] * winDH[n];
    }

    fft.performRealOnlyForwardTransform(workH .data(), false);
    fft.performRealOnlyForwardTransform(workDH.data(), false);

    auto* cH  = reinterpret_cast<std::complex<float>*>(workH .data());
    auto* cDH = reinterpret_cast<std::complex<float>*>(workDH.data());

    const float binW          = fs / float(N);
    const int   nBins         = N / 2;
    const float maxReassignHz = 2.0f * binW;   // ±2 bins maximum shift

    // Adaptive RMS threshold: retain bins within 50 dB of the frame RMS.
    float sumSq = 0.0f;
    for (int k = 1; k < nBins; ++k)
        sumSq += std::norm(cH[k]);
    const float rmsDB   = 10.0f * std::log10(sumSq / float(nBins) + 1e-12f);
    const float threshDB = juce::jmax(-90.0f, rmsDB - 50.0f);

    frame.clear();
    frame.reserve(size_t(nBins / 4));

    for (int k = 1; k < nBins - 1; ++k)
    {
        const float mag    = std::abs(cH[k]);
        const float magDB  = 20.0f * std::log10(mag + 1e-9f);
        if (magDB < threshDB) continue;

        const auto  denom  = cH[k] + std::complex<float>(1e-9f, 0.0f);
        ...
        frame.magLin.push_back(mag * normF);   // normalised to display scale
    }
}
    ...
    juce::MidiBuffer&)
{
    ...
    for (int sample = 0; sample < numSamples; ++sample)
    {
        ...
        if (mainFifoIndex >= fftSize)
        {
            ...
            // D3b. Spectral-reassignment display path (visual only).
            // Runs on the unwindowed mainFifo samples so it can apply its own
            // BH window.  Only when the Reassign toggle is active.
            if (reassignParam->load(std::memory_order_relaxed) >= 0.5f)
            {
                ReassignFrame raFrame;
                runFreqReassignment(mainFifo.data(), fftSize,
                    raWinH, raWinDH, raWorkH, raWorkDH,
                    *fftMain,
                    float(currentSampleRate), 2.0f / float(fftSize),
                    raFrame);
                juce::ScopedLock l(reassignLock);
                reassignMainLatest = std::move(raFrame);
                reassignMainReady  = true;
            }
        ...
        if (sidechainFifoIndex >= fftSize)
        {
            ...
            // E2. Spectral-reassignment display path for sidechain (visual only).
            if (reassignParam->load(std::memory_order_relaxed) >= 0.5f)
            {
                ReassignFrame raFrame;
                runFreqReassignment(sidechainFifo.data(), fftSize,
                    raWinH, raWinDH, raWorkH, raWorkDH,
                    *fftMain,    // same FFT size, stateless call — safe
                    float(currentSampleRate), 2.0f / float(fftSize),
                    raFrame);
                juce::ScopedLock l(reassignLock);
                reassignSideLatest = std::move(raFrame);
                reassignSideReady  = true;
            }
...
bool SpectralCompareAudioProcessor::popReassignMainFrame(ReassignFrame& out)
{
    juce::ScopedLock l(reassignLock);
    if (!reassignMainReady) return false;
    out = std::move(reassignMainLatest);
    reassignMainReady = false;
    return true;
}

bool SpectralCompareAudioProcessor::popReassignSideFrame(ReassignFrame& out)
{
    juce::ScopedLock l(reassignLock);
    if (!reassignSideReady) return false;
    out = std::move(reassignSideLatest);
    reassignSideReady = false;
    return true;
}
