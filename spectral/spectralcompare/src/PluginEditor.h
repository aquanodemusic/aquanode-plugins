...
private:
    ...
    float hzToX(float hz) const;          // Hz → canvas X (log scale, respects viewFreqMin/Max)
    void  drawFilterCurves(juce::Graphics&);
    void  drawHeatmap(juce::Graphics&);
    void  updateHeatmap();

    // -----------------------------------------------------------------------
    // Spectral-reassignment display state
    // -----------------------------------------------------------------------
    ReassignFrame reassignMainFrame, reassignSideFrame;
    bool hasReassignMain = false, hasReassignSide = false;

    // Scatter-plot periodogram for one signal's reassigned frame.
    // Each event is drawn as a thin vertical bar at its exact reassigned Hz.
    void drawReassignedSpectrum(juce::Graphics& g,
        const ReassignFrame& frame,
        juce::Colour colour, float alpha);

    // Heatmap column painting using reassigned frames (replaces updateHeatmap
    // when Reassign is active).
    void updateHeatmapReassign();
    ...
    juce::TextButton reassignButton;       // Spectral-reassignment visual toggle
    ...
    std::unique_ptr<ButtonAttachment>   reassignAttachment;
