...
// ---------------------------------------------------------------------------
// Spectral reassignment output frame (display-only, produced on audio thread,
// consumed on message thread).  Holds reassigned frequency (Hz) and the
// corresponding normalised linear magnitude (same scale as the IIR display
// buffers: 2 / fftSize).
// ---------------------------------------------------------------------------
struct ReassignFrame
{
    std::vector<float> freqHz;
    std::vector<float> magLin;
    void clear()           { freqHz.clear();   magLin.clear(); }
    void reserve(size_t n) { freqHz.reserve(n); magLin.reserve(n); }
};
...
public:
    ...
    bool  isReassignActive() const { return reassignParam && reassignParam->load(std::memory_order_relaxed) >= 0.5f; }

    // Spectral-reassignment display frames — call from message thread only.
    // Returns true and moves the latest frame into `out` when a new one is ready.
    bool popReassignMainFrame(ReassignFrame& out);
    bool popReassignSideFrame(ReassignFrame& out);
...
private:
    ...
    // -----------------------------------------------------------------------
    // Spectral-reassignment visual path (display only — audio output unchanged)
    // -----------------------------------------------------------------------
    std::atomic<float>* reassignParam = nullptr;

    // Blackman-Harris window + its analytical time-derivative for reassignment
    std::vector<float> raWinH, raWinDH;
    // Scratch FFT work buffers (size 2*fftSize each)
    std::vector<float> raWorkH, raWorkDH;

    // Latest reassigned frame handed off to the editor under reassignLock.
    // The audio thread writes; the message thread reads and takes ownership.
    ReassignFrame      reassignMainLatest, reassignSideLatest;
    bool               reassignMainReady = false, reassignSideReady = false;
    juce::CriticalSection reassignLock;

    void buildReassignWindows();
    static void runFreqReassignment(const float* data, int N,
        const std::vector<float>& winH,
        const std::vector<float>& winDH,
        std::vector<float>& workH,
        std::vector<float>& workDH,
        juce::dsp::FFT& fft,
        float fs, float normF,
        ReassignFrame& frame);
