...
public:
    PianoRollComponent()
    {
        ...
        cellHeight = 15; // Reduced to fit 5 tracks layout
        ...
        // Initialize 5 automation lanes (unassigned)
        automationLanes[0] = AutomationLane();
        automationLanes[1] = AutomationLane();
        automationLanes[2] = AutomationLane();
        automationLanes[3] = AutomationLane();
        automationLanes[4] = AutomationLane();
    ...
    void resized() override
    {
        ...
        int automationTotalHeight = (5 * automationLaneHeight) + (4 * automationSpacing) + 40; // 40 for top margin
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        if (selectedAutomationLane >= 0 && selectedAutomationLane < 5)
        {
    ...
    void mouseDrag(const juce::MouseEvent& e) override
    {
        ...
        if (isEditingAutomation && selectedAutomationLane >= 0 && selectedAutomationLane < 5)
        {
            int automationLaneHeight = 80;
            int automationStartY = numNotes * cellHeight + 20;
            int automationSpacing = 10;
    ...
    {
        if (track < 0 || track >= 5 || midiNote < 0 || midiNote >= 128)
            return false;
...
private:
    ...
    public:
        ...
        void paint(juce::Graphics& g) override
        {
            ...
            juce::Colour automationColors[5] = { juce::Colours::orange, juce::Colours::cyan, juce::Colours::magenta,
                                                   juce::Colours::lime, juce::Colours::hotpink };
    ...
    std::vector<Note> trackNotes[5];  // Notes for each track
    AutomationLane automationLanes[5];  // 5 assignable automation lanes
    int currentTrack = 0;
    int selectedAutomationLane = -1;  // -1 = none, 0-4 = lane index
    ...
    struct SavedAutomationAssignment
    {
        ...
        int track = -1;           // Which track (0-4)
        int effectSlot = -1;      // Which effect slot (0-2), only used if !isInstrument
    };
    SavedAutomationAssignment savedAutomationAssignments[5];
...
public:
        ...
        const juce::AudioIODeviceCallbackContext& context) override
    {
        ...
        juce::MidiBuffer trackMidiBuffers[5];
        ...
        juce::AudioBuffer<float> track3Buffer(numOutputChannels, numSamples); // Capture Track 3 for sidechain into Track 2
        juce::AudioBuffer<float> track5Buffer(numOutputChannels, numSamples); // Capture Track 5 for sidechain into Track 4
        juce::AudioBuffer<float> mixBuffer(numOutputChannels, numSamples);
        mixBuffer.clear();

        // Process Track 5 first (to use as sidechain for Track 4)
        trackBuffer.clear();
        tracks[4].processBlock(trackBuffer.getArrayOfWritePointers(),
            numOutputChannels, numSamples, trackMidiBuffers[4], nullptr);

        // Copy Track 5 output for sidechain use (BEFORE volume is applied)
        for (int ch = 0; ch < numOutputChannels; ++ch)
        {
            track5Buffer.copyFrom(ch, 0, trackBuffer, ch, 0, numSamples);
        }

        // Apply Track 5 volume control to its OUTPUT (not the sidechain signal)
        float t5Vol = track5Volume.load();
        for (int ch = 0; ch < numOutputChannels; ++ch)
        {
            trackBuffer.applyGain(ch, 0, numSamples, t5Vol);
        }

        // Mix Track 5 into output (with volume applied)
        for (int ch = 0; ch < numOutputChannels; ++ch)
        {
            mixBuffer.addFrom(ch, 0, trackBuffer, ch, 0, numSamples);
        }

        // Process Track 4 WITH sidechain from Track 5 (unaffected by Track 5 volume)
        trackBuffer.clear();

        // Apply sidechain5 amount
        juce::AudioBuffer<float> scaledSidechain5(numOutputChannels, numSamples);
        for (int ch = 0; ch < numOutputChannels; ++ch)
        {
            scaledSidechain5.copyFrom(ch, 0, track5Buffer, ch, 0, numSamples);
            scaledSidechain5.applyGain(ch, 0, numSamples, sidechain5Amount.load());
        }

        tracks[3].processBlock(trackBuffer.getArrayOfWritePointers(),
            numOutputChannels, numSamples, trackMidiBuffers[3], &scaledSidechain5);

        // Mix Track 4 into output
        for (int ch = 0; ch < numOutputChannels; ++ch)
        {
            mixBuffer.addFrom(ch, 0, trackBuffer, ch, 0, numSamples);
        }

        // Process Track 3 (to use as sidechain for Track 2)
        trackBuffer.clear();
    ...
    void setSidechain5Amount(float amount)
    {
        sidechain5Amount.store(juce::jlimit(0.0f, 1.0f, amount));
    }

    float getSidechain5Amount() const
    {
        return sidechain5Amount.load();
    }

    void setTrack5Volume(float volume)
    {
        track5Volume.store(juce::jlimit(0.0f, 1.0f, volume));
    }

    float getTrack5Volume() const
    {
        return track5Volume.load();
    }

private:
    TrackAudioProcessor tracks[5];
    ...
    std::atomic<float> sidechain5Amount{ 0.0f }; // Track 5 -> Track 4 sidechain
    std::atomic<float> track5Volume{ 1.0f }; // Track 5 volume
    ...
    juce::MidiBuffer keyboardMidiQueue[5];
};
...
// Individual listener for each plugin to correctly track which plugin was touched
    ...
    juce::TextButton trackButtons[5];

    // Per-track plugin load buttons
    juce::TextButton loadInstrumentButtons[5];
    juce::TextButton loadEffectButtons[5][3]; // [track][effect slot]

    // GUI open buttons
    juce::TextButton openInstrumentGUIButtons[5];
    juce::TextButton openEffectGUIButtons[5][3]; // [track][effect slot]

    // MIDI import/export buttons per track
    juce::TextButton importMIDIButtons[5];
    juce::TextButton exportMIDIButtons[5];
    ...
    juce::TextButton automationLaneButtons[5];  // Select which lane to edit
    juce::TextButton assignAutomationButtons[5];  // Assign lane to last touched parameter
    juce::Label automationLaneLabels[5];  // Show what's assigned
    ...
    // New sliders for track 5 -> track 4 sidechain
    juce::Label sidechain5Label;
    juce::Slider sidechain5Slider;

    juce::Label track5VolumeLabel;
    juce::Slider track5VolumeSlider;
    ...
    std::unique_ptr<juce::AudioPluginInstance> vstInstruments[5];
    std::unique_ptr<juce::AudioPluginInstance> vstEffects[5][3]; // [track][effect slot]

    juce::AudioPluginFormatManager pluginFormatManager;
    juce::KnownPluginList knownPluginList;

    juce::PluginDescription savedInstrumentDesc[5];
    juce::PluginDescription savedEffectDesc[5][3];

    juce::String loadedInstrumentNames[5];
    juce::String loadedEffectNames[5][3];
    ...
    juce::Component::SafePointer<PluginWindow> instrumentWindows[5];
    juce::Component::SafePointer<PluginWindow> effectWindows[5][3];
