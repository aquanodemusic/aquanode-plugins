...
void MainComponent::paint(juce::Graphics& g)
{
    ...
    g.setFont(16.0f);
    g.drawText("SimpleDAW (3-Track DAW) by aquanode", getLocalBounds().removeFromTop(35), juce::Justification::centred, true);
}
                ...
                for (int laneIdx = 0; laneIdx < 3; ++laneIdx)
                {
                    ...
                    if (lane.isAssigned())
                    {
                        ...
                        if (plugin != nullptr)
                        {
                            ...
                            // Check instruments
                            for (int track = 0; track < 3 && !found; ++track)
                            {
                        ...
                        juce::Timer::callAfterDelay(200, [this, projectState]()
                            {
                                for (auto child : projectState)
                                {
                                    if (child.hasType("AutomationAssignment"))
                                    {
                                        ...
                                        if (isInstrument)
                                        {
                                            if (track >= 0 && track < 3 && vstInstruments[track] != nullptr)
                                            {
                                        ...
                                        else
                                        {
                                            ...
                                            if (track >= 0 && track < 3 && effectSlot >= 0 && effectSlot < 3 &&
    ...
    chooser->launchAsync(flags, [this, chooser](const juce::FileChooser& fc)
        {
            ...
            if (file != juce::File())
            {
                ...
                while (currentSample < totalSamples)
                {
                    ...
                    // Get sidechain amount and track 3 volume
                    ...
                    // Create buffers for each track and track 3 sidechain
                    juce::AudioBuffer<float> trackBuffers[3];
                    for (int i = 0; i < 3; ++i)
                ...
                if (outputStream != nullptr)
                {
                    ...
                    if (writer != nullptr)
                    {
                        ...
                        // Apply -6 dB headroom for simplicity
