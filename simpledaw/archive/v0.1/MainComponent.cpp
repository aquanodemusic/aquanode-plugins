...
{
    // Create parameter change listener
    parameterListener = std::make_unique<ParameterChangeListener>(*this);
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        loadInstrumentButtons[i].setButtonText("Load / Change Synth");
        ...
        {
            loadEffectButtons[i][fx].setButtonText("Load / Change FX" + juce::String(fx + 1));
    ...
    for (int i = 0; i < 3; ++i)
    {
        automationLaneButtons[i].setButtonText("Lane " + juce::String(i + 1));
        ...
        assignAutomationButtons[i].setButtonText("Assign");
    ...
    automationSectionLabel.setText("Automation Lanes (Bottom of Piano Roll):", juce::dontSendNotification);
    automationSectionLabel.setJustificationType(juce::Justification::centredLeft);
    automationSectionLabel.setFont(juce::Font(14.0f, juce::Font::bold));
    addAndMakeVisible(automationSectionLabel);
    ...
    sidechainLabel.setText("Sidechain Track 3 into Track 2:", juce::dontSendNotification);
    ...
    sidechainSlider.setSliderStyle(juce::Slider::LinearHorizontal);
    sidechainSlider.setTextBoxStyle(juce::Slider::TextBoxRight, false, 50, 20);
    ...
    track3VolumeLabel.setText("Track 3 Vol:", juce::dontSendNotification);
    ...
    track3VolumeSlider.setSliderStyle(juce::Slider::LinearHorizontal);
    track3VolumeSlider.setTextBoxStyle(juce::Slider::TextBoxRight, false, 50, 20);
    ...
    backgroundColorLabel.setText("BG Color:", juce::dontSendNotification);
...
void MainComponent::paint(juce::Graphics& g)
{
    ...
    g.setFont(16.0f);
    g.drawText("3-Track DAW", getLocalBounds().removeFromTop(35), juce::Justification::centred, true);
}
...
void MainComponent::resized()
{
    ...
    auto colorArea = controlRow1.removeFromRight(280);
    ...
    colorArea.removeFromRight(5);
    backgroundColorLabel.setBounds(colorArea.removeFromRight(65));
    ...
    auto controlRow2 = area.removeFromTop(40);
    controlRow2.reduce(10, 5);

    bpmLabel.setBounds(controlRow2.removeFromLeft(50));
    controlRow2.removeFromLeft(5);
    bpmSlider.setBounds(controlRow2.removeFromLeft(160));
    controlRow2.removeFromLeft(15);

    sidechainLabel.setBounds(controlRow2.removeFromLeft(210));
    controlRow2.removeFromLeft(5);
    sidechainSlider.setBounds(controlRow2.removeFromLeft(160));
    controlRow2.removeFromLeft(15);

    track3VolumeLabel.setBounds(controlRow2.removeFromLeft(100));
    controlRow2.removeFromLeft(5);
    track3VolumeSlider.setBounds(controlRow2.removeFromLeft(160));

    area.removeFromTop(8);

    // ===== TRACK CONTROLS: 3 rows (one horizontal line per track) =====
    for (int track = 0; track < 3; ++track)
    {
        auto trackRow = area.removeFromTop(52);
        ...
        // Load Synth
        loadInstrumentButtons[track].setBounds(trackRow.removeFromLeft(150));
        trackRow.removeFromLeft(6);

        // Synth GUI
        openInstrumentGUIButtons[track].setBounds(trackRow.removeFromLeft(95));
        trackRow.removeFromLeft(12);

        // FX1 + FX1 GUI
        loadEffectButtons[track][0].setBounds(trackRow.removeFromLeft(150));
        trackRow.removeFromLeft(6);
        openEffectGUIButtons[track][0].setBounds(trackRow.removeFromLeft(90));
        trackRow.removeFromLeft(8);

        // FX2 + FX2 GUI
        loadEffectButtons[track][1].setBounds(trackRow.removeFromLeft(150));
        trackRow.removeFromLeft(6);
        openEffectGUIButtons[track][1].setBounds(trackRow.removeFromLeft(90));
        trackRow.removeFromLeft(8);

        // FX3 + FX3 GUI
        loadEffectButtons[track][2].setBounds(trackRow.removeFromLeft(150));
        trackRow.removeFromLeft(6);
        openEffectGUIButtons[track][2].setBounds(trackRow.removeFromLeft(90));

        area.removeFromTop(4); // Space between tracks
    }

    area.removeFromTop(8);

    // ===== AUTOMATION LANE CONTROLS (below Track 3) =====
    auto automationRow = area.removeFromTop(35);
    automationRow.reduce(10, 5);

    // Automation section label
    automationSectionLabel.setBounds(automationRow.removeFromLeft(270));
    automationRow.removeFromLeft(10);

    for (int i = 0; i < 3; ++i)
    {
        automationLaneButtons[i].setBounds(automationRow.removeFromLeft(70));
        automationRow.removeFromLeft(5);
        assignAutomationButtons[i].setBounds(automationRow.removeFromLeft(70));
        automationRow.removeFromLeft(5);
        automationLaneLabels[i].setBounds(automationRow.removeFromLeft(120));
        automationRow.removeFromLeft(12);
    }
...
void MainComponent::loadPluginFromDescription(const juce::PluginDescription& desc, bool isInstrument,
    int trackOrSlot, const juce::String& pluginStateBase64)
{
    juce::String error;
    ...
    if (instance != nullptr)
    {
        if (isInstrument)
        {
            ...
            if (vstInstruments[track] != nullptr)
            {
                ...
                if (parameterListener != nullptr)
                    parameterListener->removePlugin(vstInstruments[track].get());
            ...
            // Restore plugin state if provided
        ...
        else // Effect
        {
            ...
            if (vstEffects[track][effectSlot] != nullptr)
            {
                ...
                // Remove from parameter listener tracking
                if (parameterListener != nullptr)
                    parameterListener->removePlugin(vstEffects[track][effectSlot].get());
            ...
            if (file != juce::File() && file.existsAsFile())
            {
                ...
                if (xml != nullptr)
                {
                    ...
                    if (projectState.isValid() && projectState.hasType("Project"))
                    {
                        ...
                        for (auto trackNode : projectState)
                        {
                            if (trackNode.hasType("Track"))
                            {
                                ...
                                if (instrumentNode.isValid())
                                {
                                    ...
                                    loadPluginFromDescription(desc, true, trackIndex, state);
                                }
                                ...
                                for (auto effectNode : trackNode)
                                {
                                    if (effectNode.hasType("Effect"))
                                    {
                                        ...
                                        loadPluginFromDescription(desc, false, effectSlot, state);
                                    }
                                }
                            }
                        }
                        ...
                        {
                            juce::MessageManager::callAsync([this, pianoRollState]()
                                {
                                    pianoRoll.setState(pianoRollState);

                                    // Restore automation assignments after a short delay to ensure plugins are fully loaded
                                    juce::Timer::callAfterDelay(100, [this]()
                                        {
                                            pianoRoll.restoreAutomationAssignments(
                                                vstInstruments,
                                                vstEffects
                                            );

                                            // Update automation lane labels
                                            for (int i = 0; i < 3; ++i)
                                            {
                                                if (pianoRoll.getAutomationLane(i).isAssigned())
                                                {
                                                    automationLaneLabels[i].setText(
                                                        pianoRoll.getAutomationLane(i).getAssignmentName(),
    ...
    chooser->launchAsync(flags, [this, chooser](const juce::FileChooser& fc)
        {
            ...
            if (file != juce::File())
            {
                ...
                const int samplesPerStep = static_cast<int>(sampleRate / sixteenthNotesPerSecond);
                ...
                while (currentSample < totalSamples)
                {
                    ...
                    for (int track = 0; track < 3; ++track)
                    {
                        ...
                        // Create temp buffer for track processing
                        juce::AudioBuffer<float> tempBuffer(numChannels, samplesThisTime);
                        tempBuffer.clear();

                        // Process instrument
                        vstInstruments[track]->processBlock(tempBuffer, midiMessages);
                        ...
                        for (int fx = 0; fx < 3; ++fx)
                        {
                            if (vstEffects[track][fx] != nullptr)
                            {
                                ...
                                vstEffects[track][fx]->processBlock(tempBuffer, emptyMidi);
                            }
                        }
                        ...
                        {
                            mixBuffer.addFrom(ch, 0, tempBuffer, ch, 0, samplesThisTime);
                        }
                    }
...
bool MainComponent::keyPressed(const juce::KeyPress& key, juce::Component* originatingComponent)
{
    ...
    // Send MIDI note on to current track's instrument
    if (vstInstruments[currentTrack] != nullptr)
    {
        activeKeyboardNotes.insert(midiNote);

        // Create a small audio buffer and send the note
        const int bufferSize = 32;
        juce::AudioBuffer<float> buffer(2, bufferSize);
        buffer.clear();

        juce::MidiBuffer midiBuffer;
        midiBuffer.addEvent(juce::MidiMessage::noteOn(1, midiNote, (juce::uint8)100), 0);

        // Process through instrument
        vstInstruments[currentTrack]->processBlock(buffer, midiBuffer);

        // Process through effects
        for (int fx = 0; fx < 3; ++fx)
        {
            if (vstEffects[currentTrack][fx] != nullptr)
            {
                juce::MidiBuffer emptyMidi;
                vstEffects[currentTrack][fx]->processBlock(buffer, emptyMidi);
            }
        }
    }
...
void MainComponent::setupParameterListeners(juce::AudioPluginInstance* plugin)
{
    ...
    // Create a lambda-based listener for this specific plugin
    // The lambda captures the plugin pointer, so we know which plugin the parameter belongs to
    for (auto* param : plugin->getParameters())
    {
        param->addListener(parameterListener.get());
    }
    
    // Store plugin pointer in listener so it knows which plugin sent the event
    parameterListener->addPlugin(plugin);
}
