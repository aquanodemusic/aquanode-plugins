...
private:
    ...
    // Freeze algorithm selector: Spectral (original) vs Cepstral (smoother)
    juce::ComboBox freezeTypeCombo;
    juce::Label freezeTypeLabel;

    // Stereo analysis toggle (off = mono mixdown, original behaviour)
    juce::ToggleButton stereoToggle;

    // Phase lock toggle (off = smeared/independent phase, original behaviour)
    juce::ToggleButton phaseLockToggle;

    // Playhead Slider - long slider spanning the control panel width, tracks
    // and controls playback position (host automatable via playheadParam).
    // Follows the current playback position when the user isn't dragging it,
    // and scrubs position (like clicking the waveform) when they are.
    juce::Slider playheadSlider;
    ...
    // Relabels/revalues the HF Boost slider to act as the Cepstral "Character"
    // control when Cepstral freeze type is selected, and back again for Spectral
    void updateHfBoostSliderForFreezeType();
