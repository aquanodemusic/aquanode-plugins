...
void WaveformDisplay::mouseDown(const juce::MouseEvent& event)
{
    ...
    processor.setUserScrubbing(true);
    processor.playheadParam->beginChangeGesture();
...
{
    // End the scrub gesture so the DAW registers a clean automation write
    processor.playheadParam->endChangeGesture();
    processor.setUserScrubbing(false);
}
    ...
    spectrumVisualizer(p)
{
    ...
    // Algorithm combo box (Spectral / Cepstral) - applies to both freeze
    // and normal time-stretched playback; Spectral leaves either mode
    // exactly as it originally behaved.
    addAndMakeVisible(freezeTypeCombo);
    freezeTypeCombo.addItem("Spectral", 1);
    freezeTypeCombo.addItem("Cepstral", 2);
    freezeTypeCombo.setSelectedId(audioProcessor.freezeTypeParam->getIndex() + 1, juce::dontSendNotification);
    freezeTypeCombo.onChange = [this]
        {
            int index = freezeTypeCombo.getSelectedId() - 1;
            audioProcessor.freezeTypeParam->beginChangeGesture();
            *audioProcessor.freezeTypeParam = index;
            audioProcessor.freezeTypeParam->endChangeGesture();

            // HF Boost slider doubles as the Cepstral "Character" control
            updateHfBoostSliderForFreezeType();
        };

    addAndMakeVisible(freezeTypeLabel);
    freezeTypeLabel.setText("Algorithm", juce::dontSendNotification);
    freezeTypeLabel.setJustificationType(juce::Justification::centred);
    freezeTypeLabel.setFont(juce::Font(11.0f));

    // Stereo toggle - off by default, mono mixdown left exactly as it was
    addAndMakeVisible(stereoToggle);
    stereoToggle.setButtonText("Stereo");
    stereoToggle.setToggleState(audioProcessor.stereoModeParam->get(), juce::dontSendNotification);
    stereoToggle.onClick = [this]
        {
            bool stereo = stereoToggle.getToggleState();
            audioProcessor.stereoModeParam->beginChangeGesture();
            *audioProcessor.stereoModeParam = stereo;
            audioProcessor.stereoModeParam->endChangeGesture();
        };

    // Phase Lock toggle - off by default (smeared transients, original
    // behaviour); on locks spectral peaks' neighbourhoods for sharper hits
    addAndMakeVisible(phaseLockToggle);
    phaseLockToggle.setButtonText("Phase Lock");
    phaseLockToggle.setToggleState(audioProcessor.phaseLockParam->get(), juce::dontSendNotification);
    phaseLockToggle.onClick = [this]
        {
            bool locked = phaseLockToggle.getToggleState();
            audioProcessor.phaseLockParam->beginChangeGesture();
            *audioProcessor.phaseLockParam = locked;
            audioProcessor.phaseLockParam->endChangeGesture();
        };

    //==========================================================================
    // Playhead Slider Setup
    //==========================================================================

    // Long slider spanning the control panel; mirrors playback position and
    // lets the user scrub it directly (including in freeze mode), and is
    // fully DAW automatable via audioProcessor.playheadParam.
    addAndMakeVisible(playheadSlider);
    playheadSlider.setRange(0.0, 1.0, 0.00001);
    playheadSlider.setValue(audioProcessor.playheadParam->get(), juce::dontSendNotification);
    playheadSlider.setSliderStyle(juce::Slider::LinearHorizontal);
    playheadSlider.setTextBoxStyle(juce::Slider::NoTextBox, false, 0, 0);
    playheadSlider.setColour(juce::Slider::trackColourId, juce::Colours::cyan);
    playheadSlider.setColour(juce::Slider::thumbColourId, juce::Colours::cyan);

    playheadSlider.onDragStart = [this]
        {
            audioProcessor.setUserScrubbing(true);
            audioProcessor.playheadParam->beginChangeGesture();
        };
    playheadSlider.onValueChange = [this]
        {
            // Only push into the processor while the user is actually
            // dragging -- otherwise this would also fire (and fight with)
            // the programmatic updates made from timerCallback() below.
            if (playheadSlider.isMouseButtonDown())
                audioProcessor.setPlayheadPosition(static_cast<float>(playheadSlider.getValue()));
        };
    playheadSlider.onDragEnd = [this]
        {
            audioProcessor.playheadParam->endChangeGesture();
            audioProcessor.setUserScrubbing(false);
        };
    ...
    timeStretchSlider.onDragStart = [this] { audioProcessor.timeStretch->beginChangeGesture(); };
    timeStretchSlider.onDragEnd = [this] { audioProcessor.timeStretch->endChangeGesture(); };
    ...
    hopSizeSlider.onDragStart = [this] { audioProcessor.hopSizeParam->beginChangeGesture(); };
    hopSizeSlider.onDragEnd = [this] { audioProcessor.hopSizeParam->endChangeGesture(); };
    ...
    glideSlider.onDragStart = [this] { audioProcessor.glideParam->beginChangeGesture(); };
    glideSlider.onDragEnd = [this] { audioProcessor.glideParam->endChangeGesture(); };
    ...
    hfBoostSlider.onValueChange = [this]
        {
            // This slider is shared: it drives HF Boost in Spectral freeze
            // type and Character (cepstral lifter cutoff) in Cepstral
            // freeze type, following whichever is currently selected.
            if (audioProcessor.freezeTypeParam->getIndex() == 1)
                *audioProcessor.characterParam = hfBoostSlider.getValue();
            else
                *audioProcessor.hfBoostParam = hfBoostSlider.getValue();
        };
    // This slider is shared between two parameters, so bracket the gesture
    // on whichever one is currently active.
    hfBoostSlider.onDragStart = [this]
        {
            if (audioProcessor.freezeTypeParam->getIndex() == 1)
                audioProcessor.characterParam->beginChangeGesture();
            else
                audioProcessor.hfBoostParam->beginChangeGesture();
        };
    hfBoostSlider.onDragEnd = [this]
        {
            if (audioProcessor.freezeTypeParam->getIndex() == 1)
                audioProcessor.characterParam->endChangeGesture();
            else
                audioProcessor.hfBoostParam->endChangeGesture();
        };
    ...
    microMovementSlider.onDragStart = [this] { audioProcessor.microMovementParam->beginChangeGesture(); };
    microMovementSlider.onDragEnd = [this] { audioProcessor.microMovementParam->endChangeGesture(); };
    ...
    // Sync the shared HF Boost/Character slider to whichever freeze type
    // is currently selected (handles state restored from a saved session)
    updateHfBoostSliderForFreezeType();
...
void GrainfreezeAudioProcessorEditor::resized()
{
    ...
    auto topBar = bounds.removeFromTop(200);
    topBar.reduce(10, 10);

    // Bottom row of the top bar: Stereo + Phase Lock toggles alongside the
    // playhead slider, so the slider doesn't need its own row and cost extra
    // vertical space.
    auto togglesAndPlayheadRow = topBar.removeFromBottom(20);
    stereoToggle.setBounds(togglesAndPlayheadRow.removeFromLeft(80));
    togglesAndPlayheadRow.removeFromLeft(8);
    phaseLockToggle.setBounds(togglesAndPlayheadRow.removeFromLeft(110));
    togglesAndPlayheadRow.removeFromLeft(12);
    playheadSlider.setBounds(togglesAndPlayheadRow);
    ...
    freezeTypeLabel.setBounds(buttonArea.removeFromTop(16));
    buttonArea.removeFromTop(2);
    freezeTypeCombo.setBounds(buttonArea.removeFromTop(24));
...
void GrainfreezeAudioProcessorEditor::timerCallback()
{
    ...
    // Follow the current playback position when the user isn't dragging the
    // slider themselves; while dragging, the slider is the source of truth.
    if (!playheadSlider.isMouseButtonDown())
        playheadSlider.setValue(audioProcessor.getPlayheadPosition(), juce::dontSendNotification);
...
void GrainfreezeAudioProcessorEditor::updateHfBoostSliderForFreezeType()
{
    bool cepstral = audioProcessor.freezeTypeParam->getIndex() == 1;

    if (cepstral)
    {
        hfBoostLabel.setText("Character", juce::dontSendNotification);
        hfBoostSlider.setValue(audioProcessor.characterParam->get(), juce::dontSendNotification);
    }
    else
    {
        hfBoostLabel.setText("HF Boost", juce::dontSendNotification);
        hfBoostSlider.setValue(audioProcessor.hfBoostParam->get(), juce::dontSendNotification);
    }
}
