    ...
    wtDisplayB(p, 1, "OSC B")
{
    ...
    cycleSizeEditorA.onTextChange = [this] {
        int v = cycleSizeEditorA.getText().getIntValue();
        if (v >= 16) audioProcessor.setCycleSizeParam(0, v);
    };
    ...
    cycleSizeEditorB.onTextChange = [this] {
        int v = cycleSizeEditorB.getText().getIntValue();
        if (v >= 16) audioProcessor.setCycleSizeParam(1, v);
    };
...
void TranswaveAudioProcessorEditor::resized()
{
    ...
    cycleSizeLabelA.setBounds(S(104, 678, 40, 20));
    cycleSizeEditorA.setBounds(S(148, 674, 56, 28));
    filenameLabelA.setBounds(S(212, 678, 372, 20));

    // Right zone (Slot B)
    filenameLabelB.setBounds(S(596, 678, 380, 20));
    cycleSizeLabelB.setBounds(S(980, 678, 40, 20));
    cycleSizeEditorB.setBounds(S(1024, 674, 56, 28));
    loadButtonB.setBounds(S(1084, 674, 84, 28));
