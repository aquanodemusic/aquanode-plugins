/*
    PhizmOscWaveFactory.cpp
    -----------------------
    Drop-in wave generation for PhizmOsc, ported from its bigger sibling
    Phizmo. Adds two ways to fill a wavetable slot
    ...
    loaded file: a shared .phizm opens on a machine that has never seen the
...
//==============================================================================
bool TranswaveAudioProcessor::generateRandomWavetable(int slot, juce::int64 seed, int numFrames,
    int cycleSamples, juce::String& statusOut,
    int archetypeOverride)
{
    if (slot < 0 || slot >= 2) { statusOut = "Bad slot"; return false; }
    ...
    if (archIdx < 0)
        archIdx = genArchetypeSetting.load() - 1;           // 0 in the list = Auto
    ...
    // Per-harmonic level scatter for this table, drawn once from the seed so a
    // re-roll of the same seed reproduces it exactly. Empty when the archetype
    // asks for no roughness, in which case evaluateKeyframe skips it entirely.
    //
    // The scatter widens with harmonic number (roughHi), because the reference
    // tables are markedly more ragged up top than they are around the
    // fundamental - a low harmonic sits where the spectrum says it should, a
    // high one lands wherever the original filter and sampler left it.
    std::vector<float> roughVec;
    if (A.rough > 0.0f)
    {
        juce::Random roughRng(seed ^ 0x5EED6047);
        roughVec.assign((size_t)maxH + 1, 1.0f);
        for (int h = 1; h <= maxH; ++h)
        {
            // Box-Muller from two uniforms: a normal scatter puts most
            // harmonics near the envelope and a few a long way off it, which is
            // the distribution the reference tables actually show. A uniform
            // one spreads them evenly and sounds artificial.
...
//==============================================================================
bool TranswaveAudioProcessor::transwavifyFile(int slot, const juce::File& file, int numFrames,
...
//==============================================================================
bool TranswaveAudioProcessor::installGeneratedFrames(int slot,
    ...
    bool genUnpitched, const juce::String& status)
{
    ...
    // like a loaded file, and a shared .phizm still opens on a machine that has
...
#pragma warning (pop)
...

    {
        ...
        wt[slot].genArchetype = -1;   // generateRandomWavetable() sets the real one
    ...
    // Silence first: no voice can be left reading the old frames.
    for (auto& v : voices)
    {
        v.active = false;
        v.envStage  = TranswaveVoice::Env::Idle;
        v.env2Stage = TranswaveVoice::Env::Idle;
...
bool TranswaveAudioProcessor::rerollSlot(int slot, juce::String& statusOut)
{
    if (slot < 0 || slot >= 2) { statusOut = "Bad slot"; return false; }
    ...
    if (origin == 2)
    {
        ...
        auto tmp = juce::File::getSpecialLocation(juce::File::tempDirectory)
            .getChildFile("phizmosc_reroll.wav");
...
bool TranswaveAudioProcessor::exportSlotAsWav(int slot, const juce::File& dest, juce::String& statusOut) const
{
    if (slot < 0 || slot >= 2) { statusOut = "Bad slot"; return false; }
...
//==============================================================================
// Menu-facing helpers. The archetype list is kept here rather than in the
// editor so there is exactly one place where a new archetype has to be added.
juce::StringArray TranswaveAudioProcessor::getArchetypeNames()
{
    juce::StringArray names;
    names.add("Auto");
    for (int i = 0; i < (int)Archetype::NumArchetypes; ++i)
        names.add(archetypeName(i));
    return names;
}

int TranswaveAudioProcessor::getSlotOrigin(int slot) const
{
    if (slot < 0 || slot >= 2) return 0;
    juce::ScopedLock l(wt[slot].lock);
    return wt[slot].loaded ? wt[slot].origin : 0;
}

juce::String TranswaveAudioProcessor::getSlotStatus(int slot) const
{
    if (slot < 0 || slot >= 2) return {};
    juce::ScopedLock l(wt[slot].lock);
    return wt[slot].genStatus;
}
