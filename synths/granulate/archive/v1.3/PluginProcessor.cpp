    ...
    int numSamples)
{
    ...
    // Clamp reversedGrains to actual maxGrains
            ...
            if (mousePressedNow && !mouseReleasedNow)
            {
                ...
                if (mouseGrainTimer >= grainInterval)
                {
                    ...
                    for (int v = 0; v < maxGrains; ++v)
                    {
                        ...
                        {
                            triggerGrain(mouseGrains[v], -1, basePos, v < reversedGrains);
                            break;
                        }
                    }
                }
            ...
            for (int v = 0; v < maxGrains; ++v)
            {
                ...
                if (idx >= 0 && idx < maxSampleIndex)
                {
                    ...
                    if (numChannels == 1)
                    {
                        ...
                        if (grain.isReverse && idx > 0)
                            sample = d[idx] + (float)(1.0 - frac) * (d[idx - 1] - d[idx]);
                        else
                    ...
                    else
                    {
                        ...
                        if (grain.isReverse && idx > 0)
                            sample = 0.5f * (l[idx] + (float)(1.0 - frac) * (l[idx - 1] - l[idx])
                                + r[idx] + (float)(1.0 - frac) * (r[idx - 1] - r[idx]));
                        else
                ...
                // Deactivate: forward grains hit end, reverse grains hit start
                if (grain.grainEnvelopePhase >= 1.0f
                    || (!grain.isReverse && grain.samplePosition >= maxSampleIndex)
                    || (grain.isReverse && grain.samplePosition <= 0.0))
            ...
            if (!nv.isReleased)
            {
                ...
                if (nv.grainTimer >= grainInterval)
                {
                    ...
                    for (int v = 0; v < maxGrains; ++v)
                    {
                        ...
                        {
                            triggerGrain(nv.grains[v], nv.midiNote, basePosKnob, v < reversedGrains);
                            break;
                        }
                    }
                }
