...
private:
    ...
    // ---- Display snapshot (written by audio thread, read by GUI thread) -----
    // Audio thread updates this under a ScopedTryLock (never blocks).
    // GUI thread reads it under a normal ScopedLock (waits for audio to finish).
