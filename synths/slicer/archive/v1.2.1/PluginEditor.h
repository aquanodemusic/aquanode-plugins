...
private:
    ...
    // Attachments MUST be declared last so they are destroyed first (before the
    // sliders/combo/buttons they reference). C++ destroys members in reverse
    // declaration order, so placing attachments after everything else prevents
    // a use-after-free that crashes the DAW when the editor is closed.
