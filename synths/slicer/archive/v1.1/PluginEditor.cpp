    ...
    : AudioProcessorEditor(&p), audioProcessor(p), waveformDisplay(p)
{
    ...
    reverseModeButton.setButtonText("Reverse Mode");
...
void SlicerAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    g.drawText("Click twice to add slice | Drag to move | Right-click to delete | Drag yellow bars for region | MIDI B1 (or B2) = Random Stream, C2 (C3) and above = Slices.",
        15, 40, 770, 20, juce::Justification::left);
...
void SlicerAudioProcessorEditor::resized()
{
    ...
    const int exportSectionY = 230;
    const int smallKnobSize = 70;

    // Slice End Mode combobox - below fadeout
    sliceEndModeCombo.setBounds(exportSectionX, exportSectionY + smallKnobSize + 10, 160, 24);
    
    // Reverse mode toggle button
    reverseModeButton.setBounds(exportSectionX, exportSectionY + smallKnobSize + 45, 160, 30);
    
    // Export slices button
    exportButton.setBounds(exportSectionX, exportSectionY + smallKnobSize + 85, 160, 40);
}
