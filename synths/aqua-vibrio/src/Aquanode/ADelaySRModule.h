...
{
public:
    // FORKED for Aqua Vibrio: added Ratio, which offsets the right channel's
    // time against the left's, and a time-modulation LFO (Rate and Depth) so
    // the delay drifts. Both are things the Virus delay has and this module
    // did not. Upstream has the first six parameters.
    enum ParamIndex { pTime = 0, pTapVolume, pAttack, pRelease, pDryWet, pMode,
                      pRatio, pModRate, pModDepth };
