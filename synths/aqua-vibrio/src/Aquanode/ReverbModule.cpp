...
void ReverbModule::blockStart()
{
    ...
    // Feedback pushes the tail longer than Decay alone would, up to the edge
    // of freezing - which is where a Virus reverb sits on a pad patch.
    const float feedback = juce::jlimit (0.0f, 1.0f, param (pFeedback) * 0.01f);

    p.roomSize   = juce::jlimit (0.0f, 0.98f,
        0.35f * param (pRoomSize) * 0.01f + 0.63f * decayNorm + 0.25f * feedback);
...
void ReverbModule::processSample (const StereoFrame* inputs, StereoFrame* outputs)
{
    ...
    // Colour: below centre the tail darkens, above it brightens. A one-pole
    // split and a weighted recombination, which is cheaper than a shelf and
    // sounds the same on a reverb tail.
    const float colour = juce::jlimit (-1.0f, 1.0f, param (pColour));
    ...
    if (colour != 0.0f)
    {
        ...
        {
            tilt[ch] += (*wetCh[ch] - tilt[ch]) * a;
            const float high = *wetCh[ch] - tilt[ch];
            *wetCh[ch] = tilt[ch] * (1.0f - colour) + high * (1.0f + colour);
        }
    }
...
static ModuleDescriptor reverbDescriptor()
{
    ...
    d.description = "A pre-delay line into a classic reverb core, with decay time and room size on the front.";
    ...
    d.params = {
        ...
        makeRotary ("feedback", "Feedback", 0.0f, 100.0f, 0.0f, 1, "%"),
        makeRotary ("colour",   "Colour",   -1.0f, 1.0f, 0.0f, 1)
    };
    return d;
}
