...
void AquaFilterModule::processVoiceSample (int v, const StereoFrame* inputs, StereoFrame* outputs)
{
    ...
    const int   type  = (int) param (pType);       // 0=LP12 1=LP24 2=LP24+ 3=BP 4=HP 5=SVF LP 6=Notch
    ...
    else   // SVF modes (Simper): BP / HP / LP / Notch
    {
        ...
        for (int c = 0; c < 2; ++c)
        {
            ...
            switch (type)
            {
                case 3:  y = v1; break;                     // BP
                case 5:  y = v2; break;                     // SVF LP (12 dB)
                case 6:  y = in - k * v1; break;            // Notch
                default: y = in - k * v1 - v2; break;       // HP
            }
            outputs[0][(size_t) c] = y;
        }
    }
...
static ModuleDescriptor aquaFilterDescriptor()
{
    ...
    d.params = {
        ...
        makeCombo  ("type",     "Filter Type", { "LP12", "LP24", "LP24+", "BP", "HP", "SVF LP", "Notch" }, 1, 1, 2),
