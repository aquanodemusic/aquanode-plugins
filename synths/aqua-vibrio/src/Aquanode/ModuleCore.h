...
public:
    ...
    // Index-based setter for the audio thread: no string hashing, no
    // allocation. Values outside the descriptor's range are clamped.
    void setParameterAt (int index, float value)
    {
        if (index >= 0 && index < (int) paramMin.size())
            paramValues[(size_t) index].store (juce::jlimit (paramMin[(size_t) index], paramMax[(size_t) index], value),
...
public:
    ...
    std::unique_ptr<SynthModule> createInstance (const juce::String& typeId) const
    {
        ...
        // Getting here means typeId has no registered module - almost always
        // because the module's .cpp isn't actually part of the build (a
        // stale/un-regenerated IDE project, an incremental build that missed
        // it, or the linker discarding the self-registration symbol). Fail
        // loudly here, naming the missing type, rather than handing back a
        // null pointer that the caller will silently dereference later.
        //
        // NOTE: DBG() and jassert() both compile to nothing in Release
        // builds, which is exactly where this was going unnoticed and then
        // crashing as a bare access violation. juce::Logger is active in
        // every build configuration.
        juce::Logger::writeToLog ("ModuleFactory::createInstance: no module registered for typeId \"" + typeId + "\"");
        jassertfalse;
        return nullptr;
    }
