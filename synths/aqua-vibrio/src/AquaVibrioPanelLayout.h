...
inline const std::vector<RegionSlot>& panelLayout()
{
    static const std::vector<RegionSlot> slots
    {
        ...
        //   column 3 - Effects (fx rack), column 4 - Amp.
        // Everything after that (envelopes, LFOs, matrix slots, arp, common)
        // just flows into whichever lane is shortest at that point.

        // --- column 1: oscillator ---------------------------------------------
        { "oscCommon",     "Osc Mixer",         0, 1.30f, 3 , "Oscillator" },
        { "oscA",          "Oscillator 1",      0, 1.45f, 3 , "Oscillator" },
        { "oscB",          "Oscillator 2",      0, 1.45f, 3 , "Oscillator" },
        { "oscC",          "Oscillator 3",      0, 0.72f, 3 , "Oscillator" },
        // Unison is how a voice is built, not an effect - it belongs with the
        // oscillators it stacks, which is also where you go looking for it.
        { "unison",        "Unison",            0, 0.62f, 3 , "Oscillator" },
        ...
        // --- column 3: effects rack ---------------------------------------------
        // In the order the audio actually passes through them, so stepping
        // down the selector walks the signal path:
        //   Input -> Distortion -> Filter Bank -> Vocoder -> Character -> EQ
        //         -> Phaser -> Chorus -> Delay -> Reverb -> Granulator
        { "input",         "Input",             2, 0.60f, 3 , "Effects" },
        { "distortion",    "Distortion",        2, 0.62f, 3 , "Effects" },
        { "filterBank",    "Filter Bank",       2, 0.70f, 3 , "Effects" },
        { "vocoder",       "Vocoder",           2, 0.70f, 3 , "Effects" },
        { "character",     "Character",         2, 0.42f, 3 , "Effects" },
        ...
        { "granulator",    "Granulator",        2, 0.60f, 3 , "Effects" },
