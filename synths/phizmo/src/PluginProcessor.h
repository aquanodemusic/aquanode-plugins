...
struct BlockEnvCoeffs
{
    ...
    double octA = 1.0;
    double octB = 1.0;
    // Tune (semitones) + Fine (cents) pitch multipliers, per oscillator.
    // These only depend on SP.tuneA/fineA/tuneB/fineB, which are constant for
    // the whole block, so they're computed once here instead of per sample.
    double tuneMA = 1.0;
    double tuneMB = 1.0;
};
...
struct SoundParams
{
    ...
    // Per-oscillator evolution playhead mode: 0 NORM, 1 RAND, 2 LOCK, 3 FIX.
    float evoMode = 0.f;
    float evoModeB = 0.f;
...
struct StereoFilter4Pole
{
    ...
    void setType(int type, float freq, float q, double sr)
    {
        ...
        case 2: {
            float q1 = q * 0.7071f, q2 = q * 1.3066f;
            s1.setHighpass(freq, q1, sr); s2.setHighpass(freq, q2, sr);
        } break;
...
struct PhizmoVoice
{
    ...
    // Note-start order stamp, used by the EVO: LOCK playhead mode to find the
    // "first note still held" (the anchor) among a Sound's active voices.
    unsigned long long evoOrder = 0;
...
public:
    ...
    // --- Per-voice evolution playheads (one per sounding note) --------------
    // The visualiser iterates these to draw one playhead per active voice on
    // the focused Sound. getEvoPlayheadSound() returns the voice's Sound index,
    // or -1 if the voice is silent (so no playhead is drawn).
    int   getEvoPlayheadCount() const { return MAX_VOICES; }
    int   getEvoPlayheadSound(int voice) const
    {
        if (voice < 0 || voice >= MAX_VOICES) return -1;
        return voicePubSound[voice].load();
    }
    float getEvoPlayheadScan(int voice, int osc) const
    {
        if (voice < 0 || voice >= MAX_VOICES) return 0.f;
        return (osc == 0 ? voicePubScanA : voicePubScanB)[voice].load();
    }

    // FIX-mode master playhead: a free-running scan per Sound+osc that keeps
    // cycling even with no notes held. New notes in FIX mode follow it.
    float getEvoFixPhase(int sound, int osc) const
    {
        if (sound < 0 || sound >= NUM_SOUNDS) return 0.f;
        return evoFixPub[sound][osc == 0 ? 0 : 1].load();
    }
    ...
    // v3 adds the per-slot origin descriptor. Older presets load unchanged:
    // the attributes are simply absent and every slot reads back as "file".
    static constexpr int kPresetFormatVersion = 3;
    ...
    //==========================================================================
    // 2.0 — wave generation (PhizmoWaveFactory.cpp)
    //==========================================================================
    // Both fill wt[slot] at the given cycle size and behave from then on
    // exactly like a table loaded from disk: same playback path, same
    // visualiser, same preset embedding.
    //
    // Pass seed = 0 to roll a fresh one. numFrames 4..256, cycleSamples 16..4096.
    // archetypeOverride: -1 uses the genArchetype parameter (or rolls one when
    // that is set to Auto); 0..7 forces a specific family. Recipe presets use
    // the override so a stored seed always regenerates the same wave.
    ...
    // Turns any audio file into a Transwave. Pitched material is resynthesised
    // from its harmonics; unpitched material is sliced and resampled.
    ...
    // Re-runs whatever produced the slot, with a new seed for random slots.
    bool rerollSlot(int slot, juce::String& statusOut);

    // Writes the slot's rendered wavetable audio out as an ordinary .wav file.
    // Works for every origin (file, random recipe, transwavified) since a
    // rendered WAV is always kept in memory for the slot even when the preset
    // zip itself skips embedding it (random/recipe slots). Returns false if
    // the slot is empty or the write fails.
    bool exportSlotAsWav(int slot, const juce::File& dest, juce::String& statusOut) const;

    int          getSlotOrigin(int slot) const;      // 0 file, 1 random, 2 transwavified
    juce::int64  getSlotSeed(int slot) const;
    juce::String getSlotStatus(int slot) const;

    //==========================================================================
    // 2.0 — band-limited engine (PhizmoBandlimit.cpp)
    //==========================================================================
    void  buildMips(int slot);        // call after a table is installed
    void  clearMips(int slot);
    // Returns false if the mips are not available right now (not built yet, or
    // a rebuild is swapping them in). The caller then takes the vintage path,
    // so a rebuild can never drop a sample or read freed memory.
    bool  sampleFrameMip(int slot, float frameIndex, double phase, double freqHz, float& out) const;
    bool  isBandlimitedActive() const;
...
private:
    struct WavetableSlot
    {
        ...
        // --- 2.0: where this table came from -------------------------------
        // Generated tables are rendered out as an ordinary wavetable WAV into
        // originalFileData, so the preset zip embeds them exactly like a loaded
        // file and a shared .phizmo still opens on a machine that has never
        // seen the wave. The descriptor below is kept alongside so the wave can
        // also be re-rolled or re-analysed, and so the UI can say what it is.
        int          origin = 0;            // 0 = file, 1 = random, 2 = transwavified
        juce::int64  genSeed = 0;
        int          genArchetype = -1;     // -1 = rolled, 0..7 = a named family
        int          genFrames = 32;
        bool         genUnpitched = false;
        juce::String genStatus;             // human-readable analysis result

        mutable juce::CriticalSection lock;
    };

    // --- 2.0: band-limited engine -------------------------------------------
    // A mip pyramid built alongside each slot rather than inside WavetableSlot,
    // so the struct and its serialisation are untouched. Mip 0 keeps 1024
    // harmonics, mip 1 keeps 512, and so on; playback picks the coarsest level
    // whose top harmonic still fits under Nyquist for the note being played.
    struct MipSet
    {
        static constexpr int kMipCount = 10;
        static constexpr int kBaseLength = 2048;
        static constexpr int kBaseHarms = 1024;

        std::vector<std::array<std::vector<float>, kMipCount>> levels;  // [frame][mip]
        int numFrames = 0;
        std::atomic<bool> ready{ false };

        // A SpinLock, not a CriticalSection: the audio thread only ever
        // *tries* to take it and falls back to the vintage read if a rebuild
        // happens to be swapping the levels in at that moment. That keeps the
        // audio thread from ever blocking on the message thread.
        mutable juce::SpinLock swapLock;

        static int levelLength(int m) { return juce::jmax(64, kBaseLength >> m); }
        static int levelHarms(int m) { return juce::jmax(1, kBaseHarms >> m); }
    };
    // 8 wavetable slots: slot = sound * 2 + osc  (osc 0 = A, osc 1 = B)
    WavetableSlot wt[NUM_SOUNDS * 2];
    MipSet        mips[NUM_SOUNDS * 2];
...
private:
    ...
    // --- Per-voice evolution playheads --------------------------------------
    // One published scan position per polyphony voice, so the evolution
    // visualiser can draw a separate playhead for every sounding note instead
    // of a single shared one. The old single slot per oscillator was written by
    // every voice on the same Sound, so whichever voice ran last in the block
    // won — that is what made the playhead jump and wiggle with chords.
    // voicePubSound is the Sound the voice is playing, or -1 when the voice is
    // silent (→ no playhead drawn for it).
    std::atomic<int>   voicePubSound[MAX_VOICES];
    std::atomic<float> voicePubScanA[MAX_VOICES];   // raw scan phase, osc A
    std::atomic<float> voicePubScanB[MAX_VOICES];   // raw scan phase, osc B

    // --- EVO playhead modes (NORM / RAND / LOCK / FIX) ----------------------
    // FIX keeps one perpetual master scan per Sound + oscillator, advanced every
    // sample regardless of whether any note is playing. evoFixPub mirrors it for
    // the editor so the master bar is always visible in FIX mode.
    double             evoFixPhase[NUM_SOUNDS][2] = {};
    int                evoFixDir[NUM_SOUNDS][2] = {};   // +1/-1 for ping-pong
    std::atomic<float> evoFixPub[NUM_SOUNDS][2];
    // Monotonic note-start counter; each voice's evoOrder is stamped from it so
    // LOCK can pick the oldest still-sounding voice of a Sound as the anchor.
    unsigned long long evoOrderCounter = 0;
    // Sets a freshly triggered voice's scan start point per the osc's EVO mode.
    void applyEvoStart(int voiceIndex, int sound);
    ...
    std::atomic<float>* pEvoMode = nullptr;
    std::atomic<float>* pEvoModeB = nullptr;
    ...
    // 2.0 engine switches
    std::atomic<float>* pOscEngine = nullptr;   // 0 = Vintage, 1 = Band-limited
    std::atomic<float>* pGritVintage = nullptr;   // GRIT forces the vintage path
    ...
    // 2.0: freqHz and slot are needed to select a mip level. Both have safe
    // defaults, so any call site that does not care keeps working unchanged and
    // simply gets the original raw path.
    float sampleFrameRaw(const WavetableSlot& s, float frameIndex, double phase, bool interp,
        float smoothAmt = 0.f, double freqHz = 0.0, int slot = -1) const;
    bool installGeneratedFrames(int slot, std::vector<std::vector<float>>&& frames,
        int cycleSamples, const juce::String& displayName,
