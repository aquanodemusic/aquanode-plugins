...
PhizmoAudioProcessor::createParameterLayout()
{
    ...
    //==========================================================================
    // 2.0 additions. Every default here reproduces 1.x behaviour exactly, so an
    // existing preset or project loads and sounds identical.
    //==========================================================================

    // Oscillator engine. "Vintage" is the original raw read: bilinear, no
    // band-limiting, aliases above roughly C4 - which is what the hardware's
    // sample playback chip does, and what GRIT is built on top of.
    // "Band-limited" reads a mip pyramid instead and stays clean across the
    // whole keyboard. Default 0 = unchanged.
    params.push_back(std::make_unique<juce::AudioParameterChoice>(
        "oscEngine", "Osc Engine",
        juce::StringArray{ "Vintage (aliasing)", "Band-limited" }, 0));

    // GRIT quantises the read position to as few as 6 steps per cycle, and it
    // gets its bite from having a full-bandwidth table to fold. Band-limiting
    // the source first makes it noticeably tamer, so by default any GRIT above
    // zero drops that voice back to the vintage path.
    params.push_back(std::make_unique<juce::AudioParameterBool>(
        "gritVintage", "GRIT Forces Vintage", true));

    // Wave generation settings, stored so a project remembers how a slot was
    // made and the Re-roll button behaves consistently.
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        "genFrames", "Generate Frames",
        juce::NormalisableRange<float>(4.f, 256.f, 1.f), 32.f));
    params.push_back(std::make_unique<juce::AudioParameterBool>(
        "genUnpitched", "Generate: Slice Mode", false));

    // Random Transwave archetype. "Auto" rolls one per generation; the named
    // entries bias the roll towards a family of behaviour. The parameter ranges
    // behind each were derived by measuring a set of Fizmo wavetables - their
    // spectral tilt, how far the harmonic centroid travels, frame-to-frame
    // motion, odd/even balance, peakiness and comb depth. It reproduces how
    // those categories behave, not their data.
    params.push_back(std::make_unique<juce::AudioParameterChoice>(
        "genArchetype", "Random Archetype",
        juce::StringArray{ "Auto", "Resonant Sweep", "Formant", "Bell Stack",
                           "Analog Smooth", "Unrelated", "Hollow", "Comb Groove",
                           "Organ Drawbar",
                           "Metallic Sweep", "Inharmonic Bell", "Spectral Grit",
                           "Vocal Morph" }, 0));
    ...
    apvts(*this, nullptr, "Parameters", createParameterLayout())
{
    ...
    pOscEngine   = apvts.getRawParameterValue("oscEngine");
    pGritVintage = apvts.getRawParameterValue("gritVintage");
    ...
    const juce::String& displayFilePath, int cs, int slot)
{
    ...
    // 2.0: loading from disk always resets the slot to "file" origin.
    {
        juce::ScopedLock sl(wt[slot].lock);
        wt[slot].origin = 0;
        wt[slot].genSeed = 0;
    ...
    // Silence the voices BEFORE rebuilding the mip pyramid, so no voice can be
    // reading the levels while they are replaced.
    for (auto& v : voices) { v.active = false; v.envStage = PhizmoVoice::Env::Idle; }
    buildMips(slot);
    return true;
}

int PhizmoAudioProcessor::getSlotOrigin(int s) const
{ juce::ScopedLock l(wt[s].lock); return wt[s].origin; }
juce::int64 PhizmoAudioProcessor::getSlotSeed(int s) const
{ juce::ScopedLock l(wt[s].lock); return wt[s].genSeed; }
juce::String PhizmoAudioProcessor::getSlotStatus(int s) const
{ juce::ScopedLock l(wt[s].lock); return wt[s].genStatus; }
...
//==============================================================================
bool PhizmoAudioProcessor::isBandlimitedActive() const
{
    return pOscEngine != nullptr && pOscEngine->load() > 0.5f;
}

float PhizmoAudioProcessor::sampleFrameRaw(const WavetableSlot& s, float fi, double ph, bool interp,
                                           float smoothAmt, double freqHz, int slot) const {
    if (s.frames.empty()) return 0.f;

    // --- 2.0 band-limited path ---------------------------------------------
    // Falls through to the original raw read whenever the engine is set to
    // Vintage, the mips are not built yet, or the caller did not supply a
    // frequency - so nothing can silently change behaviour.
    float mipResult = 0.f;
    if (slot >= 0 && freqHz > 0.0 && isBandlimitedActive()
        && sampleFrameMip(slot, fi, ph, freqHz, mipResult))
    {
        // Cycle-edge smoothing behaves identically in both engines.
        if (smoothAmt > 0.001f)
        {
            float fadeLen = smoothAmt * 0.15f;
            float env = 1.f;
            if (ph < (double)fadeLen)              env = (float)(ph / (double)fadeLen);
            else if (ph > 1.0 - (double)fadeLen)   env = (float)((1.0 - ph) / (double)fadeLen);
            mipResult *= env;
        }
        return mipResult;
    }
...
void PhizmoAudioProcessor::addAllSlotsToZip(juce::ZipFile::Builder& zb, juce::XmlElement& xml)
{
    for (int i = 0; i < NUM_SOUNDS * 2; ++i)
    {
        ...
        int origin = 0, genFrames = 32, genArch = -1; juce::int64 seed = 0;
        bool genUnpitched = false; juce::String genStatus;
        {
            ...
            if (wt[i].loaded)
            {
                ...
                origin = wt[i].origin;
                seed = wt[i].genSeed;
                genFrames = wt[i].genFrames;
                genUnpitched = wt[i].genUnpitched;
                genStatus = wt[i].genStatus;
                genArch = wt[i].genArchetype;
            }
        }
        if (!has) continue;
        ...
        // 2.0: how this table was made. The audio bytes are embedded either
        // way, so a v3 preset opens fine in 1.x - these attributes are simply
        // ignored there - and a v2 preset opens fine here, defaulting to
        // origin 0 ("file").
        if (origin != 0)
        {
            xml.setAttribute("slotOrigin"    + juce::String(i), origin);
            xml.setAttribute("slotSeed"      + juce::String(i), juce::String(seed));
            xml.setAttribute("slotGenFrames" + juce::String(i), genFrames);
            xml.setAttribute("slotGenSlice"  + juce::String(i), genUnpitched ? 1 : 0);
            xml.setAttribute("slotGenStatus" + juce::String(i), genStatus);
            xml.setAttribute("slotGenArch"   + juce::String(i), genArch);
        }
        // A random slot is fully described by its seed and archetype, so there
        // is no reason to carry half a megabyte of audio for it. "Recipe"
        // presets like this are a couple of kilobytes and contain no sampled
        // material at all. Sampled and file-loaded slots still embed their
        // bytes, so they keep working on a machine that has never seen them.
...
{
    // Pass 1: any slot described as a recipe (origin 1) is rebuilt from its
    // seed. Deterministic, so it comes back byte-identical to what was saved,
    // and the preset needed no audio to carry it.
    for (int i = 0; i < NUM_SOUNDS * 2; ++i)
    {
        if (xml.getIntAttribute("slotOrigin" + juce::String(i), 0) != 1) continue;

        const juce::int64 seed = xml.getStringAttribute("slotSeed" + juce::String(i), "0")
                                    .getLargeIntValue();
        if (seed == 0) continue;

        const int frames = xml.getIntAttribute("slotGenFrames" + juce::String(i), 32);
        const int cycle  = xml.getIntAttribute("slotCycle"     + juce::String(i), 2048);
        const int arch   = xml.getIntAttribute("slotGenArch"   + juce::String(i), -1);

        juce::String status;
        generateRandomWavetable(i, seed, frames, cycle, status, arch);
    }

    // Pass 2: slots whose audio really is embedded.
    for (int e = 0; e < zip.getNumEntries(); ++e)
    {
        ...
        // 2.0: restore how the table was made. The embedded bytes are already
        // installed above, so this is only metadata - it makes Re-roll work and
        // lets the UI say "Random" rather than "file". Absent in v2 presets, in
        // which case the slot correctly stays as origin 0.
        const int origin = xml.getIntAttribute("slotOrigin" + juce::String(slot), 0);
        if (origin != 0)
        {
            juce::ScopedLock l(wt[slot].lock);
            wt[slot].origin       = origin;
            wt[slot].genSeed      = xml.getStringAttribute("slotSeed" + juce::String(slot), "0")
                                       .getLargeIntValue();
            wt[slot].genFrames    = xml.getIntAttribute("slotGenFrames" + juce::String(slot), 32);
            wt[slot].genUnpitched = xml.getIntAttribute("slotGenSlice" + juce::String(slot), 0) != 0;
            wt[slot].genStatus    = xml.getStringAttribute("slotGenStatus" + juce::String(slot));
            wt[slot].genArchetype = xml.getIntAttribute("slotGenArch" + juce::String(slot), -1);
        }
    }
}
    ...
    const BlockEnvCoeffs& bc, const SoundParams& SP)
{
    ...
    const int slotA = wt[rawA].loaded ? rawA : 0;
    const int slotB = wt[rawB].loaded ? rawB : 1;
    const WavetableSlot& sA_wt = wt[slotA];
    const WavetableSlot& sB_wt = wt[slotB];
    ...
    // 2.0: GRIT wants a full-bandwidth table to fold. When it is engaged and
    // "GRIT Forces Vintage" is on, hand the reads slot = -1 so they take the
    // original raw path regardless of the engine setting.
    const bool gritOverride = (SP.grit > 0.0f)
                            && (pGritVintage == nullptr || pGritVintage->load() > 0.5f);
    const int  blSlotA = gritOverride ? -1 : slotA;
    const int  blSlotB = gritOverride ? -1 : slotB;
    ...
    if (sA_wt.loaded)
    {
        ...
        float raw = sampleFrameRaw(sA_wt, fiA, lA, doInterp, smoothA, fA, blSlotA);
        raw = applyBitCrush(raw, SP.bitCrush);
        float w = SP.stereoWidth;
        float rawB = sampleFrameRaw(sA_wt, fiA, std::fmod(lA + phB, 1.0), doInterp, smoothA, fA, blSlotA);
    ...
    if (sB_wt.loaded)
    {
        // Note: no lock needed here — processBlock() already holds every
        // wt[] slot's lock for the duration of the per-sample loop.
        float raw = sampleFrameRaw(sB_wt, fiB, lB, doInterp, smoothB, fB, blSlotB);
        raw = applyBitCrush(raw, SP.bitCrush);
        float w = SP.stereoWidth;
        float rawA2 = sampleFrameRaw(sB_wt, fiB, lA, doInterp, smoothB, fB, blSlotB);
