...
void SampleFieldLookAndFeel::drawRotarySlider(juce::Graphics& g,
    ...
    juce::Slider& /*slider*/)
{
    ...
    if (pos > 0.001f)
    {
        ...
        juce::ColourGradient fg(fromU32(colCyanDark), cx, cy - radius,
            fromU32(colCyan), cx, cy + radius, false);
        g.setGradientFill(fg);
        g.strokePath(fillPath, juce::PathStrokeType(4.0f, juce::PathStrokeType::curved,
            juce::PathStrokeType::rounded));
    }
    ...
    // Body disc
    {
        ...
        g.fillEllipse(cx - radius * 0.78f, cy - radius * 0.78f,
...
void SampleFieldLookAndFeel::drawButtonBackground(juce::Graphics& g,
    juce::Button& button,
    ...
    bool isOver, bool isDown)
{
    ...
    juce::ColourGradient grad(top, 0.0f, bounds.getY(),
        bot, 0.0f, bounds.getBottom(), false);
...
void SampleFieldLookAndFeel::drawButtonText(juce::Graphics& g,
    juce::TextButton& button, bool /*isOver*/, bool isDown)
{
...
void LabelledKnob::sliderValueChanged(juce::Slider*)
{
    updateValueLabel();
}
...
{
    valueLabel.setText(formatParamValue(paramID, (float)slider.getValue()),
...
// TempoLockButton
//==============================================================================
TempoLockButton::TempoLockButton()
{
    setRepaintsOnMouseActivity(true);
}

void TempoLockButton::setCurrentSteps(int s)
{
    currentSteps = juce::jlimit(0, 8, s);
    repaint();
}
...
void TempoLockButton::paint(juce::Graphics& g)
{
    ...
    // Fill — active: cyan gradient, inactive: muted
    if (active)
    {
        juce::ColourGradient grad(
            fromU32(SampleFieldLookAndFeel::colCyan).brighter(isOver ? 0.15f : 0.0f), 0.0f, b.getY(),
    ...
    {
        g.setColour(isOver
            ? fromU32(SampleFieldLookAndFeel::colKnobTrack).brighter(0.08f)
            : fromU32(SampleFieldLookAndFeel::colKnobTrack));
    }
    g.fillRoundedRectangle(b, cr);

    // Border
    g.setColour(active
        ? fromU32(SampleFieldLookAndFeel::colCyanDark).withAlpha(0.7f)
        : fromU32(SampleFieldLookAndFeel::colCyanLight).withAlpha(0.9f));
    ...
    g.setColour(active
        ? juce::Colours::white
        : fromU32(SampleFieldLookAndFeel::colTextMid));
...
//==============================================================================
// SampleFieldAudioProcessorEditor
//==============================================================================
    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    sampleCountLabel.setColour(juce::Label::textColourId,
        fromU32(SampleFieldLookAndFeel::colTextMid));
    sampleCountLabel.setJustificationType(juce::Justification::centred);
    addAndMakeVisible(sampleCountLabel);
    updateSampleCountLabel();

    // Global knobs — all four now added
    addAndMakeVisible(knobPan);
    addAndMakeVisible(knobRate);
    addAndMakeVisible(knobVol);
    addAndMakeVisible(knobTime);   // FIX: was missing

    // Tempo lock button — synced to processor state, wired to callback
    tempoLockBtn.setCurrentSteps(audioProcessor.getTempoLock());   // FIX: was missing
    tempoLockBtn.onStepChanged = [this](int steps)                 // FIX: was missing
        {
            audioProcessor.setTempoLock(steps);
        };
    addAndMakeVisible(tempoLockBtn);                                // FIX: was missing
    ...
    // APVTS attachments — attTime now created
    ...
    attTime = std::make_unique<SliderAttachment>(audioProcessor.apvts, "time", knobTime.slider);  // FIX: was missing
    ...
    setSize(480, 430);   // slightly taller to fit 2x2 global grid + tempo button
    startTimerHz(4);
}
...
//==============================================================================
void SampleFieldAudioProcessorEditor::timerCallback()
{
    updateSampleCountLabel();
}
...
void SampleFieldAudioProcessorEditor::updateSampleCountLabel()
{
    ...
    sampleCountLabel.setText(juce::String(n) + " / 128 samples",
        juce::dontSendNotification);
}
...
void SampleFieldAudioProcessorEditor::paintBackground(juce::Graphics& g)
{
    ...
    juce::ColourGradient bg(fromU32(0xFFE4F7FB), b.getCentreX(), 0.0f,
        fromU32(0xFFCCEFF7), b.getCentreX(), b.getBottom(), false);
    g.setGradientFill(bg);
    g.fillAll();

    juce::ColourGradient radial(juce::Colours::white.withAlpha(0.55f),
        b.getCentreX(), b.getY() + 40.0f,
        juce::Colours::white.withAlpha(0.0f),
        b.getCentreX(), b.getY() + 200.0f, true);
    ...
    juce::ColourGradient gloss(juce::Colours::white.withAlpha(0.45f),
        b.getCentreX(), card.getY(),
        juce::Colours::white.withAlpha(0.0f),
        b.getCentreX(), card.getY() + card.getHeight() * 0.4f, false);
...
void SampleFieldAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    juce::ColourGradient nameGrad(fromU32(SampleFieldLookAndFeel::colCyan), 0.0f, 28.0f,
        fromU32(SampleFieldLookAndFeel::colCyanDark), 0.0f, 46.0f, false);
    g.setGradientFill(nameGrad);
    g.drawText("S A M P L E F I E L D", getLocalBounds().withHeight(56),
        juce::Justification::centred, false);
}
...
void SampleFieldAudioProcessorEditor::resized()
{
    ...
    const int knobH = 105;
    const int knobW = 78;
    const int knobGapX = 4;
    const int knobGapY = 6;
    const int tempoLockH = 22;   // height of the tempo lock pill button

    // ---- GLOBAL section (2x2 grid: Pan/Rate top row, Vol/Time bottom row) ----
    headerGlobal.setBounds(area.removeFromTop(sectionH));
    area.removeFromTop(4);

    {
        // Two rows of 2 knobs each, centred
        const int totalW = 2 * knobW + knobGapX;
        const int startX = area.getX() + (area.getWidth() - totalW) / 5;
        const int rowY1 = area.getY();
        const int rowY2 = rowY1 + knobH + knobGapY;

        knobPan.setBounds(startX, rowY1, knobW, knobH);
        knobRate.setBounds(startX + (knobW + knobGapX), rowY1, knobW, knobH);
        knobVol.setBounds(startX + 2* (knobW + knobGapX), rowY1, knobW, knobH);
        knobTime.setBounds(startX + 3* (knobW + knobGapX), rowY1, knobW, knobH);

        // Tempo lock button sits directly under the Time knob, same width
        tempoLockBtn.setBounds(startX + 3* (knobW + knobGapX), rowY1 + knobH + 2, knobW, tempoLockH);

        area.removeFromTop(knobH + knobGapY + knobH + tempoLockH + 6);
    }

    area.removeFromTop(-110);

    // ---- RANDOM section (4 knobs in one row) ----
...

    {
        ...
        const int rowY = area.getY();

        knobPanRnd.setBounds(startX, rowY, knobW, knobH);
        knobRateRnd.setBounds(startX + (knobW + knobGapX), rowY, knobW, knobH);
        knobVolRnd.setBounds(startX + 2 * (knobW + knobGapX), rowY, knobW, knobH);
        knobSkip.setBounds(startX + 3 * (knobW + knobGapX), rowY, knobW, knobH);
    }
}