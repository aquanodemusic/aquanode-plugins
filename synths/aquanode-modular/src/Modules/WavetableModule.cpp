...
static ModuleDescriptor wavetableDescriptor()
{
    ...
    d.description =
        "Load any audio file and it is read as a stack of single-cycle frames; Position scans and "
        "crossfades through them, morphing the timbre without touching the pitch (which the "
        "Oscillator's Sample mode cannot do - it only reads the first cycle). Position In is the "
        "socket to reach for first: an LFO or ADSR there is what makes a wavetable patch move. "
        "Frame Size must match how the file was made - 2048 is a common convention.";
