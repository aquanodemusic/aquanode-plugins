...
// Pitch Lock Filter - adapted from the PitchControlEQ VST, reduced to its core idea:
...
public:
    ...
    {
        // the note keys can be driven by knob-modulation cables (LFO / DAW
        // automation into a key) - param() folds that modulation in, so the
        // effective on/off pattern is re-checked every block and the filter
        // bank is rebuilt whenever a key flips
        juce::uint32 pattern = 0;
        for (int i = 0; i < 12; ++i)
            if (param (6 + i) > 0.5f)   // nc0..nc11, base + cable modulation
                pattern |= (1u << i);

        const bool patternChanged = pattern != livePattern.exchange (pattern, std::memory_order_relaxed);

        if (dirty.exchange (false, std::memory_order_relaxed) || patternChanged)
            rebuildFilters();
    }

    // effective (cable-modulated) state of a note class, for the keyboard UI
    bool isClassLiveOn (int noteClass) const
    {
        return (livePattern.load (std::memory_order_relaxed) >> juce::jlimit (0, 11, noteClass)) & 1u;
    }
...
private:
    ...
    std::atomic<juce::uint32> livePattern { 0 };   // effective nc0..nc11 bits (incl. cable mod)
