...
void AquanodeModularAudioProcessor::startVoice (
    ...
    bool listenersOnly, bool selfReleased)
{
    ...
    for (const auto& n : nodes)
    {
        ...
        const bool shouldHear = listenersOnly
            ? (n->midiSourceId == midiSource || ! hasAddMidiIn)
...
void AquanodeModularAudioProcessor::engineNoteOn (const ProcessGraph& graph, int note, float velocity01)
{
    ...
    // plus whatever each listened-to Midi Add adds around it
    for (const auto& ma : graph.midiAdds)
    {
        ...
        {
            const int added = note + adder->offsetSemitones (slot);
            if (added < 0 || added > 127)
                continue;   // transposed off the end of the keyboard

            startVoice (nodes, added, note, ma.instanceId, velocity01, true);
        }
    }
}
    ...
    const std::vector<std::shared_ptr<ModuleInstance>>& nodes, int note, int driverInstanceId)
{
    ...
    {
        if (voiceStates[v].active && voiceStates[v].held
            && voiceStates[v].midiSource == driverInstanceId && voiceStates[v].note == note)
        {
                                              ...
                                              int toModule, const juce::String& toSocket)
{
    ...
    // a generator follows a single Midi Add, so a new one replaces the old
    if (dstMidi)
        cables.erase (std::remove_if (cables.begin(), cables.end(),
            [&] (const CableInfo& c) { return c.toModule == toModule && c.toSocket == toSocket; }),
            cables.end());
...
void AquanodeModularAudioProcessor::rebuildGraph()
{
    ...
    for (const auto& c : cables)
    {
        ...
        {
            instances[(size_t) to]->midiSourceId = c.fromModule;
            instances[(size_t) to]->midiSourceReplaces =
                instances[(size_t) from]->dsp->midiSourceReplacesInput();
            continue;
        }
    ...
    for (const auto& inst : instances)
    {
        ...
        if (hasListener)
            graph->midiAdds.push_back ({ inst->id, inst->dsp.get() });
    }
