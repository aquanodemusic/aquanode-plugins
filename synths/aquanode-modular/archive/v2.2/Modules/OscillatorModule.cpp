...
static ModuleDescriptor oscillatorDescriptor()
{
    ...
    d.description =
        "An FM operator with true DX7-style phase modulation, and the main generator of this "
        "synth. FM In wants another Oscillator's Audio Out (that is what makes it an operator "
        "stack); Env In wants an ADSR, and with nothing patched there the note simply gates on "
        "and off. Both arrive as that voice's OWN signals, so operator stacks and envelopes act "
        "per note like a real polysynth.";
