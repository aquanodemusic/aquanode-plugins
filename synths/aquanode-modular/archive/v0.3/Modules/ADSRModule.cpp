...
void ADSRModule::processSample (const StereoFrame*, StereoFrame* outputs)
{
    ...
    // The output is a single mono modulation signal shared by everything it
    // feeds, so it must stay in [0,1]. Summing voices would let two held notes
    // reach ~2.0, which then doubles amplitudes and (on an FM operator) the
    // modulation index - causing runaway loudness. Take the loudest active
    // voice instead: retriggers and chords stay musical and bounded.
    float peak = 0.0f;
    ...
    for (int v = 0; v < VoiceAllocator::maxVoices; ++v)
    {
        ...
        peak = juce::jmax (peak, level[v]);
    }

    outputs[0][0] = peak;   // unipolar [0,1]
    outputs[0][1] = peak;
}
