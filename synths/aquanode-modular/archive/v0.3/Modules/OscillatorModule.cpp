...
void OscillatorModule::processSample (const StereoFrame* inputs, StereoFrame* outputs)
{
    ...
    for (int v = 0; v < VoiceAllocator::maxVoices; ++v)
    {
        ...
        if (envConnected)
        {
            // amplitude comes entirely from the external envelope on Env In.
            // once the note is released AND the envelope has gone (and stayed)
            // silent, free the voice - this stops enveloped tones lingering.
            gain = 1.0f;
            if (! slot.gate)
            {
                if (envIn <= 1.0e-4f)
                ...
                if (silentFor[v] > silenceReleaseSeconds)
                {
                    voices.freeVoice (v);
                    continue;
                }
