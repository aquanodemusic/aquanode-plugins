...
//==============================================================================
// VoiceAllocator - 24-voice pool, oldest-voice stealing, retrigger-same-note
//==============================================================================
...
public:
    ...
    Slot slots[maxVoices];

    // returns the voice index; wasRetrigger true if the same note was already sounding
    int noteOn (int note, bool& wasRetrigger)
    {
        wasRetrigger = false;
        for (int i = 0; i < maxVoices; ++i)
            if (slots[i].inUse && slots[i].note == note)
            {
                slots[i].gate = true;
                slots[i].order = ++counter;
                wasRetrigger = true;
                return i;
            }
        ...
        for (int i = 0; i < maxVoices; ++i)
            if (! slots[i].inUse)
                return activate (i, note);

        int oldest = 0;
        for (int i = 1; i < maxVoices; ++i)
            if (slots[i].order < slots[oldest].order)
                oldest = i;
        return activate (oldest, note);
    }

    int noteOff (int note)
    {
        for (int i = 0; i < maxVoices; ++i)
            if (slots[i].inUse && slots[i].gate && slots[i].note == note)
            {
                slots[i].gate = false;
                return i;
            }
    ...
    void allGatesOff()
    {
        for (auto& s : slots) s.gate = false;
    }

    void freeVoice (int i)
    {
        slots[i] = Slot();
    }
...
public:
    ...
    // inputs / outputs indexed by descriptor input/output order.
    // Audio sockets use both channels; Modulation sockets use channel 0 only.
    virtual void processSample (const StereoFrame* inputs, StereoFrame* outputs) = 0;

    //=== MIDI (Oscillator / Sampler / ADSR only) ==============================
