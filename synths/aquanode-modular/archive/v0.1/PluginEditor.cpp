...
{
    constexpr int pad = 8;
    constexpr int socketRowH = 26;
    constexpr int nameRowH = 16;
    constexpr int knobRowH = 68;
    constexpr int wideRowH = 26;      // rows containing only combos/buttons
    constexpr int unitsPerRow = 5;
    constexpr int socketSize = 12;
    ...
    for (const auto& spec : desc.params)
    {
        ...
        switch (spec.type)
        {
            ...
            case ParamType::RotarySteppedList:
            {
                ...
                slider->setTextBoxStyle (juce::Slider::TextBoxBelow, false, 42, 15);
...
int ModuleComponent::layoutEverything (bool apply)
{
    ...
    auto layoutSocketRow = [&] (const std::vector<const SocketSpec*>& list, bool isInput)
    {
        if (list.empty())
            return;
        ...
        if (apply)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                ref.centre = { layout::pad + innerW * (2 * i + 1) / (2 * n), y + 7 };
    ...
    // 1. input sockets, 2. module name, 3. output sockets
    layoutSocketRow (desc.inputs(), true);
    y += layout::nameRowH;
    layoutSocketRow (desc.outputs(), false);
    y += 2;
    ...
    for (auto rowIt = 0;; ++rowIt)
    {
        ...
        if (apply)
        {
            ...
            for (auto* c : rowControls)
            {
                ...
                if (c->spec->type == ParamType::Rotary || c->spec->type == ParamType::RotarySteppedList)
                {
                    c->labelArea = area.removeFromTop (12);
                    c->component->setBounds (area.reduced (1));
                }
                else
                {
                    c->labelArea = {};
                    c->component->setBounds (area.withSizeKeepingCentre (w - 4, 20)
                                                 .withY (y + (rowH - 20) / 2 + (hasKnob ? 6 : 0)));
                }
                x += w;
            }
        }
...
void ModuleComponent::paint (juce::Graphics& g)
{
    ...
    // module name between the input and output socket rows
    const bool hasInputs = ! desc.inputs().empty();
    const int nameY = layout::pad + (hasInputs ? layout::socketRowH : 0);
    g.setColour (juce::Colours::black);
    g.setFont (juce::Font (juce::FontOptions (13.0f, juce::Font::bold)));
    g.drawText (desc.displayName, 0, nameY, getWidth(), layout::nameRowH, juce::Justification::centred);

    // sockets: circular = Audio, square = Modulation, small functional label
    ...
    for (const auto& s : sockets)
    {
        ...
        if (s.kind == SocketKind::Audio)
            ...
            g.drawRect (c.x - r, c.y - r, 2 * r, 2 * r, 1.2f);

        // socket label (distinguishes same-type sockets like FM In / Env In)
        const SocketSpec* spec = nullptr;
        for (const auto& sk : processor.getInstance (instanceId)->descriptor().sockets)
        ...
        {
            g.setColour (juce::Colours::black);
            g.drawText (spec->label, s.centre.x - 34, s.centre.y + 7, 68, 10,
