...
NoteAdderProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterBool>(
        "scaletransposeroot", "Scale Transpose Root", false));
    layout.add(std::make_unique<juce::AudioParameterInt>(
    ...
    apvts(*this, nullptr, "NoteAdder", createParameterLayout())
{
    ...
    addLengthParam = dynamic_cast<juce::AudioParameterFloat*>(vts->getParameter("addlength"));
    ...
    scaleTransposeRootParam = dynamic_cast<juce::AudioParameterBool*>(vts->getParameter("scaletransposeroot"));
    ...
    juce::MidiBuffer& midi)
{
    ...
    for (const auto meta : combinedInput)
    {
        ...
        if (msg.isNoteOn())
        {
            ...
            if (discardMode == 2 && !isNoteInScale(note))
            {
                ...
                // ── Cancel any pending Add-Length note-offs for notes we're about to play ──
                // A pending note-off queued by a previous release of `rerouted` (or any note
                // whose added pitches overlap with ours) must not cut off the notes we are
                // turning on right now.  Without this, shared added pitches (e.g. two different
                // input notes both generating G4) accumulate stale kill-events that cascade.
                if (!pendingNoteOffs.empty())
                {
                    const int ch1 = msg.getChannel();
                    pendingNoteOffs.erase(
                        std::remove_if(pendingNoteOffs.begin(), pendingNoteOffs.end(),
                            [&](const PendingNoteOff& pno) {
                                if (pno.midiChannel != ch1) return false;
                                if (pno.noteNumber == rerouted) return true;
                                for (int n : addedNotes)
                                    if (pno.noteNumber == n) return true;
                                return false;
                            }),
                        pendingNoteOffs.end());
                }
            ...
            // Scale Transpose Root: if enabled in scale mode, shift the root note
            // by the same in-scale steps as the added notes (off by default).
            const int scaleStepsForRoot = (scaleMode && scaleTransposeRootParam->get())
                ? scaleTransposeParam->get() : 0;
            const int playRoot = (scaleStepsForRoot != 0)
                ? juce::jlimit(0, 127, noteAtScaleDegreeOffset(note, scaleStepsForRoot))
                : note;
            if (playRoot != note)
                reroutedNoteMap[ch][note] = playRoot;

            // Input note passthrough – untouched, no humanization
            if (!dropInput && !cloudMuteOn)
                processed.addEvent(
                    juce::MidiMessage::noteOn(msg.getChannel(), playRoot, (juce::uint8)inputVel), pos);

            auto addedNotes = computeAddedNotes(note);

            // ── Cancel any pending Add-Length note-offs for notes we're about to play ──
            // Same rationale as the reroute path above: prevents shared added pitches
            // (common in scale/chord mode) from being prematurely killed by stale
            // pending note-offs from a previous release.  Also handles same-key re-press.
            if (!pendingNoteOffs.empty())
            {
                const int ch1 = msg.getChannel();
                pendingNoteOffs.erase(
                    std::remove_if(pendingNoteOffs.begin(), pendingNoteOffs.end(),
                        [&](const PendingNoteOff& pno) {
                            if (pno.midiChannel != ch1) return false;
                            if (pno.noteNumber == playRoot) return true;
                            for (int n : addedNotes)
                                if (pno.noteNumber == n) return true;
                            return false;
                        }),
            ...
            for (int added : addedNotes)
            {
                ...
                if (!animOn)
                {
                    ...
                    {
                        pendingHumanizedNotes.push_back({ msg.getChannel(), playRoot, added,
            ...
            if (animOn)
            {
                killAnimForNote(ch, playRoot, processed, pos);

                AnimNoteState state;
                state.channel = ch;
                state.velocity = inputVel;  // base; tick fns apply humanize themselves
                state.inputNote = playRoot;
                state.baseNotes = addedNotes;
                state.currentNotes = addedNotes;
                state.wanderSteps.assign(addedNotes.size(), 0);
                state.invStep = 0;
        ...
        else if (msg.isNoteOff())
        {
            ...
            // Add Length: delay user-played and static-added note-offs only.
            // Animation note-offs (killAnimForNote / processCloudNoteOffs) are never delayed.
            const float addLenSec = addLengthParam->get();
            const int   delSamples = addLenSec > 0.0f
                ? juce::roundToInt(addLenSec * (float)currentSampleRate) : 0;

            if (!dropInput && !cloudMuteOn)
            {
                if (delSamples > 0)
                    pendingNoteOffs.push_back({ msg.getChannel(), noteForOff, delSamples + pos });
                else
                    processed.addEvent(
            ...
            if (animOn)
                killAnimForNote(ch, noteForOff, processed, pos);  // animation: never delayed
            else
            {
                for (int added : activeAddedNotes[ch][noteForOff])
                {
                    if (delSamples > 0)
                        pendingNoteOffs.push_back({ msg.getChannel(), added, delSamples + pos });
                    else
                        processed.addEvent(
    ...
    // to erase any pending notes whose key was released before they fired.
    {
        ...
        for (auto& pn : pendingHumanizedNotes)
        {
            if (pn.samplesRemaining < numSamples)
            {
                ...
                // Cancel any pending Add-Length note-off for this exact pitch.
                // This deferred note-on fires BEFORE 1c runs, so a stale pending
                // note-off from a prior release would immediately kill it in 1c.
                if (!pendingNoteOffs.empty())
                {
                    const int ch1 = pn.midiChannel;
                    const int noteNum = pn.addedNote;
                    pendingNoteOffs.erase(
                        std::remove_if(pendingNoteOffs.begin(), pendingNoteOffs.end(),
                            [ch1, noteNum](const PendingNoteOff& pno) {
                                return pno.midiChannel == ch1 && pno.noteNumber == noteNum;
                            }),
                        pendingNoteOffs.end());
                }
    ...
    // ── 1c. Fire expired Add-Length note-offs directly into processed ──────────
    {
        std::vector<PendingNoteOff> carry;
        carry.reserve(pendingNoteOffs.size());
        for (auto& pno : pendingNoteOffs)
        {
            if (pno.samplesRemaining < numSamples)
                processed.addEvent(
                    juce::MidiMessage::noteOff(pno.midiChannel, pno.noteNumber, (juce::uint8)0),
                    pno.samplesRemaining);
            else
            {
                pno.samplesRemaining -= numSamples;
                carry.push_back(pno);
            }
        }
        pendingNoteOffs = std::move(carry);
    }
    ...
    // Add Length is handled at note-off origin (in the note-off handler above).
    // Note-offs from animation/cloud go through immediately; only user/static notes
    // are deferred via pendingNoteOffs, which section 1c already fires into processed.
