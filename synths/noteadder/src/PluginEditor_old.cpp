// This is a version of the editor that has the new add length slider logic,
// but the layout is not cleaned up yet.
    ...
    : AudioProcessorEditor(&p), proc(p), pianoRoll(p)
{
    ...
    // ── Scale in-scale transpose (ctrlRow2, scale-mode only) ─────
    ...
    addLengthLabel.setText("+Length", juce::dontSendNotification);
...
void NoteAdderEditor::updateModeUI()
{
    ...
    // Scale in-scale transpose: only visible in scale mode
    scaleTranspLabel.setVisible(isScale);
    scaleTranspSlider.setVisible(isScale);
    updateInversionUI();
...
void NoteAdderEditor::resized()
{
    ...
    // ── Global control row 2: Scale Transpose (scale mode only) + Global Transpose ──
    {
        ...
        // ── Equal-track layout ─────────────────────────────────────────────
        // trackW   : slider travel distance — identical for all three controls
        // *BoxW    : value-box width, sized to each control's longest display string
        // *LblW    : label width, sized to each label's text
        // lblGap   : tight gap between a label's right edge and the slider
        // slGap    : section separator between one slider and the next label
        //
        // At sf=1.0: x starts at ox+4=54, available=506px; total used=506px.
        //   Scale Transp.:  stLblW + lblGap + (trackW + stBoxW) + slGap  = 68+2+98+4  = 172
        //   Transpose:      gtLblW + lblGap + (trackW + gtBoxW) + slGap  = 52+2+112+4 = 170
        //   Add Length:     alLblW + lblGap + (trackW + alBoxW)           = 48+2+114   = 164
        //                                                               Total = 506 px
        const int trackW = juce::roundToInt(66 * L.sf);   // slider track — same for all three
        const int stBoxW = juce::roundToInt(46 * L.sf);   // Scale Transp. box  (fits \"-12\")
        const int gtBoxW = juce::roundToInt(46 * L.sf);   // Transpose box      (fits \"-12 st\")
        const int alBoxW = juce::roundToInt(38 * L.sf);   // Add Length box     (fits \"10.00 s\")
        const int stLblW = juce::roundToInt(68 * L.sf);   // \"Scale Transp.\" label
        const int gtLblW = juce::roundToInt(52 * L.sf);   // \"Transpose\" label
        const int alLblW = juce::roundToInt(48 * L.sf);   // \"Add Length\" label
        const int lblGap = juce::roundToInt(2 * L.sf);   // label  → slider gap
        const int slGap = juce::roundToInt(4 * L.sf);   // slider → next label gap
        // Derived slider widths (track + value box):
        const int stSlW = trackW + stBoxW;   // 98 px
        const int gtSlW = trackW + gtBoxW;   // 112 px
        const int alSlW = trackW - 10 + alBoxW;   // 114 px

        int x = ox + juce::roundToInt(4 * L.sf);
        scaleTranspLabel.setBounds(x, cy, stLblW, juce::roundToInt(20 * L.sf)); x += stLblW + lblGap;
        scaleTranspSlider.setTextBoxStyle(juce::Slider::TextBoxRight, false, stBoxW, 20);
        scaleTranspSlider.setBounds(x, cy - 2, stSlW, slH); x += stSlW + slGap;

        globalTransposeLabel.setBounds(x, cy, gtLblW, juce::roundToInt(20 * L.sf)); x += gtLblW + lblGap;
        globalTransposeSlider.setTextBoxStyle(juce::Slider::TextBoxRight, false, gtBoxW, 20);
        globalTransposeSlider.setBounds(x, cy - 2, gtSlW, slH); x += gtSlW + slGap;

        addLengthLabel.setBounds(x, cy, alLblW, juce::roundToInt(20 * L.sf)); x += alLblW + lblGap;
        addLengthSlider.setTextBoxStyle(juce::Slider::TextBoxRight, false, alBoxW, 20);
        addLengthSlider.setBounds(x, cy - 2, alSlW, slH);
    }
    ...
    // ── Scale tab ─────────────────────────────────────────────
    {
        ...
        // Row 2: Harmonic mode
        {
            ...
            const int hCmbW = juce::roundToInt(190 * L.sf);  // narrowed from 250
            const int andW = juce::roundToInt(26 * L.sf);
            const int hCmb2W = juce::roundToInt(120 * L.sf);
            int x = ox + margin;
        ...
        // Toggle rows 1 & 2 share column positions to form a clean 2×3 grid:
        //   Col1: discardCombo (row1) / randomSkipBtn+label (row2)
        //   Col2: discardInputBtn+label (row1) / lockBassBtn+label (row2)
        //   Col3: bwColorsBtn (row1) / resetColorsBtn (row2)
        {
            const int btnW = juce::roundToInt(82 * L.sf);
            const int tLblW = juce::roundToInt(155 * L.sf);
            const int colGap = juce::roundToInt(12 * L.sf);
            // Col3 is right-aligned
            const int col3X = ox + W - margin - btnW;
            // Col2 is positioned so its toggle+label ends just before col3
            const int col2X = col3X - colGap - tLblW - 3 - toggleW;

            // Toggle row 1
            {
                const int ty = L.scaleTog1Y + (30 - 22) / 2;
                // discardCombo fills from margin to col2 with a small gap
                const int discardCmbW = col2X - (ox + margin) - juce::roundToInt(10 * L.sf);
                int x = ox + margin;
                discardCombo.setBounds(x, ty, discardCmbW, 22);
                x = col2X;
                discardInputBtn.setBounds(x, ty, toggleW, 22); x += toggleW + 3;
                discardInputLabel.setBounds(x, ty, tLblW, 22);
                bwColorsBtn.setBounds(col3X, ty, btnW, 22);
            }

            // Toggle row 2 – three toggles sharing the space equally before resetColorsBtn
            {
                const int ty = L.scaleTog2Y + (30 - 22) / 2;
                const int totalW = col3X - juce::roundToInt(8 * L.sf) - (ox + margin);
                const int slotW  = totalW / 3;
                const int togLblW = slotW - toggleW - 3;
                int x = ox + margin;
                randomSkipBtn.setBounds(x, ty, toggleW, 22);       x += toggleW + 3;
                randomSkipLabel.setBounds(x, ty, togLblW, 22);     x += togLblW;
                lockBassBtn.setBounds(x, ty, toggleW, 22);         x += toggleW + 3;
                lockBassLabel.setBounds(x, ty, togLblW, 22);       x += togLblW;
                scaleTransposeRootBtn.setBounds(x, ty, toggleW, 22); x += toggleW + 3;
                scaleTransposeRootLabel.setBounds(x, ty, togLblW, 22);
                resetColorsBtn.setBounds(col3X, ty, btnW, 22);
            }
        }
    ...
    // ── PerNote (CUSTOM) tab ──────────────────────────────────
    {
        ...
        // RND / RST below the last sharp row (row index 5 in right column)
        {
            const int btnW = juce::roundToInt(54 * L.sf);
            const int btnY = pnRowStart + 5 * rowH + (rowH - comboH) / 2;
            int x = ox + colHalf + margin;
            pnRandomizeBtn.setBounds(x, btnY, btnW, comboH);   x += btnW + g3;
            pnResetBtn.setBounds(x, btnY, btnW, comboH);
        }
    ...
    {
        // Row 2: rate (combo for BPM-sync, slider for free) + "Allow for longer notes" toggle
        // Slightly compact sizes to leave room for cluster decay on the right.
        const int cy2 = L.animRow2Y + (L.animRowH - comboH) / 2;
        const int rateLblW = juce::roundToInt(38 * L.sf);
        const int rateCmbW = juce::roundToInt(110 * L.sf);
        const int rateSlW = juce::roundToInt(150 * L.sf);
        const int varBtnGap = juce::roundToInt(10 * L.sf);
        const int varBtnW = juce::roundToInt(120 * L.sf);

        // Scale
        {
            const int rateEndX = ox + margin + rateLblW + gap + rateSlW;
            ...
            scaleAnimUpToBtn.setBounds(rateEndX + varBtnGap, cy2, varBtnW, comboH);
        }
        // Manual
        {
            const int rateEndX = ox + margin + rateLblW + gap + rateSlW;
            ...
            manAnimUpToBtn.setBounds(rateEndX + varBtnGap, cy2, varBtnW, comboH);
        }
        // PerNote
        {
            const int rateEndX = ox + margin + rateLblW + gap + rateSlW;
            pnAnimRateLabel.setBounds(ox + margin, cy2, rateLblW, comboH);
            pnAnimRateCombo.setBounds(ox + margin + rateLblW + gap, cy2, rateCmbW, comboH);
            pnAnimRateSlider.setBounds(ox + margin + rateLblW + gap, cy2 - 1, rateSlW, comboH + 2);
            pnAnimUpToBtn.setBounds(rateEndX + varBtnGap, cy2, varBtnW, comboH);
        }
        // Cluster – Decay anchored to the right of row 2 (shared position for scale + pn)
        {
            const int dcLblW = juce::roundToInt(44 * L.sf);
            const int dcSlW = juce::roundToInt(100 * L.sf);
            const int dcGap = juce::roundToInt(10 * L.sf);
            const int rateEndX = ox + margin + rateLblW + gap + rateSlW;
            const int afterAllow = rateEndX + varBtnGap + varBtnW + dcGap;
            scaleClusterDecayLabel.setBounds(afterAllow, cy2, dcLblW, comboH);
            scaleClusterDecaySlider.setBounds(afterAllow + dcLblW + gap, cy2 - 1, dcSlW, comboH + 2);
            pnClusterDecayLabel.setBounds(afterAllow, cy2, dcLblW, comboH);
            pnClusterDecaySlider.setBounds(afterAllow + dcLblW + gap, cy2 - 1, dcSlW, comboH + 2);
        }
    }

    {
        ...
        // Scale – Cloud row 3
        {
            const int dLblW = juce::roundToInt(58 * L.sf), dCmbW = juce::roundToInt(80 * L.sf);
            ...
            scaleCloudDensityCombo.setBounds(x, cy3, dCmbW, comboH);  x += dCmbW + gap + 4;
        ...
        // Manual – Cloud row 3
        {
            ...
            manCloudDensityCombo.setBounds(x, cy3, dCmbW, comboH); x += dCmbW + gap;
        ...
        // PerNote – Cloud row 3
        {
            ...
            const int g16 = juce::roundToInt(16 * L.sf);
            int x = ox + margin;
            pnCloudDensityLabel.setBounds(x, cy3, dLblW, comboH); x += dLblW + gap;
            pnCloudDensityCombo.setBounds(x, cy3, dCmbW, comboH); x += dCmbW + gap;
            pnCloudOctaveLabel.setBounds(x, cy3, sLblW, comboH);  x += sLblW + gap;
            pnCloudOctaveCombo.setBounds(x, cy3, sCmbW, comboH);  x += sCmbW + g16;
            pnCloudDecayLabel.setBounds(x, cy3, dcLblW, comboH);  x += dcLblW + gap;
            pnCloudDecaySlider.setBounds(x, cy3 - 1, dcSlW, comboH + 2);
        }
...

    {
        ...
        {
            int x = ox + margin;
            pnCloudVelLabel.setBounds(x, cy4, vLblW, comboH);    x += vLblW + gap;
            pnCloudVelMinCombo.setBounds(x, cy4, vCmbW, comboH); x += vCmbW + gap;
            pnCloudVelDashLabel.setBounds(x, cy4, dashW, comboH); x += dashW + gap;
            pnCloudVelMaxCombo.setBounds(x, cy4, vCmbW, comboH);
        }
