...
private:
    ...
    // ── Global transpose (always visible, lives in anim row 1) ──
    juce::Label  globalTransposeLabel;
    juce::Slider globalTransposeSlider;
    std::unique_ptr<SA> globalTransposeAtt;

    // ── Scale in-scale transpose (visible only in Scale mode, in ctrlRow2) ────────
    juce::Label  scaleTranspLabel;
    juce::Slider scaleTranspSlider;
    std::unique_ptr<SA> scaleTranspAtt;

    // ── Add Length: value box always visible to the right of Transpose ────────
    juce::Label  addLengthLabel;
    juce::Slider addLengthSlider;
    std::unique_ptr<SA> addLengthAtt;
    ...
    juce::Label    harmonicMode2AndLabel;
    juce::ComboBox harmonicMode2Combo;
    std::unique_ptr<CBA> harmonicMode2ComboAtt;

    // 0=Allow all  1=Discard non-scale  2=Play nearest in-scale
    juce::ComboBox  discardCombo;
    std::unique_ptr<CBA> discardComboAtt;

    juce::ToggleButton discardInputBtn;
    juce::Label        discardInputLabel;
    std::unique_ptr<BA> discardInputAtt;

    juce::ToggleButton randomSkipBtn, lockBassBtn, scaleTransposeRootBtn;
    juce::Label        randomSkipLabel, lockBassLabel, scaleTransposeRootLabel;
    std::unique_ptr<BA> randomSkipAtt, lockBassAtt, scaleTransposeRootAtt;
    juce::TextButton   resetColorsBtn{ "RST CLR" };
    juce::TextButton   bwColorsBtn{ "BW CLR" };

    juce::TextButton resetManualBtn{ "RST" };
    juce::TextButton rndActivateBtn{ "RND Act" };
    juce::TextButton rndSemiBtn{ "RND Sem" };
    juce::TextButton rndOctBtn{ "RND Oct" };
    ...
    juce::ToggleButton scaleAnimUpToBtn;         // param: "animuptovar"
    std::unique_ptr<BA>  scaleAnimUpToAtt;
    ...
    juce::Label    scaleClusterLowNotesLabel, scaleClusterHighNotesLabel;
    juce::Label    scaleClusterLowOctLabel, scaleClusterHighOctLabel;
    juce::Label    scaleClusterLowOctSepLabel, scaleClusterHighOctSepLabel;
    juce::ComboBox scaleClusterLowNotesCombo, scaleClusterHighNotesCombo;
    juce::ComboBox scaleClusterLowOctStartCombo, scaleClusterLowOctEndCombo;
    juce::ComboBox scaleClusterHighOctStartCombo, scaleClusterHighOctEndCombo;
    std::unique_ptr<CBA> scaleClusterLowNotesAtt, scaleClusterHighNotesAtt;
    std::unique_ptr<CBA> scaleClusterLowOctStartAtt, scaleClusterLowOctEndAtt;
    std::unique_ptr<CBA> scaleClusterHighOctStartAtt, scaleClusterHighOctEndAtt;

    juce::Label    scaleClusterVelLabel, scaleClusterVelDashLabel;
    juce::ComboBox scaleClusterVelMinCombo, scaleClusterVelMaxCombo;
    std::unique_ptr<CBA> scaleClusterVelMinAtt, scaleClusterVelMaxAtt;

    juce::Label  scaleClusterDecayLabel;
    juce::Slider scaleClusterDecaySlider;
    std::unique_ptr<SA>  scaleClusterDecayAtt;
    ...
    juce::ToggleButton manAnimUpToBtn;           // param: "manimuptovar"
    std::unique_ptr<BA>  manAnimUpToAtt;
    ...
    // ── PerNote (CUSTOM) tab ──────────────────────────────────
    // Pitch-class layout: naturals (0,2,4,5,7,9,11) in col 1,
    //                     sharps   (1,3,6,8,10)     in col 2.
    static constexpr int PN_NATURALS[7] = { 0, 2, 4, 5, 7, 9, 11 };
    static constexpr int PN_SHARPS[5] = { 1, 3, 6, 8, 10 };

    juce::Label      pnChordLabels[12];
    juce::ComboBox   pnChordCombos[12];
    juce::Label      pnCustomLabel[12];
    juce::TextEditor pnCustomEditors[12];

    juce::TextButton pnRandomizeBtn{ "RND" };
    juce::TextButton pnResetBtn{ "RST" };
    juce::Label      pnHintLabel;

    // ════════════════════════════════════════════════════════════════
    // ── PERNOTE animation section  (visible only in PerNote mode)  ──
    // ── Modes: 0=Off  1=ReVoice  2=Cloud                           ──
    // ════════════════════════════════════════════════════════════════
    juce::Label        pnAnimModeLabel;
    juce::ComboBox     pnAnimModeCombo;
    juce::ToggleButton pnAnimSyncBtn;
    juce::Label        pnAnimSyncLabel;
    std::unique_ptr<BA>  pnAnimSyncAtt;
    juce::ToggleButton pnAnimUpToBtn;            // param: "pnanimuptovar"
    std::unique_ptr<BA>  pnAnimUpToAtt;

    juce::Label    pnAnimRateLabel;
    juce::ComboBox pnAnimRateCombo;
    juce::Slider   pnAnimRateSlider;
    std::unique_ptr<SA>  pnAnimRateSliderAtt;

    juce::Label    pnCloudDensityLabel, pnCloudOctaveLabel, pnCloudDecayLabel;
    juce::ComboBox pnCloudDensityCombo, pnCloudOctaveCombo;
    juce::Slider   pnCloudDecaySlider;
    std::unique_ptr<SA>  pnCloudDecayAtt;
    std::unique_ptr<CBA> pnCloudDensAtt, pnCloudOctaveAtt;

    juce::Label    pnCloudVelLabel, pnCloudVelDashLabel;
    juce::ComboBox pnCloudVelMinCombo, pnCloudVelMaxCombo;
    std::unique_ptr<CBA> pnCloudVelMinAtt, pnCloudVelMaxAtt;

    // ── PerNote Cluster params ────────────────────────────────────────────────────
    juce::Label    pnClusterLowNotesLabel, pnClusterHighNotesLabel;
    juce::Label    pnClusterLowOctLabel, pnClusterHighOctLabel;
    juce::Label    pnClusterLowOctSepLabel, pnClusterHighOctSepLabel;
    juce::ComboBox pnClusterLowNotesCombo, pnClusterHighNotesCombo;
    juce::ComboBox pnClusterLowOctStartCombo, pnClusterLowOctEndCombo;
    juce::ComboBox pnClusterHighOctStartCombo, pnClusterHighOctEndCombo;
    std::unique_ptr<CBA> pnClusterLowNotesAtt, pnClusterHighNotesAtt;
    std::unique_ptr<CBA> pnClusterLowOctStartAtt, pnClusterLowOctEndAtt;
    std::unique_ptr<CBA> pnClusterHighOctStartAtt, pnClusterHighOctEndAtt;

    juce::Label    pnClusterVelLabel, pnClusterVelDashLabel;
    juce::ComboBox pnClusterVelMinCombo, pnClusterVelMaxCombo;
    std::unique_ptr<CBA> pnClusterVelMinAtt, pnClusterVelMaxAtt;

    juce::Label  pnClusterDecayLabel;
    juce::Slider pnClusterDecaySlider;
    std::unique_ptr<SA>  pnClusterDecayAtt;

    // ── Cloud mute (shared across all three anim sections) ────────
    // Visible only when Cloud mode is active. Suppresses incoming/added
    // note passthroughs so only the cloud-generated notes are heard.
    juce::ToggleButton  cloudMuteInputBtn;
    juce::Label         cloudMuteInputLabel;
    std::unique_ptr<BA> cloudMuteInputAtt;
    ...
    void buildPerNoteTab();
    void buildPerNoteAnimSection();
    ...
    void updatePerNoteAnimUI();
    void updatePerNoteCustomVisibility(int pc);
    void updateFonts();

    void repopulateScaleRateCombo();
    void repopulateManualRateCombo();
    void repopulatePerNoteRateCombo();

    void rebuildPickerCombo();
    void doManualRandomize();
    void doPerNoteRandomize();
    void doPerNoteReset();
    void saveManualPreset();
    void loadManualPreset();
    void applyCustomString(int pc);
    ...
    static void populateClusterNotesCombo(juce::ComboBox&);
    static void populateClusterOctaveCombo(juce::ComboBox&);
