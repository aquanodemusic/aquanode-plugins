...
// Global control row (humanize + velocity sliders) – always visible
static constexpr int CTRL_ROW_H = 36;
static constexpr int CTRL_ROW_Y = HEADER_H;      // 52

// Tab content pane – shifted down by CTRL_ROW_H
static constexpr int TAB_Y = CTRL_ROW_Y + CTRL_ROW_H;   // 88
static constexpr int TAB_H = 298;

// Animation section below the tab pane
static constexpr int ANIM_Y = TAB_Y + TAB_H;        // 386
static constexpr int ANIM_LBL_H = 22;
static constexpr int ANIM_ROW_H = 34;
static constexpr int ANIM_ROW1_Y = ANIM_Y + ANIM_LBL_H + 2;   // 410
static constexpr int ANIM_ROW2_Y = ANIM_ROW1_Y + ANIM_ROW_H;  // 444
static constexpr int ANIM_ROW3_Y = ANIM_ROW2_Y + ANIM_ROW_H;  // 478
static constexpr int ANIM_ROW4_Y = ANIM_ROW3_Y + ANIM_ROW_H;  // 512
static constexpr int TOTAL_H = ANIM_ROW4_Y + ANIM_ROW_H + 10; // 556
...
static constexpr int MANUAL_ROWS_Y = TAB_Y + COL_HDR_OFFSET + COL_HDR_H;  // 110

// Scale tab internal layout (relative to TAB_Y)
static constexpr int SCALE_ROW1_Y = TAB_Y + 22;
static constexpr int SCALE_ROW2_Y = SCALE_ROW1_Y + 36;
static constexpr int SCALE_TOG1_Y = SCALE_ROW2_Y + 36;
static constexpr int SCALE_TOG2_Y = SCALE_TOG1_Y + 30;
static constexpr int SCALE_INV_Y = SCALE_TOG2_Y + 32;
static constexpr int SCALE_PREV_Y = SCALE_INV_Y + 38;
...
void NoteAdderEditor::paint(juce::Graphics& g)
{
    ...
    g.fillRect(PIANO_W, CTRL_ROW_Y, W, CTRL_ROW_H);
    g.setColour(SEP.withAlpha(0.5f));
    g.fillRect(PIANO_W + 10, CTRL_ROW_Y + CTRL_ROW_H - 1, W - 20, 1);
...
void NoteAdderEditor::resized()
{
    ...
    // ── Global control row: Humanize Time + Humanize Vel ─────
    {
        const int rowY = CTRL_ROW_Y;
        const int rowH = CTRL_ROW_H;
        const int cy = rowY + (rowH - 22) / 2;
        const int lblW = 64, sliderW = 190;
        int x = ox + margin;

        humanizeTimeLabel.setBounds(x, cy, 74, 20);               x += 74 + 4;
        humanizeTimeSlider.setBounds(x, cy - 2, 155, 24);         x += 155 + 20;
        humanizeVelLabel.setBounds(x, cy, 74, 20);                x += 74 + 4;
        humanizeVelSlider.setBounds(x, cy - 2, 155, 24);
    }

    // ── REC button: always visible in bottom-right corner ─────
    {
        const int recW = 46, recH = 28, margin2 = 10;
        recBtn.setBounds(PIANO_W + W - margin2 - recW,
            TOTAL_H - margin2 - recH, recW, recH);
    }
    ...
    // ── Scale tab ─────────────────────────────────────────────
    {
        ...
        // Toggle row 1
        {
            ...
            discardInputLabel.setBounds(x, ty, 155, 22); x += 155 + 12;
            bwColorsBtn.setBounds(x, ty, 82, 22);
        }
...

    {
        ...
        const int rateLblW = 38, rateCmbW = 120, rateSliderW = 200;
        ...
        scaleAnimRateSlider.setBounds(ox + margin + rateLblW + 4, cy2 - 1, rateSliderW, comboH + 2);
        ...
        manAnimRateSlider.setBounds(ox + margin + rateLblW + 4, cy2 - 1, rateSliderW, comboH + 2);
    }

    {
        ...
        // Scale – Cloud row 3
        {
            ...
            const int dcLblW = 44, dcSliderW = 140;
            int x = ox + margin;
            ...
            scaleCloudDecaySlider.setBounds(x, cy3 - 1, dcSliderW, comboH + 2);
        }
        ...
        // Manual – Cloud row 3
        {
            ...
            manCloudDecaySlider.setBounds(x, cy3 - 1, dcSliderW, comboH + 2);
        }
    }
