...
NoteAdderProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterInt>(
        "humanizevel", "Humanize Velocity", 0, 50, 0));
...
double NoteAdderProcessor::computeTickSamples() const
{
    ...
    // ReVoice (1), Wander (2), Drift (3) fire note-off + note-on in the same tick.
    // If the tick interval is shorter than one block, multiple ticks fire at
    // samplePos=0 causing stuck notes. Enforce a floor of (blockSize+1) samples
    // for these modes. Cloud (4) uses a separate decay queue so it is safe at any rate.
    const int curAnim = isScale ? animModeParam->get()
        : ((manualAnimModeParam->get() == 2) ? 4 : manualAnimModeParam->get());
    ...
    int samplePos, juce::MidiBuffer& out)
{
    ...
    const int decayMs = juce::roundToInt((isScale ? cloudDecayParam : manualCloudDecayParam)->get());
        ...
        else if (msg.isNoteOff())
        {
            ...
            pendingHumanizedNotes.erase(
                std::remove_if(pendingHumanizedNotes.begin(), pendingHumanizedNotes.end(),
                    ...
                    { return pn.midiChannel == msg.getChannel() && pn.inputNote == note; }),
