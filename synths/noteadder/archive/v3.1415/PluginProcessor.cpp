...
NoteAdderProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterInt>(
        "pnanimmode", "PerNote Anim Mode", 0, 2, 0));
    layout.add(std::make_unique<juce::AudioParameterBool>(
...
double NoteAdderProcessor::computeTickSamples() const
{
    ...
    else if (isPerNote) { const int m = pnAnimModeParam->get();     curAnim = (m == 2) ? 4 : m; }
...
std::vector<int> NoteAdderProcessor::computeAddedNotes(int note)
{
    ...
    if (modeParam->get() == 2)
    {
        ...
        {
            // Types 1-7: Maj, Maj7, Maj9, Min, Min7, Min9, 6/9
            int count = 0;
    ...
    juce::MidiBuffer& midi)
{
    ...
    else if (perNoteMode) { const int m = pnAnimModeParam->get();     curAnim = (m == 2) ? 4 : m; }
    else { const int m = manualAnimModeParam->get();    curAnim = (m == 2) ? 4 : m; }
