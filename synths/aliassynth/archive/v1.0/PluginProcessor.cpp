...
struct AliasVoice : public juce::SynthesiserVoice
{
    ...
    void renderNextBlock(juce::AudioBuffer<float>& buffer, int startSample, int numSamples) override
    {
        ...
        float baseFreq = juce::MidiMessage::getMidiNoteInHertz(getCurrentlyPlayingNote());
        ...
        for (int sample = 0; sample < numSamples; ++sample)
        {
            ...
            // --- Scheme Selection ---
            if (srMode) // 1. ORIGINAL DOWNSAMPLING SCHEME
            {
                if (sampleHoldCounter <= 0)
                {
                    // Generate full-resolution sample (Nyquist at max)
            ...
            else // 2. SPECTRAL FOLDING SCHEME
            {
