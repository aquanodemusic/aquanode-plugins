...
public:
    ...
    void renderNextBlock(juce::AudioBuffer<float>& buf, int start, int num) override {
        ...
        float f1Am = std::pow(2.0f, (float)o1octA + o1pitch / 12.0f);
        float f1Bm = std::pow(2.0f, (float)o1octB + o1pitch / 12.0f + o1detune / 1200.0f);
        float f2Am = std::pow(2.0f, (float)o2octA + o2pitch / 12.0f);
        float f2Bm = std::pow(2.0f, (float)o2octB + o2pitch / 12.0f + o2detune / 1200.0f);
