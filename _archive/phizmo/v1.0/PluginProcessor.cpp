    ...
    const BlockEnvCoeffs& bc, const SoundParams& SP)
{
    ...
    double tuneMA = std::pow(2.0, ((double)SP.tuneA + (double)SP.fineA * 0.01) / 12.0);
    double tuneMB = std::pow(2.0, ((double)SP.tuneB + (double)SP.fineB * 0.01) / 12.0);
    ...
    float rvA = rawEvoPosA + v.frameOffset + v.velFrameOffset + vPosLFO * SP.posLFODepth * 0.5f;
    rvA = std::fmod(rvA, 1.f); if (rvA < 0.f) rvA += 1.f;
    float rvB = rawEvoPosB + v.frameOffset + v.velFrameOffset + vPosLFO * SP.posLFODepth * 0.5f;
