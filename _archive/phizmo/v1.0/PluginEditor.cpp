...
void PhButton::paintButton(juce::Graphics& g, bool over, bool down)
{
    ...
    g.setColour(juce::Colours::white.withAlpha(0.92f));
    g.setFont(PhizmoLookAndFeel::panelFont());
    const float textW = g.getCurrentFont().getStringWidthFloat(getButtonText());
...
void PhizmoAudioProcessorEditor::resized()
{
    ...
    const float BW = 62.f, BH = 20.f;     // button cell — taller, for 12px text
    ...
    // OSC FILTER — now also carries F.ENV (filter-envelope depth), lifted out of
    // the ADVANCED row into the spare column on the right of this section.
    ...
    // ================= ROW 3 : y 256 .. 352 =================
    const float y3 = 256.f, ky3 = y3 + 18.f, mb3 = ky3 + KH + 2.f;
    x = 10.f;
...
void EvoCurveEditor::paint(juce::Graphics& g)
{
    ...
    // Playhead — rides the drawn curve at the raw scan phase for the focused
    // Voice. Because the dot uses the scan phase (its X) and reads the curve's
    // own value at that X (its Y), it is always exactly on the curve, however
    // much modulation is being applied to the frame the audio actually reads.
    const int slot = proc.getEditSound() * 2 + osc;
    float ph = proc.getCurrentScanPhase(slot);
