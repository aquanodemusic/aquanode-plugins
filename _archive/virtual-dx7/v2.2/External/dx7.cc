...
DX7::DX7(ToSynth*& ts, ToGui*& tg, const char* rf)
	...
	loadFirmwareFromMemory();
	cartFile.reserve(256);
}

void DX7::loadFirmwareFromMemory() {
	if (!vdx7_firmware_rom) return;
	const uint8_t* rom = vdx7_firmware_rom;
...
void DX7::setBank(int n, bool cart) {
	...
	const uint8_t *ram = vdx7_factory_voices + 4096*(n&0x7);
