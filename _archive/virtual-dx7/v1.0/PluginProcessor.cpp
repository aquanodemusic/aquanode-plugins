...
#include "BinaryData.h"       // embedded Firmware/*.bin (generated by the build)

// ============================================================================
//  Firmware / factory-voice / example-RAM data, embedded from Firmware/*.bin.
//  The emulator (External/dx7.cc) reads the first two through these pointers.
// ============================================================================
const uint8_t* vdx7_firmware_rom   = reinterpret_cast<const uint8_t*> (BinaryData::dx7_bin);
const uint8_t* vdx7_factory_voices = reinterpret_cast<const uint8_t*> (BinaryData::voices_bin);
static const uint8_t* vdx7_example_ram =
    reinterpret_cast<const uint8_t*> (BinaryData::example_ram);
...
Bank factoryBank (int bank) {
    ...
    return Bank (vdx7_factory_voices + (size_t) bank * 4096);
}
...
DX7Engine::DX7Engine() {
    ...
    synth_   = new DX7Synth (nullptr);   // no ram file; we inject RAM ourselves
    synth_->toGui   = toGui_;
...
void DX7Engine::bootThreadFn (double fs) {
    ...
    synth_->start();
    std::memcpy (&synth_->dx7.memory[0x1000], vdx7_example_ram, 6144);  // known-good battery RAM
...
void DX7Engine::pushMidi (const uint8_t* bytes, int len) {
    if (! isReady() || len <= 0) return;
    synth_->queueMidiRx ((uint32_t) len, bytes);
}

void DX7Engine::renderMono (float* out, int n) {
    if (! isReady()) { std::memset (out, 0, sizeof (float) * (size_t) n); return; }
    drainCommands();
    int i = 0;
    while (i < n) {
        if (chunkPos_ >= kChunk) {
...
{
    buildAutoParams();
    currentVoice_ = vdx7::factoryBank (0).voice (0);   // ROM1A #1 (BRASS 1)
    bank_ = 0; prog_ = 0;
