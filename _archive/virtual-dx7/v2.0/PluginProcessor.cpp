...
//
//  Nothing is embedded. The emulator (External/dx7.cc) reads the ROM through
//  these two globals; RomStore owns the bytes and points them here whenever the
//  user loads or clears a file. Both are null until then, which is exactly the
//  condition that makes DX7Engine run the native engine instead.
// ============================================================================
const uint8_t* vdx7_firmware_rom   = nullptr;
const uint8_t* vdx7_factory_voices = nullptr;

void vdx7::RomStore::installPointers()
{
    vdx7_firmware_rom   = extern_firmware_;
    vdx7_factory_voices = extern_voices_;
}
...
bool usingStarterBank() { return ! RomStore::get().hasVoices(); }

// The bundled starter bank, packed once on first use.
static const Bank& starterBank()
{
    static const Bank b = []
    {
        Bank tmp;
...
Bank factoryBank (int bank) {
    ...
    Bank out;
    if (RomStore::get().copyBank (bank, out.packed()))
        return out;
...
DX7Engine::DX7Engine() {
    ...
    // library absent this hands over an empty function and changes nothing.
    if (auto p = mts_.provider())
        native_.tuning().setFrequencyProvider (std::move (p));
    emulatorWanted_.store (RomStore::get().hasFirmware(), std::memory_order_release);
    updateNativeDisplay();
}
...
void DX7Engine::romsChanged() {
    const bool haveFirmware = RomStore::get().hasFirmware();
...
void DX7Engine::bootThreadFn (double fs) {
    auto runSecs = [&](double s){ int n = (int)(s * fs / kChunk); for (int i=0;i<n;i++) synth_->run(); };
    ...
    synth_->dx7.loadFirmwareFromMemory();
    synth_->start();
    ...
    // on the way up.
    {
        ...
        std::memcpy (fallback, factoryBank (0).packed(), 4096);
        const auto ram = RomStore::get().buildBatteryRam (fallback);
      ...
      apvts (*this, nullptr, "VDX7", createLayout())
{
    ...
    // On desktop, pick up whatever ROM was last used so a new instance is ready
    // without asking again. On Android this is a no-op: the picker returns a
    // document URI whose permission does not reliably outlive the app, so the
    // user is asked once per session instead.
    auto& roms = vdx7::RomStore::get();
    if (! roms.hasFirmware())
        roms.tryLoadFromPath (vdx7::RomStore::recallPath (vdx7::kRomPathKey), true);
    if (! roms.hasVoices())
        roms.tryLoadFromPath (vdx7::RomStore::recallPath (vdx7::kVoicesPathKey), false);

    romPath_    = roms.firmwareSource();
    voicesPath_ = roms.voicesSource();

    currentVoice_ = vdx7::factoryBank (0).voice (0);   // first patch of bank 1
    bank_ = 0; prog_ = 0;
...
juce::AudioProcessorValueTreeState::ParameterLayout VDX7AudioProcessor::createLayout()
{
    ...
    const Voice def = factoryBank (0).voice (0);       // sensible defaults (ROM1A #1)
...
{
    if (! vdx7::RomStore::get().loadFirmware (data, size, source, errorOut))
        return false;

    romPath_ = source;
   #if ! JUCE_ANDROID
    vdx7::RomStore::rememberPath (vdx7::kRomPathKey, source);
   #endif
    afterRomChange();
    return true;
}
...
bool VDX7AudioProcessor::loadFactoryVoices (const void* data, size_t size,
                                            ...
                                            juce::String& errorOut)
{
    if (! vdx7::RomStore::get().loadVoices (data, size, source, errorOut))
        return false;

    voicesPath_ = source;
   #if ! JUCE_ANDROID
    vdx7::RomStore::rememberPath (vdx7::kVoicesPathKey, source);
   #endif
    afterRomChange();
    return true;
}
...
{
    vdx7::RomStore::get().clearFirmware();
    vdx7::RomStore::get().clearVoices();
    romPath_.clear();
    voicesPath_.clear();
   #if ! JUCE_ANDROID
    vdx7::RomStore::rememberPath (vdx7::kRomPathKey,    {});
    vdx7::RomStore::rememberPath (vdx7::kVoicesPathKey, {});
   #endif
    afterRomChange();
}
...
void VDX7AudioProcessor::afterRomChange()
{
    ...
    if (! userActive_)
        currentVoice_ = vdx7::factoryBank (bank_).voice (prog_);

    loadVoiceIntoModel (currentVoice_);
}
...
juce::String VDX7AudioProcessor::engineDescription() const
{
    ...
    if (engine_.mtsEspActive())              s += " - MTS-ESP";
...
{
    vdx7::Bank b = (userActive_ && hasUserBank_) ? userBank_ : vdx7::factoryBank (bank_);
...
void VDX7AudioProcessor::selectFactory (int bank, int prog)
{
    ...
    userActive_ = false;
    loadVoiceIntoModel (vdx7::factoryBank (bank_).voice (prog_));
}
    ...
    if (auto xml = getXmlFromBinary (data, sizeInBytes))
    {
        ...
        if (vt.isValid() && vt.hasType ("VDX7STATE"))
        {
            ...
            if (vced.getSize() >= (size_t) vdx7::kVcedSize)
            {
                ...
                // Re-open the ROMs this project was saved with, if they are
                // still where they were. Failing is not an error: the plugin
                // simply keeps running the native engine, and the user can
                // point it at the files again from the ROM menu.
                {
                    auto& roms = vdx7::RomStore::get();
                    ...
                    bool changed = false;
                    if (rp.isNotEmpty() && ! roms.hasFirmware())
                        changed |= roms.tryLoadFromPath (rp, true);
                    if (vp.isNotEmpty() && ! roms.hasVoices())
                        changed |= roms.tryLoadFromPath (vp, false);
