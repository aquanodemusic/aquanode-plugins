...
// Access to the 8 factory banks. When the user has loaded a factory voice ROM
// these are its ROM1A..ROM4B; otherwise there is a single bundled starter bank
// of original patches, repeated so the selector always has something to offer.
Bank        factoryBank (int bank);
const char* factoryBankName (int bank);
bool        usingStarterBank();          // true when no voice ROM is loaded
