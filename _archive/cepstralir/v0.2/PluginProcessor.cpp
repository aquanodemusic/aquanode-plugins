...
bool CepstralIRProcessor::reprocessIR()
{
    ...
    if (linearPhaseParam->get())
    {
        ...
        // intelligentLength is the full width (diameter) needed, capped at 10000 samples (~227ms at 44.1kHz)
        intelligentLength = std::min(10000, foundRadius * 2 + 512);  // Add small buffer
        intelligentLength = std::max(intelligentLength, 1024);  // Minimum size
    }
    else
    {
