...
private:
    ...
    void setTooltip()
    {
        juce::Component::setName ("User pattern: click to toggle a step, drag up or down for velocity");
    }
...
private:
    ...
    {
        const int headerHeight = framed ? 16 : 0;
...
public:
    ...
    void resized() override
    {
        ...
        if (hasOwnSelector)
        {
            ...
            selector.setBounds (header.withTrimmedLeft (juce::jmax (60, header.getWidth() / 3)));
            r.removeFromTop (4);
        }
        else
        {
            r.removeFromTop (18);   // just the title, drawn by paint()
        }
    ...
    int preferredHeightForWidth (int width)
    {
        ...
        return tallest + (hasOwnSelector ? 26 : 22);
    }
