...
void FormantFilterModule::renderLane (int lane, const StereoFrame* inputs, StereoFrame* outputs)
{
    ...
    // Poles and Slope together set how steep the skirt around each formant
    // is. Poles is the whole number of cascaded stages; Slope crossfades
    // between one stage fewer and the full count, so a sweep through the
    // control is continuous rather than stepping between filter orders.
    const int poles = juce::jlimit (1, kMaxPoles, (int) param (pPoles));
    const float slope = juce::jlimit (0.0f, 1.0f, param (pSlope));
    ...
    for (int ch = 0; ch < 2; ++ch)
    {
        ...
        // Shape tilts this side's formants against the other's: one channel
        // can sit a little brighter or lower than its partner, which is what
        // gives the bank its width instead of the same filter twice.
        const float shape = ch == 0 ? param (pShapeL) : param (pShapeR);
        const float shapeScale = std::pow (2.0f, shape * 0.5f);

        for (int f = 0; f < kFormants; ++f)
        {
            const float centre = (kFreq[i0][f] * (1.0f - t) + kFreq[i1][f] * t)
            ...
            // cascade: each stage filters the one before it
            float signal = x;
            float previousStage = 0.0f;

            for (int p = 0; p < poles; ++p)
            {
                low[lane][ch][f][p] += fq * band[lane][ch][f][p];
                const float hp = signal - low[lane][ch][f][p] - band[lane][ch][f][p] * damping;
                band[lane][ch][f][p] += fq * hp;

                previousStage = signal;
                signal = band[lane][ch][f][p];
            }

            // crossfade the last stage in, so Slope is continuous
            const float staged = poles > 1 ? previousStage * (1.0f - slope) + signal * slope
                                           : signal;

            // Each extra pole narrows the peak but also stacks gain at the
            // centre; without this a four-pole vowel filter is a +12 dB
            // stage and anything using it clips.
            sum += staged * lvl / std::sqrt ((float) poles);
        }
...
static ModuleDescriptor formantFilterDescriptor()
{
    ...
    d.params = {
        ...
        makeRotary ("poles",     "Poles",     1.0f, (float) FormantFilterModule::kMaxPoles, 1.0f, 2, {}, false, 1.0f),
        makeRotary ("slope",     "Slope",     0.0f, 1.0f, 1.0f, 2),
        makeRotary ("shapeL",    "Shape L",   -1.0f, 1.0f, 0.0f, 2),
        makeRotary ("shapeR",    "Shape R",   -1.0f, 1.0f, 0.0f, 2)
    };
    return d;
}
