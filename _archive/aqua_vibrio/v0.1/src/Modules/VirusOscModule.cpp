...
void VirusOscModule::voiceNoteOn (int voice, int note, bool retrigger)
{
    ...
    for (int u = 0; u < kMaxUnison; ++u)
        phase[voice][u] = initPhase > 0.0f
                            ? (double) initPhase
                            : (double) u / (double) kMaxUnison;   // fan the stack out
...
//             pitch following it
//
float VirusOscModule::renderPartial (int wave, double p, float shape, float pulseWidth,
                                     float formant, double frequency)
{
    ...
    const float wavePart = table.read (wave, formantPhase, frequency, sampleRate);
    ...
    const float saw = (float) (2.0 * p - 1.0) - blep (p, dt);
    ...
    const float saw2 = (float) (2.0 * shifted - 1.0) - blep (shifted, dt);
...
void VirusOscModule::processVoiceSample (int v, const StereoFrame* inputs, StereoFrame* outputs)
{
    ...
    const int wave = juce::jlimit (0, VirusWaveTable::kNumWaves - 1, (int) param (pWave));
    ...
    if (syncEnabled || param (pSyncOut) > 0.5f)
    {
        ...
        const double syncRatio = juce::jmax (0.05,
            (double) param (pSyncRatio) * (1.0 + (double) param (pSyncEnv) * (double) envIn));
...
static ModuleDescriptor virusOscDescriptor()
{
    ...
    d.params = {
        ...
        makeRotary ("syncRatio", "Sync Freq", 0.25f, 8.0f, 1.0f, 3, {}, true),
