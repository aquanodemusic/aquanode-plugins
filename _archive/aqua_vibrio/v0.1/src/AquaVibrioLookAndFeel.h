...
{
    // chassis
    const juce::Colour background      { 0xff11201a };
    const juce::Colour panelTop        { 0xff1d3329 };
    const juce::Colour panelBottom     { 0xff152620 };
    const juce::Colour panelEdge       { 0xff0b1611 };
    const juce::Colour groupTop        { 0xff23402f };
    const juce::Colour groupBottom     { 0xff182d22 };

    // green family
    const juce::Colour accent          { 0xff3fd98a };   // live values, arcs
    const juce::Colour accentDim       { 0xff2a8a5b };
    const juce::Colour accentGlow      { 0xff7dffbc };
    const juce::Colour headerBar       { 0xff2f7f57 };

    // ink
    const juce::Colour textBright      { 0xffe6f5ec };
    const juce::Colour textLabel       { 0xff9bbcab };
    const juce::Colour textDim         { 0xff6e8c7e };

    // controls
    const juce::Colour trackEmpty      { 0xff0e1d16 };
    const juce::Colour knobFace        { 0xff1a2f25 };
    const juce::Colour comboFill       { 0xff1f3a2c };
    const juce::Colour comboBorder     { 0xff37604a };
    ...
    inline juce::Colour sectionTint (const juce::String& region)
    {
        ...
        if (region.startsWith ("osc") || region == "unison" || region == "noise")
            return juce::Colour { 0xff35c9a2 };
        if (region.startsWith ("filter") || region.startsWith ("env"))
            return juce::Colour { 0xff3fd98a };
        if (region.startsWith ("lfo") || region.startsWith ("assign") || region == "modmatrix")
            return juce::Colour { 0xff6ede73 };
        return juce::Colour { 0xff8fd96a };  // effects
    }
}
...
public:
                                  ...
                                  const juce::String& title, juce::Colour tint)
    {
        ...
        if (title.isNotEmpty())
        {
            auto header = r.removeFromTop (18.0f);
            juce::Path p;
