...
{
    // Ensure drawCache_ is fully up to date before we snapshot it.
    if (cacheDirty_ || drawCache_.getWidth() != getWidth() || drawCache_.getHeight() != getHeight())
        rebuildCache();

    // Store a copy of the current canvas as the committed snapshot.
    // Using a plain ARGB juce::Image keeps decode overhead at zero on paint.
    // If memory matters this can be PNG-compressed with juce::PNGImageFormat --
    // just add encode here and decode in rebuildCache().
    committedImage_ = drawCache_.createCopy();

    // Clear raw data-point history -- it's fully baked now.
    stampHistory_.clear();
    interpPaths_.clear();
}
...
// rebuildCache
//
//  Reconstructs drawCache_ from scratch.  Called on resize, clear, or when
//  cacheDirty_ is set.
//
//  If committedImage_ exists (always true after the first commit) we start by
//  scaling it to the current canvas size.  This means resize just stretches
//  the baked image rather than replaying hundreds of data points -- O(pixels)
//  instead of O(events).
//
//  Any uncommitted history (stampHistory_ / interpPaths_) is replayed on top.
//  In normal use this will be empty right after a commit.
//------------------------------------------------------------------------------
void DrawingCanvas::rebuildCache()
{
    ...
    // --- Base layer: scale committed snapshot to current size ---
    ...
    // --- Uncommitted dot events (single Graphics context for speed) ---
    ...
    // --- Uncommitted interpolated paths ---
    ...
    bool blurEnabled, float blurSecs) const noexcept
{
    ...
    // Blur tail (active when blurSecs > 0)
...
void DrawingCanvas::addDrawPoint(int x, int y)
{
    ...
    // Visual: one Graphics context for all harmonics
    {
        const int W = getWidth(), H = getHeight();
        if (drawCache_.getWidth() != W || drawCache_.getHeight() != H || !drawCache_.isValid())
            drawCache_ = juce::Image(juce::Image::ARGB, W, H, true);
...
/** On mouseUp:
 *    DRAW mode:   commit buffered audio events to the processor.
 *    INTERP mode: commit the path to the processor.
 *
 *  In both cases, after the audio is handed off, commitCurrentImage() is
 *  called to bake the visual into committedImage_ and clear the raw data lists.
 *  The processor's triggerBackgroundRender() then synthesises only the new
 *  content and mixes it into the committed audio buffer.
 */
void DrawingCanvas::mouseUp(const juce::MouseEvent&)
{
    ...
    if (interpMode_)
    {
        if (currentInterpStroke_.size() >= 2)
        {
            ...
            // Store visual record (replayed in rebuildCache if needed)
            interpPaths_.push_back({ currentInterpStroke_, nh, showBlur_, blurSecs_ });
            cacheDirty_ = true;  // rebuildCache will draw the polylines
    ...
    // Kick off the incremental audio render BEFORE baking the image,
    // so the render thread and the image snapshot run concurrently.
    if (onStrokeFinished) onStrokeFinished();

    // Bake visual state → committedImage_ and clear raw data lists.
    // Must happen after the stroke data is handed to the processor.
    commitCurrentImage();
}
    ...
    : AudioProcessorEditor(&p), audioProcessor_(p), canvas_(p)
{
    ...
    //--------------------------------------------------------------------------
    // Blur strength slider (0 = no blur; replaces the old Blur toggle button)
    //--------------------------------------------------------------------------
    ...
    sldBlurStrength.setRange(0.0, 10.0, 0.05);   // 0 means no blur
    ...
    sldBlurStrength.onValueChange = [this]
        {
            ...
            // Blur is active whenever the slider is above zero
    ...
    // Incremental render: only the new stroke is synthesised and mixed in
...
//------------------------------------------------------------------------------
// resized -- single toolbar row, all controls in one line
//------------------------------------------------------------------------------
void FrequencyDrawerAudioProcessorEditor::resized()
{
    auto area = getLocalBounds();

    // Single 30-pixel toolbar row (was two rows at 62px total)
