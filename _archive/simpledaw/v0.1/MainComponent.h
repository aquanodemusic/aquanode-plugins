...
public:
    ...
    {
        if (step < 0 || step >= 128) return 0.5f;
...
struct Note
{
    ...
    int startStep;     // Starting 16th note position
    int length;        // Length in 16th notes
...
public:
    PianoRollComponent()
    {
        // Initialize with 8 bars, full MIDI range (128 notes, C-2 to G8)
        numBars = 8;
        numNotes = 128;
        cellWidth = 30;
        cellHeight = 20;
    ...
    void resized() override
    {
        ...
        int contentWidth = labelWidth + (numBars * 16 * cellWidth);
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        if (!clickedAutomation)
        {
            if (e.mods.isRightButtonDown())
            {
                ...
                if (col >= 0 && col < numBars * 16 && row >= 0 && row < numNotes)
                {
            ...
            else
            {
                ...
                if (dragStartCol >= 0 && dragStartCol < numBars * 16 &&
    ...
    void mouseDrag(const juce::MouseEvent& e) override
    {
        ...
        if (isEditingAutomation && selectedAutomationLane >= 0 && selectedAutomationLane < 3)
        {
            ...
            int col = (pos.x - labelWidth) / cellWidth;
            if (col >= 0 && col < numBars * 16 && col != lastAutomationCol)
            {
                float normalizedY = static_cast<float>(pos.y - laneY) / automationLaneHeight;
...
private:
    ...
    public:
        ...
        void paint(juce::Graphics& g) override
        {
            ...
            // Draw vertical lines (16th notes)
            g.setColour(juce::Colours::darkgrey);
            for (int i = 0; i <= pianoRoll.numBars * 16; ++i)
            {
            ...
            for (int laneIdx = 0; laneIdx < 3; ++laneIdx)
            {
                ...
                g.fillRect(gridStartX, laneY, pianoRoll.numBars * 16 * pianoRoll.cellWidth, automationLaneHeight);

                // Draw lane border
                g.setColour(juce::Colours::grey);
                g.drawRect((float)gridStartX, (float)laneY, (float)pianoRoll.numBars * 16 * pianoRoll.cellWidth, (float)automationLaneHeight, 1.0f);
                ...
                for (int step = 0; step <= pianoRoll.numBars * 16; ++step)
                {
                ...
                g.drawLine(gridStartX, laneY + automationLaneHeight / 2,
                    gridStartX + pianoRoll.numBars * 16 * pianoRoll.cellWidth,
                ...
                for (const auto& pair : stepValues)
                {
                    ...
                    if (step >= 0 && step < pianoRoll.numBars * 16)
                    {
                        int x = gridStartX + step * pianoRoll.cellWidth + pianoRoll.cellWidth / 2;
                        int y = laneY + automationLaneHeight - static_cast<int>(value * automationLaneHeight);
...
public:
        ...
        const juce::AudioIODeviceCallbackContext& context) override
    {
        ...
        double sixteenthNotesPerSecond = beatsPerSecond * 4.0;
        int samplesPerStep = static_cast<int>(sampleRate / sixteenthNotesPerSecond);
...
private:
    ...
    void loadPluginFromDescription(const juce::PluginDescription& desc, bool isInstrument,
        int trackOrSlot, const juce::String& pluginStateBase64 = juce::String());
    ...
    {
    public:
        ParameterChangeListener(MainComponent& owner) : mainComponent(owner) {}

        void addPlugin(juce::AudioPluginInstance* plugin)
        {
            trackedPlugins.insert(plugin);
        }

        void removePlugin(juce::AudioPluginInstance* plugin)
        {
            trackedPlugins.erase(plugin);
        }

        void parameterValueChanged(int parameterIndex, float newValue) override
        {
            // Search through tracked plugins to find which one has this parameter
            for (auto* plugin : trackedPlugins)
            {
                if (plugin != nullptr)
                {
    ...
    private:
        ...
        std::set<juce::AudioPluginInstance*> trackedPlugins;
    };

    std::unique_ptr<ParameterChangeListener> parameterListener;
    ...
    // Color customization
    juce::Colour backgroundColor = juce::Colours::grey;
