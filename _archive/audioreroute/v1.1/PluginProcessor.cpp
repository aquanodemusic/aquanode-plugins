...
AudioRerouterPluginAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterBool>("pdc", "PDC", true));
    ...
    juce::MidiBuffer& midiMessages)
{
    ...
    else
    {
        ...
        // ── Latency reporting ──────────────────────────────────────────────
        // Only report latency when the user has PDC enabled.  When PDC is off
        // (e.g. intentional feedback loops) we always report 0 so the host
        // does not try to compensate for the structural ring-buffer delay.
        constexpr float kSignalFloor = 1.0e-4f; // ≈ -80 dBFS

        const bool pdcEnabled = apvts.getRawParameterValue("pdc")->load() > 0.5f;

        const bool activeSignal = pdcEnabled
            && channelHasTransmitter[channel].load()
        ...
        if (latencyToReport != lastReportedLatency || pdcEnabled != lastPdcEnabled)
        {
            setLatencySamples(latencyToReport);
            lastReportedLatency = latencyToReport;
            lastPdcEnabled = pdcEnabled;
        }
