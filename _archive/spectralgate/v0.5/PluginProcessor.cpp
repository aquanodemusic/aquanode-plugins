...
void SpectralGateAudioProcessor::performSpectralGate(int channel)
{
    ...
    float lowerBinLog = lowerFreqBin.load();
    float upperBinLog = upperFreqBin.load();
    ...
    // Using dB scale: -60 dB to +6 dB (66 dB range for stronger gating)
    float thresholdMagnitude = std::pow(10.0f, (magThreshold * 66.0f - 60.0f) / 20.0f);
