    ...
    : AudioProcessorEditor(&p), proc(p), pianoRoll(p)
{
    ...
    scaleTranspSlider.setTextValueSuffix(" steps");
...
void NoteAdderEditor::resized()
{
    ...
    // ── Global control row 2: Scale Transpose (scale mode only) + Global Transpose ──
    {
        ...
        // Scale Transp. sits left; Global Transpose sits at a fixed offset after it.
        // When Scale Transp. is hidden the Global Transpose stays at the same fixed
        // position so its layout is predictable regardless of mode.
        const int stLblW = juce::roundToInt(80 * L.sf);
        const int stSlW = juce::roundToInt(140 * L.sf);
        const int gtLblW = juce::roundToInt(74 * L.sf);
        const int gtSlW = juce::roundToInt(155 * L.sf);

        int x = ox + margin;
        scaleTranspLabel.setBounds(x, cy, stLblW, juce::roundToInt(20 * L.sf)); x += stLblW + lhGap;
        scaleTranspSlider.setBounds(x, cy - 2, stSlW, slH);
        x += stSlW + secGap;

        // Global Transpose always at this fixed X regardless of Scale Transp visibility
        globalTransposeLabel.setBounds(x, cy, gtLblW, juce::roundToInt(20 * L.sf)); x += gtLblW + lhGap;
        globalTransposeSlider.setBounds(x, cy - 2, gtSlW, slH);
    }
    ...
    // ── Scale tab ─────────────────────────────────────────────
    {
        ...
        //   Col3: bwColorsBtn (row1) / resetColorsBtn (row2)
        {
            ...
            // Toggle row 2
            {
                ...
                // randomSkipBtn+label fills from margin to col2 with a small gap
                const int skipLblW = col2X - (ox + margin) - juce::roundToInt(10 * L.sf) - toggleW - 3;
                int x = ox + margin;
                randomSkipBtn.setBounds(x, ty, toggleW, 22); x += toggleW + 3;
                randomSkipLabel.setBounds(x, ty, skipLblW, 22);
                x = col2X;
                lockBassBtn.setBounds(x, ty, toggleW, 22); x += toggleW + 3;
                lockBassLabel.setBounds(x, ty, tLblW, 22);
