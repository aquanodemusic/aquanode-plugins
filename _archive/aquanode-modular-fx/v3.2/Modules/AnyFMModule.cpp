...
void AnyFMModule::processSample (const StereoFrame* inputs, StereoFrame* outputs)
{
    ...
    // Env In scales the modulation depth (0..1, e.g. from an ADSR)
    const float envIn = isInputConnected (2) ? juce::jlimit (0.0f, 1.0f, inputs[2][0]) : 1.0f;
    const float target = param (pDepth) * envIn;
...
static ModuleDescriptor anyFMDescriptor()
{
    ...
    d.displayName = "FM";
    d.description =
        "FM as effect. Audio-rate cross-modulation between two arbitrary signals: Phase Mod is "
        "true through-zero PM, Ring Mod multiplies, and Feedback feeds the output back as its own "
        "modulator. Carrier In and Mod In both want audio - any two oscillators will do. Note the "
        "wet path of Phase Mod carries about 23 ms of latency from its delay centre.";
