...
static ModuleDescriptor pitchEQDescriptor()
{
    ...
    d.description =
        "Pick note classes on the little keyboard and every note in the Lo..Hi range that is NOT "
        "selected gets dampened, while selected ones can be boosted - the signal is musically "
        "re-tuned toward the chosen pitch classes. With no keys selected it passes through. "
        "Modulation is the other way round from what you would expect here: Dampen, Width and "
        "Boost refuse mod cables (they'd rebuild all 128 filters), but the KEYS accept them - "
        "drop a cable on a key and an LFO or DAW Mod switches that note class on and off.";
