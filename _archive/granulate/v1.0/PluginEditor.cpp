...
void WaveformDisplay::paint(juce::Graphics& g)
{
    ...
    // Draw waveform with reduced sample count for cleaner look
    ...
    if (numSamples > 0)
    {
        ...
        const auto& grainVoices = processor.getGrainVoices();
        
        for (const auto& voice : grainVoices)
        {
            if (voice.isActive)
            {
                float normalizedPos = voice.samplePosition / (float)numSamples;
                int playheadX = bounds.getX() + (int)(normalizedPos * bounds.getWidth());
                
                float brightness = 1.0f - voice.grainEnvelopePhase;
