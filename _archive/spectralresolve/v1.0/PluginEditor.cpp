    ...
    : AudioProcessorEditor (&p), processor (p)
{
    ...
    // Initialise temporal interpolation buffers (all rows silent)
...
//==============================================================================
// Convert a SpectralFrame to a per-pixel-row peak magnitude array.
// Identical frequency mapping to paintFrameBatch — ignores time offsets since
// the smoothing pass works in pixel space.
//==============================================================================
std::vector<float> SpectralResolverEditor::frameToPixelMags (const SpectralFrame& frame,
                                                               float freqLow, float freqHigh) const
{
    std::vector<float> pixMags (IMG_H, -200.f);
    if (freqLow >= freqHigh) return pixMags;
    ...
    for (size_t i = 0; i < frame.freqHz.size(); ++i)
    {
        ...
        const int   row  = juce::jlimit (0, IMG_H - 1, (int)((1.f - norm) * (IMG_H - 1)));
        pixMags[row] = std::max (pixMags[row], frame.magDB[i]);
    }
    return pixMags;
}
                              ...
                              juce::Colour         gradHigh)
{
    ...
    for (size_t i = 0; i < frame.freqHz.size(); ++i)
    {
        ...
        colMags[batchCol][row] = std::max (colMags[batchCol][row], frame.magDB[i]);
    }
    ...
    for (int c = 0; c < speed; ++c)
    {
        ...
        for (int y = 0; y < imgH; ++y)
        {
            ...
            img.setPixelAt (imgCol, y, juce::Colour (
...
//==============================================================================
// Paint one column of the image from a per-row magnitude array.
//==============================================================================
                                ...
                                juce::Colour              gradHigh)
{
    ...
    for (int y = 0; y < imgH; ++y)
    {
        ...
        img.setPixelAt (col, y, juce::Colour (
            (uint8_t)(gradLow.getRed()   + t * (gradHigh.getRed()   - gradLow.getRed())),
            (uint8_t)(gradLow.getGreen() + t * (gradHigh.getGreen() - gradLow.getGreen())),
            (uint8_t)(gradLow.getBlue()  + t * (gradHigh.getBlue()  - gradLow.getBlue()))
        ));
...
void SpectralResolverEditor::timerCallback()
{
    ...
    if (mainActive)
    {
        if (smoothOn)
        {
            ...
            // pixel-row mags and the current frame's, one unique blend per column.
            ...
            auto currMags = frameToPixelMags (lastFrame, freqLow, freqHigh);
            std::vector<float> colMags (IMG_H);
            ...
            for (int c = 0; c < speed; ++c)
            {
                ...
                for (int y = 0; y < IMG_H; ++y)
                {
                    ...
                    if (p <= -199.f && q <= -199.f) { colMags[y] = -200.f; continue; }
                    const float pp = (p <= -199.f) ? q : p;
                    const float qq = (q <= -199.f) ? p : q;
                    colMags[y] = pp + tc * (qq - pp);
                }
                paintSingleColumn (spectroImage, colMags, IMG_W - speed + c, threshDB, gradLow, gradHigh);
            }
    ...
    if (scActive)
    {
        if (smoothOn)
        {
            auto currMags = frameToPixelMags (scLastFrame, freqLow, freqHigh);
            std::vector<float> colMags (IMG_H);
            ...
            for (int c = 0; c < speed; ++c)
            {
                ...
                paintSingleColumn (spectroImage, colMags, IMG_W - speed + c, threshDB, gradHigh, gradLow);
            }
