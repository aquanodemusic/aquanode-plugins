    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    makeSliderLabel(gateThreshLabel, "Threshold");
    ...
    makeSliderLabel(enhLevelLabel, "Level");
    ...
    styleValueSlider(gateThresholdSlider);
    gateThresholdSlider.setColour(juce::Slider::thumbColourId, juce::Colour(0xffdd4444));
    gateThresholdSlider.setColour(juce::Slider::trackColourId, juce::Colour(0xffdd4444).withAlpha(0.4f));
    gateThresholdSlider.setTooltip("Gate threshold (dB relative to peak)");
    ...
    styleValueSlider(enhanceLevelSlider);
    enhanceLevelSlider.setColour(juce::Slider::thumbColourId, juce::Colour(0xff44aa66));
    enhanceLevelSlider.setColour(juce::Slider::trackColourId, juce::Colour(0xff44aa66).withAlpha(0.4f));
    enhanceLevelSlider.setTooltip("Enhance level (dB relative to peak)");
    addAndMakeVisible(enhanceLevelSlider);
    ...
    styleButton(drawTargetButton);
    drawTargetButton.setButtonText("Draw Main");
    drawTargetButton.setClickingTogglesState(true);
    drawTargetButton.setToggleState(false, juce::dontSendNotification);
    drawTargetButton.setColour(juce::TextButton::buttonOnColourId, audioProcessor.getSidechainSpectrumColor().darker(0.2f));
    drawTargetButton.setColour(juce::TextButton::textColourOnId,
        audioProcessor.getSidechainSpectrumColor().darker(0.2f).getPerceivedBrightness() > 0.55f
        ? juce::Colours::black : juce::Colours::white);
    drawTargetButton.setColour(juce::TextButton::buttonColourId, audioProcessor.getMainSpectrumColor().darker(0.5f));
    drawTargetButton.setColour(juce::TextButton::textColourOffId, audioProcessor.getMainSpectrumColor());
    drawTargetButton.setTooltip("Toggle which channel the spectral filter curve is drawn on");
    drawTargetButton.onClick = [this]() {
        drawingMain = !drawTargetButton.getToggleState();
        drawTargetButton.setButtonText(drawingMain ? "Draw Main" : "Draw Side");
        };
    addAndMakeVisible(drawTargetButton);

    styleButton(resetDrawsButton);
    resetDrawsButton.setButtonText("Reset Draws");
    resetDrawsButton.setTooltip("Reset both spectral filter curves to flat (0 dB)");
    ...
    gateThresholdAttachment = std::make_unique<SliderAttachment>(audioProcessor.apvts, "gateThreshold", gateThresholdSlider);
    ...
    enhanceLevelAttachment = std::make_unique<SliderAttachment>(audioProcessor.apvts, "enhanceLevel", enhanceLevelSlider);
...
SpectralCompareAudioProcessorEditor::~SpectralCompareAudioProcessorEditor()
{
    ...
    gateEnableAttachment.reset(); gateThresholdAttachment.reset();
    gateBinStartAttachment.reset(); gateBinEndAttachment.reset();
    enhanceEnableAttachment.reset(); enhanceModeAttachment.reset();
    enhanceLevelAttachment.reset(); enhanceBinStartAttachment.reset(); enhanceBinEndAttachment.reset();
...
void SpectralCompareAudioProcessorEditor::refreshUIColors()
{
    ...
    drawTargetButton.setColour(juce::TextButton::buttonOnColourId, sideCol.darker(0.15f));
    drawTargetButton.setColour(juce::TextButton::textColourOnId,
        sideCol.darker(0.15f).getPerceivedBrightness() > 0.55f ? juce::Colours::black : juce::Colours::white);
    ...
    drawTargetButton.setColour(juce::TextButton::buttonOnColourId, sideCol2);
    drawTargetButton.setColour(juce::TextButton::textColourOnId,
        sideCol2.getPerceivedBrightness() > 0.55f ? juce::Colours::black : juce::Colours::white);
    drawTargetButton.setColour(juce::TextButton::buttonColourId, mainCol.darker(0.5f));
    drawTargetButton.setColour(juce::TextButton::textColourOffId, mainCol);
    applyBtnTextColor(drawTargetButton);
...
void SpectralCompareAudioProcessorEditor::mouseDown(const juce::MouseEvent& event)
{
    ...
    if (drawingMain)
        audioProcessor.setMainFilterCurveRange(bin, bin, dB, dB);
    else
        audioProcessor.setSidechainFilterCurveRange(bin, bin, dB, dB);
}
...
void SpectralCompareAudioProcessorEditor::mouseDrag(const juce::MouseEvent& event)
{
    ...
    const float startDB = juce::jlimit(-kFilterRange, kFilterRange, filterYToDB(filterLastDragY));
    const int   endBin = filterXToBin(x);
    const float endDB = juce::jlimit(-kFilterRange, kFilterRange, filterYToDB(y));

    if (drawingMain)
        audioProcessor.setMainFilterCurveRange(startBin, endBin, startDB, endDB);
    else
        audioProcessor.setSidechainFilterCurveRange(startBin, endBin, startDB, endDB);
...
void SpectralCompareAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    if (showSidechain)
        drawSpectrum(g, sidechainIsFrozen ? frozenSidechainDisplay : sidechainDisplayData,
            audioProcessor.getSidechainSpectrumColor(), 0.7f);
    if (showMain)
        drawSpectrum(g, mainIsFrozen ? frozenMainDisplay : mainDisplayData,
...
void SpectralCompareAudioProcessorEditor::drawFxRegionBars(juce::Graphics& g)
{
...

    {
        ...
        const juce::Colour col = juce::Colour(0xffdd4444).withAlpha(on ? 0.75f : 0.25f);
        g.setColour(col);
        ...
        g.setFont(8.0f * scaleFactor);
        if (x1 >= ca.getX() && x1 <= ca.getRight()) g.drawText(gLabel, (int)x1 + 1, ca.getY() + 2, (int)(16 * scaleFactor), (int)(10 * scaleFactor), juce::Justification::left, false);
        if (x2 >= ca.getX() && x2 <= ca.getRight() && (int)x2 != (int)x1) g.drawText(gLabel, (int)x2 + 1, ca.getY() + 2, (int)(16 * scaleFactor), (int)(10 * scaleFactor), juce::Justification::left, false);
    }
    {
        ...
        const juce::Colour col = juce::Colour(0xff44aa66).withAlpha(on ? 0.75f : 0.25f);
        g.setColour(col);
        float x1 = hzToBinX(loHz), x2 = hzToBinX(hiHz);
        if (x1 >= ca.getX() && x1 <= ca.getRight()) g.drawVerticalLine((int)x1, (float)ca.getY(), (float)ca.getBottom());
        if (x2 >= ca.getX() && x2 <= ca.getRight() && (int)x2 != (int)x1) g.drawVerticalLine((int)x2, (float)ca.getY(), (float)ca.getBottom());
        g.setFont(8.0f * scaleFactor);
        if (x1 >= ca.getX() && x1 <= ca.getRight()) g.drawText(eLabel, (int)x1 + 1, ca.getY() + (int)(14 * scaleFactor), (int)(40 * scaleFactor), (int)(10 * scaleFactor), juce::Justification::left, false);
        if (x2 >= ca.getX() && x2 <= ca.getRight() && (int)x2 != (int)x1) g.drawText(eLabel, (int)x2 + 1, ca.getY() + (int)(14 * scaleFactor), (int)(40 * scaleFactor), (int)(10 * scaleFactor), juce::Justification::left, false);
    }
}
...
void SpectralCompareAudioProcessorEditor::drawOutputSpectrum(juce::Graphics& g)
{
    ...
    if (interpolate)
    {
        ...
        g.setColour(col.withAlpha(colAlpha2 * 0.10f));
        g.fillPath(fillPath);
        g.setColour(col.withAlpha(colAlpha2 * 0.95f));
        g.strokePath(fillPath, juce::PathStrokeType(2.0f * scaleFactor, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
        // redraw stroke only along the top line
...
void SpectralCompareAudioProcessorEditor::drawMorphedSpectrum(juce::Graphics& g)
{
    ...
    if (!interpolate)
    {
        g.setColour(col.withAlpha(morphColAlpha * 0.12f));
        for (int bin = 0; bin < nb; ++bin)
        {
            float x = binToX(bin);
            if (x < ca.getX() || x > ca.getRight()) continue;
...
void SpectralCompareAudioProcessorEditor::drawLabels(juce::Graphics& g)
{
    ...
    if (showMain)
        entry(audioProcessor.getMainSpectrumColor(), "Main" + juce::String(mainIsFrozen ? " [frozen]" : ""));
    if (showSidechain)
        entry(audioProcessor.getSidechainSpectrumColor(), "Sidechain" + juce::String(sidechainIsFrozen ? " [frozen]" : ""));
    if (showDelta)
...
void SpectralCompareAudioProcessorEditor::drawHoverInfo(juce::Graphics& g)
{
    ...
    // FIX 1: No black background box — text rendered directly onto the canvas
    const int tw = (int)(600 * scaleFactor), th = (int)(18 * scaleFactor);
    int tx = juce::jlimit(ca.getX(), ca.getRight() - tw, (int)hoverX + (int)(8 * scaleFactor));
    int ty = ca.getBottom() - th - (int)(4 * scaleFactor);
    g.setColour(audioProcessor.getTextColor());
    g.setFont(11.0f * scaleFactor);
    g.drawText(info, tx + (int)(4 * scaleFactor), ty, tw - (int)(8 * scaleFactor), th, juce::Justification::centredLeft, false);
}
...
void SpectralCompareAudioProcessorEditor::drawFilterCurves(juce::Graphics& g)
{
    ...
    auto drawCurve = [&](const std::array<float, maxDisplayBins>& curve, juce::Colour col, bool isActive)
        {
            juce::Path path;
            bool started = false;
            for (int bin = 0; bin < nb; ++bin)
    ...
    const bool mainActive = drawingMain;
    if (!mainActive)
        drawCurve(mainFilterDisplay, audioProcessor.getMainSpectrumColor(), false);
    else
        drawCurve(sidechainFilterDisplay, audioProcessor.getSidechainSpectrumColor(), false);

    if (mainActive)
        drawCurve(mainFilterDisplay, audioProcessor.getMainSpectrumColor(), true);
    else
        drawCurve(sidechainFilterDisplay, audioProcessor.getSidechainSpectrumColor(), true);
}
...
void SpectralCompareAudioProcessorEditor::resized()
{
    ...
    gateEnableButton.setVisible(vis);    gateThresholdSlider.setVisible(vis);
    gateThreshLabel.setVisible(vis);     gateLowHzLabel.setVisible(vis);
    ...
    enhanceLevelSlider.setVisible(vis);  enhLevelLabel.setVisible(vis);
    ...
    morphSmoothToggle.setVisible(vis); drawTargetButton.setVisible(vis);
    ...
    hearDeltaButton.setBounds(cell(0)); monitorSideButton.setBounds(cell(1)); nextRow();
    gateEnableButton.setBounds(cell(0)); nextRow();
    labeledRow(gateThreshLabel, gateThresholdSlider);
    ...
    labeledRow(enhLevelLabel, enhanceLevelSlider);
    ...
    morphSmoothToggle.setBounds(cell(0)); drawTargetButton.setBounds(cell(1)); resetDrawsButton.setBounds(cell(2));
}
