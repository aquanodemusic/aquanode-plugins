...
private:
    ...
    std::atomic<float>* enhanceLevelParam = nullptr;
    ...
    // Sidechain latency-compensation delay for monitor mode (fftSize samples)
    std::vector<float> sideDelayL;
    std::vector<float> sideDelayR;
    int                sideDelayPos = 0;
    ...
    // -----------------------------------------------------------------------
    std::array<float, maxBins> mainFilterCurve_write{};      // written on message thread
    std::array<float, maxBins> sidechainFilterCurve_write{};
    std::array<float, maxBins> mainFilterCurve_read{};       // audio-thread working copy
    std::array<float, maxBins> sidechainFilterCurve_read{};
    std::array<float, maxBins> mainFilterGain{};             // linear gains (audio thread)
