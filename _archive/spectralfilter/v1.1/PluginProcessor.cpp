...
void SpectralFilterAudioProcessor::shiftFilterCurve(int shiftAmount)
{
    {
        juce::ScopedLock lock(filterCurveLock);

        int n = numBins;
        shiftAmount = ((shiftAmount % n) + n) % n;
        if (shiftAmount == 0) return;

        std::rotate(filterCurveDB_write.begin(),
                    filterCurveDB_write.begin() + (n - shiftAmount),
                    filterCurveDB_write.begin() + n);
    }
    curveIsDirty.store(true, std::memory_order_release);
}

void SpectralFilterAudioProcessor::setFilterCurveShifted(const std::array<float, maxBins>& baseline, int shiftAmount)
{
    {
        juce::ScopedLock lock(filterCurveLock);
        ...
        // Write baseline rotated right by shiftAmount (bin i goes to bin (i+shift) % n)
        for (int i = 0; i < n; ++i)
            filterCurveDB_write[(i + shiftAmount) % n] = baseline[i];
    }
    curveIsDirty.store(true, std::memory_order_release);
}
...
void SpectralFilterAudioProcessor::resetColors()
{
    ...
    spectrumColor   = juce::Colour(0xff669999);
}

void SpectralFilterAudioProcessor::exportImpulseResponse(const juce::File& outputFile)
{
    // Use current FFT size for the impulse response
    const int irLength = fftSize;

    // Create a buffer for frequency domain data (interleaved real/imaginary)
    std::vector<float> frequencyData(irLength * 2, 0.0f);

    // Account for overlap-add processing characteristics
    // The plugin uses 75% overlap with double windowing (analysis + synthesis)
    // This effectively applies a squared Hann window to the frequency response
    std::vector<float> windowCompensation(numBins);
    for (int bin = 0; bin < numBins; ++bin)
    {
        // Frequency in Hz
        float freq = (static_cast<float>(bin) / static_cast<float>(numBins - 1)) * (currentSampleRate * 0.5f);

        // Hann window in frequency domain has a main lobe width
        // For double windowing (analysis + synthesis), we need less compensation
        // But the overlap-add with 75% overlap creates a near-unity reconstruction
        windowCompensation[bin] = 1.0f;  // Start with unity

        // Very slight high-frequency boost to compensate for window rolloff
        // Hann window has about -1.5 dB at the edges in frequency domain
        if (freq > 1000.0f)
        {
            float normalizedFreq = freq / (currentSampleRate * 0.5f);
            float boost = 1.0f + (normalizedFreq * normalizedFreq * 0.05f);  // Subtle boost
            windowCompensation[bin] = boost;
        }
    }

    // Fill the frequency domain with current filter curve
    {
        juce::ScopedLock lock(filterCurveLock);

        // DC bin (bin 0) - real only
        float magnitude = std::pow(10.0f, filterCurveDB_write[0] / 20.0f) * windowCompensation[0];
        frequencyData[0] = magnitude;
        frequencyData[1] = 0.0f;

        // Positive frequencies: bins 1 to N/2-1
        for (int bin = 1; bin < numBins - 1; ++bin)
        {
            float magnitude = std::pow(10.0f, filterCurveDB_write[bin] / 20.0f) * windowCompensation[bin];
            frequencyData[bin * 2] = magnitude;
            frequencyData[bin * 2 + 1] = 0.0f;
        }

        // Nyquist bin (bin N/2) - real only
        int nyquistBin = numBins - 1;
        float nyquistMag = std::pow(10.0f, filterCurveDB_write[nyquistBin] / 20.0f) * windowCompensation[nyquistBin];
        frequencyData[nyquistBin * 2] = nyquistMag;
        frequencyData[nyquistBin * 2 + 1] = 0.0f;

        // Negative frequencies: bins N/2+1 to N-1 (mirror of positive frequencies)
        for (int bin = numBins; bin < irLength; ++bin)
        {
            int mirrorBin = irLength - bin;
            if (mirrorBin > 0 && mirrorBin < numBins)
            {
                frequencyData[bin * 2] = frequencyData[mirrorBin * 2];
                frequencyData[bin * 2 + 1] = -frequencyData[mirrorBin * 2 + 1];
            }
        }
    }
    ...
    irFFT->performRealOnlyInverseTransform(frequencyData.data());

    // Copy to impulse buffer
    std::vector<float> impulse(irLength);
    for (int i = 0; i < irLength; ++i)
        impulse[i] = frequencyData[i];

    // Normalize to prevent clipping
    float maxVal = 0.0f;
    for (int i = 0; i < irLength; ++i)
        maxVal = juce::jmax(maxVal, std::abs(impulse[i]));
    ...
    if (outputFile.hasWriteAccess() || outputFile.create())
    {
        ...
        if (writer != nullptr)
        {
            const float* channelData = impulse.data();
            writer->writeFromFloatArrays(&channelData, 1, irLength);
        }
    }
}
...
void SpectralFilterAudioProcessor::performSpectralFilter(int channel)
{
    ...
    // If wetOnly, snapshot the pre-gain (dry) bins so we can subtract them after
    const bool isWetOnly = wetOnly.load(std::memory_order_relaxed);
    std::array<float, maxBins * 2> dryBins;
    if (isWetOnly)
    {
        for (int bin = 0; bin < numBins; ++bin)
        {
            dryBins[bin * 2]     = fftBuffer[channel][bin * 2];
            dryBins[bin * 2 + 1] = fftBuffer[channel][bin * 2 + 1];
        }
    }
    ...
    for (int bin = 0; bin < numBins; ++bin)
    {
        ...
        fftBuffer[channel][bin * 2]     *= gain;
        fftBuffer[channel][bin * 2 + 1] *= gain;
    }

    // Subtract dry bins to get difference (filtered - dry) only
    if (isWetOnly)
    {
        for (int bin = 0; bin < numBins; ++bin)
        {
            fftBuffer[channel][bin * 2]     -= dryBins[bin * 2];
            fftBuffer[channel][bin * 2 + 1] -= dryBins[bin * 2 + 1];
        }
    }

    // ---- Bin shift (frequency shift effect) ----
    const int shift = binShiftAmount.load(std::memory_order_relaxed);
    if (shift != 0)
    {
        std::vector<float> tmp(fftBuffer[channel].begin(),
                               fftBuffer[channel].begin() + numBins * 2);

        const bool wrap = binShiftWrap.load(std::memory_order_relaxed);
        const int  n    = numBins;

        std::fill(fftBuffer[channel].begin(),
                  fftBuffer[channel].begin() + numBins * 2, 0.0f);
        ...
        for (int bin = 0; bin < n; ++bin)
        {
            ...
            fftBuffer[channel][dest * 2]     = tmp[bin * 2];
            fftBuffer[channel][dest * 2 + 1] = tmp[bin * 2 + 1];
        }
    }

    // ---- Update display magnitudes (after shift so spectrogram matches output) ----
    if (channel == 0)
    {
        ...
        for (int bin = 0; bin < numBins; ++bin)
        {
            ...
            smoothedSpectrumMagnitudes[bin] =
                smoothedSpectrumMagnitudes[bin] * smooth + mag * (1.0f - smooth);
        }
    }
    ...
    for (int channel = 0; channel < totalIn; ++channel)
    {
        ...
        for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
        {
            ...
            if (inputFifoIndex[channel] >= fftSize)
            {
                ...
                // ---- Auto-shift: drive on channel 0 once per hop frame ----
                if (channel == 0)
                {
                    const float secondsPerHop = static_cast<float>(hopSize) / static_cast<float>(currentSampleRate);

                    // Filter curve auto-shift
                    if (autoShiftFilter.load(std::memory_order_relaxed))
                    {
                        autoShiftFilterAccum += autoShiftFilterSpeed.load(std::memory_order_relaxed) * secondsPerHop;
                        int steps = static_cast<int>(autoShiftFilterAccum);
                        if (steps != 0)
                        {
                            autoShiftFilterAccum -= static_cast<float>(steps);
                            // rotate write buffer directly (audio thread holds no UI lock here,
                            // but we need filterCurveLock to touch filterCurveDB_write)
                            {
                                juce::ScopedLock lock(filterCurveLock);
                                int n = numBins;
                                steps = ((steps % n) + n) % n;
                                std::rotate(filterCurveDB_write.begin(),
                                            filterCurveDB_write.begin() + (n - steps),
                                            filterCurveDB_write.begin() + n);
                            }
                            curveIsDirty.store(true, std::memory_order_release);
                        }
                    ...
                    {
                        autoShiftBinAccum += autoShiftBinSpeed.load(std::memory_order_relaxed) * secondsPerHop;
