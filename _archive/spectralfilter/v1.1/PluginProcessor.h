...
public:
    ...
    // Shift filter curve bins by shiftAmount (wraps around)
    void shiftFilterCurve(int shiftAmount);

    // Apply a pre-captured baseline curve shifted by shiftAmount (absolute, wrapping)
    void setFilterCurveShifted(const std::array<float, maxBins>& baseline, int shiftAmount);

    // Reset UI colours to their factory defaults
    void resetColors();
    
    // Export current filter as impulse response WAV file
    void exportImpulseResponse(const juce::File& outputFile);

    // Bin shift slider — shifts FFT bins during processing (frequency shift effect)
    std::atomic<int>  binShiftAmount { 0 };
    std::atomic<bool> binShiftWrap   { true };
    std::atomic<bool> wetOnly        { false };

    // Auto-shift: filter curve
    std::atomic<bool>  autoShiftFilter       { false };
    std::atomic<float> autoShiftFilterSpeed  { 1.0f };

    // Auto-shift: bin shift
    std::atomic<bool>  autoShiftBin          { false };
    std::atomic<float> autoShiftBinSpeed     { 1.0f };

    // Auto-shift accumulators (written from audio thread, reset from editor on toggle-off)
    float autoShiftFilterAccum { 0.0f };
    float autoShiftBinAccum    { 0.0f };
    int   autoShiftBinPos      { 0 }; // output = filtered - dry (difference only)
