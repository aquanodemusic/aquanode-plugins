...
public:
    ...
    void prepare (double sr) override
    {
        ...
        // ~12 ms one-pole on the DCW knob values: turning DCW while sound is
        // passing used to step the waveshape once per UI update, which is
        // audible as crackle. The DCW IN socket stays unsmoothed so audio-rate
        // phase-distortion modulation is untouched.
        dcwSmoothCoeff = 1.0f - std::exp ((float) (-1.0 / (0.012 * sr)));
        reset();
    }
    ...
    void voiceReset (int v) override
    {
        ...
        gate.resetVoice (v);
        phase[v] = 0.0;
        // snap the smoothers to the current knob positions so a fresh voice
        // never sweeps in from a stale shape
        dcwSm[v] = getParameterBase (pDcw);
        dcwModSm[v] = getParameterBase (pDcwMod);
    }
...
private:
    ...
    // per-voice smoothed DCW / DCW Mod knob values (zipper removal)
    float dcwSm    [aquanode::kMaxVoices] {};
    float dcwModSm [aquanode::kMaxVoices] {};
    float dcwSmoothCoeff { 0.002f };
};
