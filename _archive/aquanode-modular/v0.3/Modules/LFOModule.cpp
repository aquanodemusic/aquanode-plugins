#include "LFOModule.h"

using namespace aquanode;

void LFOModule::processSample (const StereoFrame*, StereoFrame* outputs)
{
    const double inc = param (pRate) / sampleRate;
    phase += inc;
    if (phase >= 1.0)
    {
        phase -= std::floor (phase);
        shValue = random.nextFloat() * 2.0f - 1.0f;   // new random value each cycle
    }

    float wave = 0.0f;
    switch ((int) param (pWaveform))
    {
        case 0: wave = (float) std::sin (phase * juce::MathConstants<double>::twoPi); break;   // Sine
        case 1: wave = (float) (1.0 - 4.0 * std::abs (phase - 0.5)); break;                    // Triangle
        case 2: wave = (float) (2.0 * phase - 1.0); break;                                     // Saw Up
        case 3: wave = (float) (1.0 - 2.0 * phase); break;                                     // Saw Down
        case 4: wave = phase < 0.5 ? 1.0f : -1.0f; break;                                      // Square
        case 5: wave = shValue; break;                                                         // Random S&H
    }

    // Offset shifts the centre point, so the output can be made fully unipolar
    const float out = juce::jlimit (-1.0f, 1.0f, wave * param (pLevel) + param (pOffset));
    outputs[0][0] = out;
    outputs[0][1] = out;
}

static ModuleDescriptor lfoDescriptor()
{
    ModuleDescriptor d;
    d.typeId = "util.lfo";
    d.displayName = "LFO";
    d.section = ModuleSection::Utility;
    d.sidebarOrder = 0;
    d.sockets = { modOut ("modOut", "Mod Out") };
    d.params = {
        makeRotary ("rate", "Rate", 0.1f, 100.0f, 1.0f, 0, "Hz", true),
        makeCombo  ("waveform", "Waveform",
                    { "Sine", "Triangle", "Saw Up", "Saw Down", "Square", "Random S&H" }, 0, 0, 2),
        makeRotary ("offset", "Offset", -1.0f, 1.0f, 0.0f, 0),
        makeRotary ("level",  "Level",  0.0f, 1.0f, 1.0f, 0)
    };
    return d;
}

AQUANODE_REGISTER_MODULE (LFOModule, lfoDescriptor)
