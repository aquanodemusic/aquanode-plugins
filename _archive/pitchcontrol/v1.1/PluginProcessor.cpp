...
PitchControlAudioProcessor::createParameterLayout()
{
    ...
    // Boost gain for protected frequencies (0 to +6 dB)
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        boostDBParamID(), "Boost (dB)",
        juce::NormalisableRange<float>(0.0f, 6.0f, 0.1f),
        0.0f));

    // Boost Q (how narrow the boost is)
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
...
void PitchControlAudioProcessor::rebuildFilters()
{
    ...
    for (int n = 0; n < kTotalNotes; ++n)
    {
        ...
        // Boost filters for PROTECTED notes (opposite logic from cut filters)
        // In Normal Mode: Boost protected notes
        // In Wet Only Mode: Boost non-protected notes (to enhance the isolated color)
        bool shouldBoostThisNote = m_wetOnly ? !isNoteProtected : isNoteProtected;
        
        bool boostActive = anyProtected
            && (n >= rangeFrom && n <= rangeTo)
            && shouldBoostThisNote
            && (boostDB > 0.01f); // Only activate if boost > 0
        ...
        if (boostActive)
        {
            anyBoost = true;
            double fc = midiNoteToHz(n);
            m_boostFilters[n] = makePeakingEQ(fc, m_sampleRate, (double)boostDB, (double)boostQ);
        }
        else
        {
            m_boostFilters[n] = Biquad{}; // Identity
        }
    }
...
void PitchControlAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    for (int ch = 0; ch < numCh; ++ch)
    {
        ...
        for (int s = 0; s < numSamples; ++s)
        {
            ...
            // Apply active cut filters in series
            for (int n = 0; n < kTotalNotes; ++n)
            {
                if (m_filterActive[n])
                {
            ...
            // Apply boost filters to the processed signal (enhances protected frequencies)
            for (int n = 0; n < kTotalNotes; ++n)
            {
                if (m_boostActive[n])
                {
                    processedSample = m_boostFilters[n].process(processedSample, m_boostStates[ch][n]);
                }
            }
