    ...
    m_rangeTo("Range To", PitchControlAudioProcessor::rangeToParamID(), p)
{
    ...
    m_titleLabel.setText("PitchControl by aquanode (If Q is high, lower Depth!)", juce::dontSendNotification);
...
void PitchControlAudioProcessorEditor::resized()
{
    ...
    // Controls row - 4 knobs + vertical stack of controls on the right
    const int ctrlY = 258;
    const int ctrlH = 128;
    const int knobW = 95;
    const int gap = 10;
    ...
    const int stackW = 130;
    const int stackItemH = 38;
    ...
    const int stackY = ctrlY + (ctrlH - (stackItemH * 3 + stackGap * 2)) / 2;

    m_rangeFrom.setBounds(stackX, stackY - 10, stackW, stackItemH);
    m_rangeTo.setBounds(stackX, stackY + stackItemH + stackGap - 8, stackW, stackItemH);
    m_wetOnlyButton.setBounds(stackX, stackY + 2 * (stackItemH + stackGap) -5, stackW, stackItemH);
}